...
helper functions for printing and serializing this data (as strings, S-expressions, and JSON, respectively). For a type named `t`, the `show` function will be named `show`. Otherwise,
for a type named something else like `q`, it will be `show_q`.

If you are working with an anonymous type, e.g. a tuple or list type, you can derive a 
...
#### Clean Reset
If you want to a fully clean reset you should 
