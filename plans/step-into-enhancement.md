...
_From the planning conversation:_
...
2. **Why the environment dropdown**: Putting step into there _enforces_ that a sample is selected - you can't even see the menu without having indicated a sample. This sidesteps edge cases about "what if no sample is selected." The user mentioned they might revisit putting it back in the syntax context menu later, but wants the simpler case first.
...
_Implemented 2025-01-11_

### Phase 1: Refactor Environment Dropdown ✓

- Split `env_view` into three functions: `sample_context_actions`, `sample_environment`, `sample_context_menu`
- Updated CSS class names from `sample-dropdown` to `sample-context-menu`
- SafeTriangle behavior preserved

### Phase 2: Move Step Into to Sample Context Menu ✓

- Added `StepIntoSample(Language.Sample.t, Id.t)` to `Action.refractor`
- Created `step_into_sample` function in `Refractors.re` that uses `sample.call_stack` directly
- Added `Refractor(Action.refractor)` variant to `external_action` in `ProjectorBase.re`
- Added Step Into button to context menu UI with icon

### Phase 3: Immediate Focus ✓

- Added `pending_focus` type to `Sample.Cursor.t` with `probe_id` and `target_stack`
- `step_into_sample` sets `pending_focus` before jumping
- Added `ResolvePendingFocus(list(Sample.t))` to `Action.dyn_cursor`
- Pending focus resolved in `Editor.Update.calculate` when matching sample found

### Phase 4: Evaluating Indicator ✓

- Added `Evaluating` variant to `Sample.Selection.empty_status`
- Added `~is_evaluating` parameter to `get_empty_status`
- ProbeProj detects evaluating state via `pending_focus.probe_id == current_probe_id`
- Animated spinner (⟳) with CSS `@keyframes spin` animation
- ProbeText.re returns "..." for Evaluating state

### Phase 5: Polish ✓

- All tests pass (ProbeSteps, Evaluator)
- Edge cases handled gracefully (no match keeps pending, multiple matches uses first)
- Step into on non-function does nothing (graceful degradation)

### Addendum: Scroll Fix

- Added `StepIntoSample(_)` to `scroll_active` cases in `CodeEditable.re`
- Without this, the caret wouldn't scroll into view after step-into

### Addendum: Event Propagation Fix

- **Problem**: Step into wasn't visually working despite the function returning correctly
- **Root cause**: The Step Into button's click event was bubbling up to the projector wrapper, which triggered `Project(Focus(id, kind, None))` and moved the cursor back to the probe
- **Fix**: Added `Effect.Stop_propagation` to the Step Into button's `on_pointerdown` handler in ProbeProj.re
- This prevents the wrapper's Focus action from overwriting the cursor position set by step_into_sample

### Addendum: Sample ID Mismatch Fix

- **Problem**: pending_focus couldn't find samples because the probe_id was wrong
- **Root cause**: For function literals, we need to distinguish between:
  - `jump_target` (pattern ID) - where cursor goes for UX
  - `sample_probe_id` (inner body ID) - where samples are stored in dynamics
  - `target_subterm_ids(Fun)` returns `[inner_body, pattern]`, and samples are stored under `inner_body`
- **Fix**: Use `sample_probe_id = inner_body_id` for pending_focus lookup while still using `jump_target = pat_id` for cursor placement

### Files Modified

- `src/language/dynamics/Sample.re` - pending_focus type, Evaluating status
- `src/haz3lcore/Refractors.re` - step_into_sample function
- `src/haz3lcore/zipper/action/Action.re` - StepIntoSample, ResolvePendingFocus
- `src/haz3lcore/zipper/action/DynCursorPerform.re` - resolve_pending_focus
- `src/haz3lcore/projectors/implementations/ProbeProj.re` - UI refactoring
- `src/haz3lcore/projectors/ProbeText.re` - Evaluating case
- `src/haz3lcore/projectors/ProjectorBase.re` - Refractor in external_action
- `src/web/app/common/ProjectorView.re` - handle Refractor action
- `src/haz3lcore/Editor.re` - pending focus resolution in calculate
- `src/web/app/editors/code/CodeEditable.re` - scroll_active for StepIntoSample
- `src/web/www/style/projectors/proj-probe.css` - context menu and evaluating styles
