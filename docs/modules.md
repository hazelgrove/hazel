...
own type members substituted) at its last definition — one a later item
shadows is only synthesized — and each type member must be defined as the
...
member is reported on its definition type (`ModuleTypeMemberMismatch`). When
one of the items is a hole (a hole item,
...
- `Typ.coercion(ctx, ~from, ~to_)` is coercive subtyping, checked only where
  ...
  members under fresh names, an abstract one as a fresh abstract type
  variable). An abstract member of the required signature is realized by
  whatever the provided signature declares for it; a manifest member is not
  satisfied by an abstract one. It returns the sealed signature, which is what
...
- `ModuleHelpers.missing_members` produces `Mark.ModuleMissingMembers` on
  ...
  as missing); extra members are sealed away by `Typ.coercion` at a coercion
  site and are a mismatch elsewhere. Any other expression of signature type
  analyzed against a signature that declares members it lacks gets the same
  mark instead of a generic mismatch (`StaticsBase.expectation_mismatch_mark`),
  so a module variable or a function argument names its missing members too.
...
| `src/language/statics/Mark.re`          | `ModuleMissingMembers`, `ModuleTypeMemberMismatch`, `ModuleMemberNotFound`, `ModuleTypeMemberNotFound`, `TypWantModule`, `TypeMemberCapture`, `TypAbstractMemberOfSignature`, `SigDuplicateMember`, `ImplicitNotFound`, `ImplicitAmbiguous`, `ImplicitBinderNotModule`, `ImplicitBinderPosition` |
...
| `test/Test_Typ.re`                         | `Typ.Sig`, `Typ.Coercion`, `Typ.SigPaths`, `Typ.SubstCaptures`, `Typ.Implicit`: meet, coercion, paths, strengthening, capture reporting, binders |
