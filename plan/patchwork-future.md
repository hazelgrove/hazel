...
- [ ] Clean up stale remote carets on peer disconnect
  - **Current behavior:** When a user refreshes their browser, they get a new `senderId`. The old caret (keyed by old senderId) is never removed, so ghost carets accumulate in other clients' editors.
  - **Fix:** Listen for Automerge peer disconnect events and send `remote-caret-remove`, or implement timeout-based cleanup (remove carets not updated in X seconds)
