...
let normalize_tests = (
  "Typ.normalize",
  IdTagged.FreshGrammar.Typ.[
    test_case("Type-level beta reduction", `Quick, () => {
      let ctx = Builtins.ctx_init(None);
      let ty =
        typ_app(
          typ_lam(Var("a") |> TPat.temp, list(var("a"))),
          int(),
        );
      check(typ, "beta-normalized type application", list(int()), Typ.normalize(ctx, ty));
    }),
    test_case("Parameterized alias application normalizes", `Quick, () => {
      let option_ty =
        typ_lam(
          Var("a") |> TPat.temp,
          sum([
            ...
            Variant("Some", ConstructorMap.empty_variant_ann, Some(var("a"))),
          ]),
        );
      let ctx =
        Ctx.extend_alias(
      ...
      check(
        ...
        "instantiated option",
        sum([
          ConstructorMap.Variant("None", ConstructorMap.empty_variant_ann, None),
          Variant("Some", ConstructorMap.empty_variant_ann, Some(int())),
        ]),
        normalized,
      );
    }),
    test_case("Recursive parameterized alias exposes sum constructors", `Quick, () => {
      let list_name = "List";
      ...
      let list_body =
        typ_lam(
          param,
          sum([
            ConstructorMap.Variant("Nil", ConstructorMap.empty_variant_ann, None),
            Variant(
              "Cons",
              ConstructorMap.empty_variant_ann,
      ...
      let list_ty = Rec(Var(list_name) |> TPat.temp, list_body) |> Typ.temp;
      let ctx =
        Ctx.extend_alias(
          Builtins.ctx_init(None),
          list_name,
