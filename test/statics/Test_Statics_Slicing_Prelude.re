...
let cons_head_ctor = (e: Exp.t, name: string): Id.t =>
  first(
    "cons head constructor " ++ name,
    collect_exp_ids(
      x =>
        switch (Exp.term_of(x)) {
        | Constructor(v, _)
            when
              ...
              && List.exists(
                   parent =>
                     ...
                     | Cons(head, _) => Exp.rep_id(head) == Exp.rep_id(x)
                     | _ => false
                     },
                   [e],
                 ) =>
...
let fix_body_int = (e: Exp.t, n: int): Id.t =>
  switch (Exp.term_of(e)) {
  | FixF(_, body, _) =>
    switch (Exp.term_of(body)) {
    | Atom(Int(v)) when Bigint.to_string(v) == string_of_int(n) =>
      Exp.rep_id(body)
    | _ => Alcotest.fail("fix body int literal not found")
    }
  | _ => Alcotest.fail("fix body int literal not found")
  };
...
let exp_hole = SliceReconstruct.exp_hole;
let pat_hole = SliceReconstruct.pat_hole;
let typ_hole = SliceReconstruct.typ_hole;
let tpat_hole = SliceReconstruct.tpat_hole;

let reconstruct = SliceReconstruct.reconstruct;
