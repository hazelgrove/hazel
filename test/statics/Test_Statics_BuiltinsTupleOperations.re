  ...
  let tests = [
    ...
    test_case("project_labels with non-label", `Quick, () =>
      annotated_tree_test(
        ...
        prod([int(), unknown(Internal |> Prov.fresh), int()]),
        FIError.Exp.(
          ap(
            Forward,
            var("project_labels"),
...
module GroupByLabel = {
  let tests = [
    fully_consistent_typecheck(
      ...
      Some(unknown(Internal |> Prov.fresh)),
    ),
    fully_consistent_typecheck(
              ...
              tuple([
                list_lit([
                  asc(
                    ...
                    Typ.unknown(FIError.TypeProvenance.internal()),
                  ),
                ]),
                label("b"),
              ]),
