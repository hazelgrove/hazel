...
let parse_digit_examples = [
  synthesis_case(
    ...
    ex1_src,
    "String -> Option(Digit)",
    "type Option = typfun A -> ? + Some(A) in type ? = ? in let parse_digit = fun ? : String -> case ? | ? => Some(?) | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? end in parse_digit(?)",
  ),
  analysis_case(
    ~focus=str_lit("5"),
    "ex1-parse-digit-ana",
...
let demo_examples = [
  ...
  demo_syn_case(
    ...
    "type Option = typfun A -> ? + Some(A) in type ? = ? in type ? = ? in let ? = fun ? -> case ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? | ? => Some(?) | ? => ? | ? => ? | ? => ? | ? => ? | ? => ? end in ?",
  ),
  demo_syn_case(
...
let if_joins = [
  ...
  synthesis_case(
    ~ctx=ctx_var(~ctx=prelude_ctx(either_prelude), "c", "Bool"),
    "if-join-either-explicit-sum-left",
    "if c then Left@<Int, ?>(1) else Right@<?, Bool>(true)",
    "+ Left(Int) + ?",
