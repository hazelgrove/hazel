...
let test_let_ref_to_binding =
  test_case(
    "Let: reference highlights binding + sibling uses",
    `Quick,
    () => {
...
let test_let_binding_to_refs =
  test_case(
    "Let: binding highlights all uses",
    `Quick,
    () => {
...
let test_fun_param =
  test_case(
    "Fun: parameter binding highlights body uses",
    `Quick,
    () => {
...
let test_match_case =
  test_case(
    "Match: case binding highlights body uses",
    `Quick,
    () => {
...
let test_match_ref_to_binding =
  test_case(
    "Match: reference highlights case binding",
    `Quick,
    () => {
...
let test_shadowing =
  test_case(
    "Shadowing: inner binding doesn't highlight outer",
    `Quick,
    () => {
...
let test_multiple_refs =
  test_case(
    "Multiple references all highlighted",
    `Quick,
    () => {
...
let test_tvar_binding_to_refs =
  test_case(
    ...
    () => {
      let exp =
        parse_exp("type T = Int in let x : T = 1 in let y : T = 2 in x + y");
      let info_map = statics(exp);
      let binding = find_tvar_binding(info_map, "T");
      check(bool, "found T binding", true, binding != None);
...
let test_ctr_ref_to_def =
  test_case(
    ...
    `Quick,
    () => {
      let exp = parse_exp("type T = A + B in let x : T = A in x");
      let info_map = statics(exp);
...
let test_ctr_distinct_defs =
  test_case(
    ...
    () => {
      ...
      check(
        bool,
        "A doesn't highlight B def",
        false,
        has_id(a_ids, b_def_id),
      );
      /* B highlights B's def, not A's */
      check(bool, "B highlights B def", true, has_id(b_ids, b_def_id));
      check(
        bool,
        "B doesn't highlight A def",
        false,
        has_id(b_ids, a_def_id),
      );
    },
  );
...
let test_ctr_def_to_uses =
  test_case(
    ...
    () => {
      let exp =
        parse_exp("type T = A + B in let x : T = A in let y : T = A in x");
      let info_map = statics(exp);
      let def = find_ctr_def(info_map, "A");
      check(bool, "found A definition", true, def != None);
...
let test_ctr_pat_in_case =
  test_case(
    ...
    `Quick,
    () => {
      let exp = parse_exp("type T = A + B in case A | A => A end");
      let info_map = statics(exp);
      let def = find_ctr_def(info_map, "A");
      check(bool, "found A definition", true, def != None);
      let (_, def_info) = Option.get(def);
