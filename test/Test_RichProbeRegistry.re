...
let registry_tests = [
  ...
  test_case(
    ...
    () => {
      ...
      | _ =>
        fail(
          "expected Some((2, [])) after applying ShowMenu(2) to sexp-decoded model",
        )
      };
    },
  ),
  ...
  test_case(
    ...
    () => {
      ...
      | _ =>
        fail(
          "expected Some((2, [])) after applying ShowMenu(2) to yojson-decoded model",
        )
      };
    },
  ),
  ...
  test_case(
    ...
    () => {
      ...
      | Some((2, [])) => ()
      | _ => fail("expected Some((2, [])) after applying sexp-decoded action")
      };
    },
  ),
  test_case(
