...
module PlainTests = {
  ...
  let singleton_labeled_tuple_elaborates_labels = () =>
    alco_check(
      ...
      Exp.(
        let_(
          Pat.(
            asc(var("x"), Typ.(prod([tup_label(label("l"), string())])))
          ),
  ...
  let tests = [
    ...
    test_case("Inconsistent let ascription", `Quick, () =>
      alco_check(
        {|let x : String = 4  in x|},
        Exp.(let_(Pat.(asc(var("x"), Typ.string())), int(4), var("x"))),
    ...
    test_case(
      ...
      alco_check(
        ...
        Exp.(
          let_(
            Pat.(
              asc(var("x"), Typ.(prod([tup_label(label("a"), string())])))
            ),
...
module TableElaborationTests = {
  open Haz3lcore;

  let parse_exp = (s: string) =>
    switch (Parser.to_term(s)) {
    | Some(e) => e
    | None => Alcotest.fail("Failed to parse expression: " ++ s)
    };

  /* Find the first ListLit sub-expression in an elaborated program */
  let find_list_lit = (exp: Exp.t): option(Exp.t) => {
    module M = {
  ...
  let elaborate_and_find_list = (program_str: string): Exp.t => {
  ...
  let assert_table_headers =
      (msg: string, expected_headers: list(string), exp: Exp.t) =>
...
let tests = [
  ...
  ("Table elaboration tests", TableElaborationTests.tests),
];
