        ...
        Let(
          ...
          Let(
            Cast(
              ...
              |> Typ.fresh
              |> TypSlice.t_of_typ_t,
              Unknown(Internal) |> Typ.fresh |> TypSlice.t_of_typ_t,
            )
            |> Pat.fresh,
    ...
    Alcotest.check(
      ...
      info_error_of_id(
        Let(
          Cast(
            Var("y") |> Pat.fresh,
            String |> Typ.fresh |> TypSlice.t_of_typ_t,
            Unknown(Internal) |> Typ.fresh |> TypSlice.t_of_typ_t,
          )
          |> Pat.fresh,
      ...
      () => {
        ...
        let (_typ, tuple, tl1, tl2, int_ty) =
          ...
          | Let(
              ...
              tl1,
              tl2,
              TypSlice.(s |> unparens |> unprod |> List.hd |> typ_of),
            )
          | _ =>
          ...
          Some(
            Common(
              TupleLabelError({
                ...
                invalid_labels: ["c"],
                typ:
                  TupLabel(Label("c") |> Typ.temp, int_ty)
                  |> Typ.temp
                  |> TypSlice.t_of_typ_t,
              }),
            ),
