...
let utyp = (u: UTyp.term): UTyp.t => Typ.fresh(u);
let upat = (u: UPat.term): UPat.t => Pat.fresh(u);
let uexp = (u: UExp.term): UExp.t => Exp.fresh(u);
...
let binop = (op: Operators.op_bin): QCheck.Gen.t(Operators.op_bin) =>
  pure(op);

let int_int_binop: QCheck.Gen.t(Operators.op_bin) =
  frequency([
    (1, binop(Int(Plus))),
    (1, binop(Int(Minus))),
    (1, binop(Int(Times))),
...
let int_bool_binop: QCheck.Gen.t(Operators.op_bin) =
  frequency([
...
let bool_bool_binop: QCheck.Gen.t(Operators.op_bin) =
  frequency([(1, binop(Bool(And))), (1, binop(Bool(Or)))]);

let float_float_binop: QCheck.Gen.t(Operators.op_bin) =
  frequency([
    (1, binop(Float(Plus))),
    (1, binop(Float(Minus))),
    (1, binop(Float(Times))),
...
let float_bool_binop: QCheck.Gen.t(Operators.op_bin) =
  frequency([
...
let upat_typeann = (name: Var.t, ty: UTyp.t) =>
  Cast(upat_var(name), ty, Unknown(Internal) |> Typ.temp) |> upat;

//UExp helpers
let uexp_parens = (u: UExp.t) => uexp(Parens(u));
let uexp_binop = (op: Operators.op_bin, u1: UExp.t, u2: UExp.t) =>
  BinOp(op, u1, u2) |> uexp;
...
let uexp_fun = (name: Var.t, ty: UTyp.t, u: UExp.t) =>
  Fun(upat_typeann(name, ty), u, None, None) |> uexp;
let uexp_var = (name: Var.t) => Var(name) |> uexp;
let uexp_list = (l: list(UExp.t)) => ListLit(l) |> uexp;
let uexp_ap = (u1: UExp.t, u2: UExp.t) => Ap(Forward, u1, u2) |> uexp;
  ...
  QCheck.Gen.sized_size(
    ...
    fix((self, n) =>
      ...
      | 0 =>
        frequency([
          (1, map(uexp_int, int)),
          (1, find_var(ctx, Int |> Typ.temp, map(uexp_int, int))),
        ])
      | n =>
        frequency([
          (1, map(uexp_int, int)),
    ...
    fix((self, n) =>
      ...
      | 0 =>
        frequency([
          ...
          (1, find_var(ctx, Float |> Typ.temp, map(uexp_float, float))),
        ])
      | n =>
        frequency([
          (1, map(uexp_float, float)),
        ...
        frequency([
          ...
          (
            ...
            find_var(
              ctx,
              String |> Typ.temp,
              map(uexp_string, printable_string),
            ),
          ),
        ])
      | n =>
    ...
    fix((self, n) =>
      ...
      | 0 =>
        frequency([
          ...
          (1, find_var(ctx, Bool |> Typ.temp, map(uexp_bool, bool))),
        ])
      | n =>
        frequency([
          (1, map(uexp_bool, bool)),
...
and uexp_ap_gen = (ctx: Ctx.t, ty: UTyp.t, fn: UExp.t, n: int): UExp.t => {
  ...
  | Arrow(ty1, ty2) =>
    ...
    uexp_ap_gen(ctx, ty2, uexp(Ap(Forward, fn, arg)), n);
  | _ => fn
  };
}
...
let rec testcases_gen =
  ...
  | n =>
    ...
    let elab = Elaborator.uexp_elab(m, u);
    let d =
      switch (elab) {
      | DoesNotElaborate => None
      | Elaborates(d, _, _) => Some(d)
      };
    let _ = Option.map(d => print_endline(DHExp.show(d)), d);
    let test = () =>
      Alcotest.check(
        Alcotest.testable(Typ.pp, Typ.eq),
        "Random expression\n" ++ DHExp.show(d |> Option.get),
        ty,
        d
        |> Option.bind(_, TypeAssignment.typ_of_dhexp(Builtins.ctx_init, m))
        |> Option.get,
      );
...
let type_assignment_tests = testcases_gen(1, utyp(Int), []);
...
let random_tests =
  List.map(
    (u: UExp.t) => {
      ...
      let (ty, _, _) = Elaborator.elaborated_type(m, u);
      let test = () =>
        Alcotest.check(
          Alcotest.bool,
          "Random expression",
          true,
          TypeAssignment.property_test(Some(ty), d, m),
        );
      test_case("Type assignment", `Quick, test);
    },
    QCheck.Gen.generate(~n=0, pure_random_uexp),
  );
