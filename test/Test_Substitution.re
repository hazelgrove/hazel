...
let tests = (
  ...
  [
    ...
    test_case(
      ...
      `Quick,
      () => {
        let env = Environment.of_list([("x", Exp.var("x")), ("y", Exp.var("x"))]);
    ...
    test_case(
      ...
      `Quick,
      () => {
        let env = Environment.of_list([("x", Exp.int(1)), ("y", Exp.int(2))]);
        let expr =
          Exp.let_(
            Pat.var("x"),
            Exp.int(3),
    ...
    test_case(
      ...
      `Quick,
      () => {
        let env = Environment.of_list([("f", Exp.var("g")), ("g", Exp.var("h"))]);
