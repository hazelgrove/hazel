...
let get_indicated_constraints = (ctx: Z3.context, (expr : AST.Annotated.t(Haz3lmenhir.AST.exp(Symex.assumptions), Symex.assumptions))) => {
    let (assumptions : list((Symex.assumptions))) = Indicated.extract_indicated_exps(expr);
    List.concat_map(((assumption : (Symex.assumptions))) => {
