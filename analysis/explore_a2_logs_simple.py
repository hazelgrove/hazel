#!/usr/bin/env python3
"""
Simple script to explore and analyze logs in the a2_logs directory.
This version doesn't require pandas and focuses on basic analysis.
"""
...
def main():
    ...
    # 2. Create a summary CSV (without pandas)
    with open(f"a2_logs_summary_{timestamp}.csv", "w") as f:
        f.write("filename,total_actions,duration_seconds,actions_per_second,induction_actions,session_type,success_rate,has_induction,induction_events\n")
        for result in results:
            if 'error' not in result:
                f.write(f"{result['filename']},{result['total_actions']},{result['duration_seconds']:.2f},{result['actions_per_second']:.2f},{result['induction_actions']},{result['session_type']},{result['induction_success_rate']:.3f},{result['has_induction']},{result['induction_events']}\n")
