...
module Utils = {
  ...
  /** Payload for a streaming chat completion; [OpenRouterHttp] POSTs it. */
  let streaming_chat_body = (~payload: Payload.Model.t): Json.t => {
    let streaming_payload = {
      ...payload,
      stream: true,
    };
    Payload.Utils.json_of_payload(~payload=streaming_payload);
  };
};
