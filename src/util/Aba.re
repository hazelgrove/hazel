...
let rev = (rev_a, rev_b, (as_, bs): t('a, 'b)): t('a, 'b) => (
  List.rev_map(~f=rev_a, as_),
  List.rev_map(~f=rev_b, bs),
);
...
let hd = ((as_, _): t('a, 'b)): 'a => List.hd_exn(as_);
...
let map_abas =
  ...
  as_,
  List.map(~f=f_aba, aba_triples(aba)),
);
...
let join = (f_a: 'a => 'c, f_b: 'b => 'c, aba: t('a, 'b)): list('c) => {
  ...
  List.fold_left(
    ~f=(cs, (a, b)) => [f_a(a), f_b(b), ...cs],
    ~init=[f_a(a)],
    List.rev_map2_exn(~f=(a, b) => (a, b), as_, bs),
  );
};
...
let fold_left =
  ...
  List.fold2_exn(~f=f_ba, ~init=f_a(a), bs, as_);
};
let fold_right =
