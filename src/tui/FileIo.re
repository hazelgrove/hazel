...
/* Parse a .haz file into a zipper. Implicit holes round-trip via the
   `¿` marker (see MarkerParse). */
let load = (path: string): option(Zipper.t) =>
  if (Sys.file_exists(path)) {
    let text = read_file(path);
    String.trim(text) == ""
      ? Some(Zipper.init()) : MarkerParse.of_text(~root=Exp, text);
  } else {
    Some
      (Zipper.init()); /* new file */
  };

/* Serialize a zipper the way MarkerParse.to_text does, so saved files
...
let zipper_to_text = (z: Zipper.t): string => {
  ...
  Printer.of_segment(
    ~holes=MarkerParse.default_implicit_hole,
    ~concave_holes=MarkerParse.default_implicit_hole,
    ~indent="",
