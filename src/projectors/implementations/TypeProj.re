...
module Make: Projector =
  (
    ...
    let typ_view = (model, info: info('s), utility, view_seg: View.seg('s)) => {
    ...
    let syntax_view = (info: info('s)) => info |> syntax_str |> text;
    ...
    let view =
        (
          ...
          ~parent as _: external_action('s) => Ui_effect.t(unit),
          ~view_seg: View.seg('s),
        ) =>
      View.{
        inline:
