...
let rec of_hpat = (prefix: string, rev_path: Path.steps, p: UHPat.t): PP.doc =>
  ...
  | Wild(err_status) => of_Wild(prefix, err_status, rev_path)
  ...
  | NumLit(err_status, n) => of_NumLit(prefix, err_status, rev_path, n)
  | BoolLit(err_status, b) => of_BoolLit(prefix, err_status, rev_path, b)
  | Inj(err_status, side, p1) =>
    let rev_path1 = [0, ...rev_path];
    let r1 = of_hpat(prefix, rev_path1, p1);
    of_Inj(prefix, err_status, rev_path, side, r1);
  | ListNil(err_status) => of_ListNil(prefix, err_status, rev_path)
  }
and of_skel_pat =
    (
      prefix: string,
  ...
  Html5.(
    div(
      ~a=[
        ...
        a_class(node_clss(~err_status=NotInHole, ["EmptyHole", ...clss])),
      ],
      [
    ...
    div(
      ~a=[
        ...
        a_class(
          node_clss(
            ~err_status,
            ["Var", ...classes_of_var_err_status(var_err_status)] @ clss,
          ),
        ),
      ],
...
let _of_NumLit =
  ...
  Html5.(
    div(
      ~a=[
        a_id(node_id(prefix, rev_path)),
        a_class(node_clss(~err_status, ["NumLit", ...clss])),
      ],
...
let _of_BoolLit =
  ...
  Html5.(
    div(
      ~a=[
        a_id(node_id(prefix, rev_path)),
        a_class(node_clss(~err_status, ["BoolLit", ...clss])),
      ],
...
let _of_ListNil =
  ...
  Html5.(
    div(
      ~a=[
        a_id(node_id(prefix, rev_path)),
        a_class(node_clss(~err_status, ["ListNil", ...clss])),
      ],
      [delim("[]")],
    )
  );
  ...
  | EmptyLine =>
    Html5.(
      div(
        ~a=[
          a_id(node_id(prefix, rev_path)),
          a_class(node_clss(["EmptyLine", ...clss])),
        ],
        [txt("")],
      )
    )
  ...
  | LetLine(pat, ann, def) =>
    Html5.(
      div(
        ~a=[
          a_id(node_id(prefix, rev_path)),
          a_class(node_clss(["LetLine", ...clss])),
        ],
    ...
    Html5.div(
      ~a=[
        ...
        Html5.a_class(
          node_clss(
            ~err_status,
            ["Lam", ...clss] @ ann_clss @ multi_line_clss,
          ),
        ),
      ],
      [
        Html5.div(
          ~a=[Html5.a_class(["Lam-first"])],
          [
            delim(~clss=["Lam-sym"], ~delim_index=0, LangUtil.lamSym),
            of_pat(~_clss=["Lam-arg"], prefix, rev_path, arg),
          ]
          @ (
            switch (ann) {
            | None => []
          ...
          @ [delim(~clss=["Lam-dot"], ~delim_index=2, ".")],
        ),
        Html5.div(
          ~a=[Html5.a_class(["Lam-second"])],
          [
            of_block(
              ~clss=["Lam-body"],
              palette_stuff,
