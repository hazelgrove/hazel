...
and codegen_comp = (c: Anf.comp): t(expr) => {
  switch (c.comp_kind) {
...
and codegen_rules = (rules: list(Anf.rule)): t(list(rule)) => {
  codegen_fold(codegen_rule, rules);
}

and codegen_rule = (rule: Anf.rule): t(rule) => {
...
and codegen_imm = (im: Anf.imm): t(expr) => {
  switch (im.imm_kind) {
...
and codegen_inj_side = (side: Anf.inj_side): t(expr => expr) => {
  let side' =
    switch (side) {
    | CInjL => Sum.inj_l
    | CInjR => Sum.inj_r
...
let codegen = (~opts, block: Anf.block): modl => {
