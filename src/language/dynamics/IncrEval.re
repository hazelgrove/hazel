...
let reuse_check =
  ...
  /* A bare `Fun`/`TypFun` carries no environment. `Transition`'s `Closure` rule
     evaluates subterms with `~in_closure`, which suppresses
     `wrap_closure_when_done` for a function value nested under a `Closure` --
     correct in place, because the enclosing `Closure` supplies the
     environment. A cache entry is keyed by id alone and is replayed at top
     level (the `call_stack.stack == []` guard above), where that enclosing
     `Closure` is gone. Applying the replayed function then lands in
