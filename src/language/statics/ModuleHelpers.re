    ...
    | [item, ...rest] =>
      ...
      let exports =
        ...
        | ModLet(pat, _) =>
          Pat.bound_vars(pat)
          |> List.filter(keep)
          |> List.map(name => {name, pat: pat_for_bound_name(name, pat)})
        | ModuleMod(mp, _) =>
...
let labeled_tuple_exp = (exports: list(value_export)): Exp.t => {
  let fields =
    exports
    |> List.map(({name, _}) =>
         Exp.fresh(TupLabel(Exp.fresh(Label(name)), Exp.fresh(Var(name))))
       );
...
let type_exports_alias_type = (exports: list((Var.t, Typ.t))): option(Typ.t) =>
  switch (exports) {
  | [] => None
       ...
       ((ctx, acc), item: Mod.t) =>
         ...
         | ModuleMod(mp, def) =>
           let rhs_exports_ty =
             switch (def.term) {
             | Module(inner_items) =>
               collect_type_exports(ctx, inner_items) |> type_exports_alias_type
             | Var(rhs)
             | Constructor(rhs, _) =>
...
let wrap_item =
  ...
  | ModType(tpat, typ) =>
    IdTagged.fast_copy(Mod.rep_id(item), Exp.fresh(TyAlias(tpat, typ, body)))
  ...
  | EmptyHole =>
    let e: Exp.t = IdTagged.fast_copy(Mod.rep_id(item), Exp.fresh(EmptyHole));
    Exp.fresh(Let(Pat.fresh(Wild), e, body));
  | Invalid(s) =>
    let e: Exp.t = IdTagged.fast_copy(Mod.rep_id(item), Exp.fresh(Invalid(s)));
    Exp.fresh(Let(Pat.fresh(Wild), e, body));
  | MultiHole(es) =>
    let e: Exp.t =
...
let lower = (~ctx: Ctx.t, ~ana: option(Typ.t)=?, items: list(Mod.t)): lowered => {
  let ana_labels =
    switch (ana) {
