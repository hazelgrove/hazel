...
let rec any_to_info_map =
  ...
  | Mod(m_term) =>
    ...
    | ModLet(p, e) =>
      let (co_ctx_e, m) =
        any_to_info_map(~dynamics, ~ctx, ~ancestors, Exp(e), m);
      let (_, m) = any_to_info_map(~dynamics, ~ctx, ~ancestors, Pat(p), m);
      (co_ctx_e, add_mod_info(m));
    | ModType(tp, t) =>
      let (_, m) =
        any_to_info_map(~dynamics, ~ctx, ~ancestors, TPat(tp), m);
      let (_, m) = any_to_info_map(~dynamics, ~ctx, ~ancestors, Typ(t), m);
    ...
    | ModuleMod(mp, e) =>
      let (_, m) =
        any_to_info_map(~dynamics, ~ctx, ~ancestors, MPat(mp), m);
      let (co_ctx, m) =
        any_to_info_map(~dynamics, ~ctx, ~ancestors, Exp(e), m);
      (co_ctx, add_mod_info(m));
    };
  ...
  | MPat(mp_term) =>
    ...
    | Asc(inner, typ) =>
      let (_, m) =
        any_to_info_map(~dynamics, ~ctx, ~ancestors, MPat(inner), m);
      let (_, m) =
        any_to_info_map(~dynamics, ~ctx, ~ancestors, Typ(typ), m);
  ...
  | Sig(items) =>
    let m =
      List.fold_left(
        (m, item: Sig.t) => {
          let (_, m) =
            any_to_info_map(~dynamics, ~ctx, ~ancestors, Sig(item), m);
          m;
        },
        m,
        items,
