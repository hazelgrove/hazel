...
let is_opening_bracket = (t: t): bool =>
  List.exists(~f=((op, _, _)) => equal(op, t), bracket_pairs);

let label_of_opening_bracket = (t: t): option(list(t)) =>
  List.find(~f=((op, _, _)) => equal(op, t), bracket_pairs)
  |> Option.map(~f=((_, _, lbl)) => lbl);
...
let is_explicit_hole = t =>
  String.equal(t, explicit_hole) || String.equal(t, ellipsis);
...
let is_projector_invoke = (str: t): bool =>
  ...
  | Some(name) => ProjectorKind.is_name(name)
  | None => false
  };
...
/* The trigger token naming a kind. Never carries an option suffix; callers
   that want one (Triggers.refractor_to_invoke) append `_opt` themselves.
     Probe ==> "^^probe" */
let mk_projector_invoke = (kind: ProjectorKind.t): string =>
  append(projector_invoke_prefix, ProjectorKind.name(kind));
