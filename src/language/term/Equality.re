...
/*
 ...
 one function with many parameters. It is rare that a simple (==) check will suffice, often we need to
 do things like:
...
let equality =
    (
      ~type_alpha: bool,
      ~exp_alpha: bool,
      ~ignore_wrappers: bool,
      ~ignore_casts: bool,
      ~ignore_constructor_types: bool,
      ~ignore_function_names: bool,
      ~closures_by_id: bool=true,
      ~ignore_filters: bool,
      ~ignore_unknown_provenance: bool,
      ~use_expr_wildcards: option((ClosureEnvironment.t, Exp.t) => bool)=?,
      ~ignore_fixpoints: bool,
      ~free_var_handler: option((Alphas.t, string, Exp.t) => bool)=?,
      ~env1: option(ClosureEnvironment.t)=?,
      ~env2: option(ClosureEnvironment.t)=?,
      (),
    )
    : equality => {
  if (!closures_by_id) {
  ...
  let rec exp =
    ...
    | (DynamicErrorHole(x, _), _) when ignore_wrappers => exp'(x, e2)
    | (_, DynamicErrorHole(x, _)) when ignore_wrappers => exp'(e1, x)
    | (Parens(x), _) when ignore_wrappers => exp'(x, e2)
    | (_, Parens(x)) when ignore_wrappers => exp'(e1, x)
    | (Probe(x, _), _) when ignore_wrappers => exp'(x, e2)
    | (_, Probe(x, _)) when ignore_wrappers => exp'(e1, x)
    ...
    | (Fun(p1, e1, t1, f1), Fun(p2, e2, t2, f2)) =>
      ...
      | Some(alphas_exp') =>
        exp(Alphas.combine(alphas_exp', alphas_exp), alphas_typ, e1, e2)
        && (ignore_casts || Option.equal(typ', t1, t2))
  ...
  and pat =
    ...
    | (Probe(x, _), _) when ignore_wrappers => pat'(x, p2)
    | (_, Probe(x, _)) when ignore_wrappers => pat'(p1, x)
    | (Parens(x), _) when ignore_wrappers => pat'(x, p2)
    | (_, Parens(x)) when ignore_wrappers => pat'(p1, x)
  ...
  and typ =
    ...
    // Wrappers when ignored: unwrap.
    | (Parens(x), _) when ignore_wrappers => typ'(x, t2)
    | (_, Parens(x)) when ignore_wrappers => typ'(t1, x)
    ...
    | (Sum(_), _) => false
    | (Ap(t11, t12), Ap(t21, t22)) => typ'(t11, t21) && typ'(t12, t22)
    | (Ap(_, _), _) => false
