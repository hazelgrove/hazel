...
and abbreviate_tpat = (tpat: TPat.t): TPat.t => {
  ...
  let term =
    switch (tpat.term) {
    | Unknown(prov) =>
      let (unwrapped_prov, rewrap_prov) = prov |> IdTagged.unwrap;
      let prov': TPat.term =
        switch (unwrapped_prov) {
        | Hole(CycleHole)
        | Hole(EmptyHole) => Unknown(unwrapped_prov |> rewrap_prov)
        | Hole(Invalid(str)) =>
          Unknown(
            Hole(Invalid(abbreviate_str(available^, str))) |> rewrap_prov,
          )
        | Hole(MultiHole(things)) =>
          if (available^ <= 1) {
            ...
            available := available^ - 1; // space
            Unknown(
              Hole(MultiHole(List.map(abbreviate_any, things)))
              |> rewrap_prov,
            );
          }
        | TypeSubstitution(_)
