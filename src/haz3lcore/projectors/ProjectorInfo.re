...
let mk_info =
  ...
  elaborated: {
    ...
    if (P.elaborate_syntax) {
      ...
      /* virtual-grout's Segment.root_id returns option(Id.t) directly;
         dev's version returned Id.t (raising). Use the option form. */
      let inner_id = Segment.root_id(Segment.skel(seg), seg);
