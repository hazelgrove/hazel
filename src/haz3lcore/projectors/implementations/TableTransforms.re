...
/* Generic transform pipeline lives in RendererTransforms so that
 * ListRenderer (and any future list-shaped renderer) can reuse it. */
type transform =
  RendererTransforms.transform = | Rowwise(Exp.t) | Listwise(Exp.t);

let strip_parens = RendererTransforms.strip_parens;
let apply_transforms = RendererTransforms.apply_transforms;
let to_segment = RendererTransforms.to_segment;
