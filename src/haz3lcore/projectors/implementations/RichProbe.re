...
type packed_renderer = {
  ...
  render_model:
    ...
    option(Node.t),
  /* Reports the projector shape for a packed model+value pair, so
   * ProbeProj can push surrounding code down. Returns None when the
   * pack doesn't belong to this renderer (or the expression no longer
   * parses as a value). */
  placeholder_packed:
    (packed_model, Sort.t, Exp.t) => option(ProjectorCore.Shape.t),
...
let pack_renderer =
    ...
    };
  {
    ...
    placeholder_packed: (pm, sort, exp) =>
      switch (cast_model(pm), R.parse(sort, exp)) {
      | (Some(m), Some(value)) => Some(R.placeholder(value, m))
      | _ => None
      },
