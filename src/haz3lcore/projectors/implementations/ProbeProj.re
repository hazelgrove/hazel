...
let show_purps = ref(false); //TODO(andrew)
...
module M =
  ...
  let closure_view =
      (~common, ~take_focus, statics, local, (index: int, closure: closure)) =>
    div(
      ~attrs=[Attr.classes(["closure"])],
      [value_view(~common, ~take_focus, ~statics, local, closure, index)]
      ...
      List.map(
        closures =>
          Node.div(
            ...
            List.map(
              closure_view(~common, ~take_focus, statics, local),
              closures,
            ),
          ),
        groups,
  ...
  let offside_view =
    ...
    Node.div(
      ~attrs=[
        ...
        Attr.on_keydown(
          key_handler(len_of_exp, local, id, statics, dynamics, escape),
        ),
      ],
      switch (dynamics) {
      | Some(closures) =>
        ...
        @ closure_group_view(~common, ~take_focus, statics, local, groups)
        @ (is_cut_off ? extras : []);
      | None => []
      },
    );
  ...
  let view =
      (
        ~common: Common.t,
        local: action(_) => Ui_effect.t(unit),
        _parent: external_action => Ui_effect.t(unit),
        info: info,
        statics,
        dynamics,
        m: model(_),
