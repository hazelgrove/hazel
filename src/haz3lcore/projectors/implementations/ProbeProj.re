...
let value_view = (info: info, view_term, local, closure: closure, index: int) => {
  ...
  let {view, length} = view_term(Grammar.Exp(term));
...
let env_val = (closure, view_term, en: Dynamics.Probe.Env.entry): Node.t => {
  Node.div(
    ...
    [
      ...
      | Val(d) =>
        ...
        view_term(Grammar.Exp(term)).view;
      },
    ],
  );
};

let env_view = (closure: closure, view_term): Node.t =>
  Node.div(
    ~attrs=[Attr.classes(["live-env"])],
    closure.env
    |> ListUtil.dedup
    |> rm_opaques
    |> List.map(env_val(closure, view_term)),
  );

let closure_view =
    (info: info, view_term, local, (index: int, closure: closure)) =>
  div(
    ~attrs=[Attr.classes(["closure"])],
    [value_view(info, view_term, local, closure, index)]
    @ (hide_env(info) ? [] : [env_view(closure, view_term)]),
  );

let closure_group_view =
    (info, view_term, local, groups: list(list((int, closure)))) => {
  let group_views =
    List.map(
      closures =>
        Node.div(
          ~attrs=[Attr.classes(["closure-group"])],
          List.map(closure_view(info, view_term, local), closures),
        ),
      groups,
    );
  group_views == []
...
let offside_view =
    (info: info, local: action(_) => Ui_effect.t(unit), view_term) =>
  Node.div(
    ...
    | Some(closures) =>
      ...
      @ closure_group_view(info, view_term, local, groups)
      @ (is_cut_off ? extras : []);
    | _ => []
    },
  );
...
let round_up = (len_of_exp, closure: closure): unit => {
  let cur =
    len_of_exp(abbreviated_exp(ClosureLength.get(closure), closure.value));
  let goal = cur + 1;
...
let key_handler = (len_of_exp, local, info: info, _, evt) => {
  open Effect;
...
let view =
  ...
  div(
    ~attrs=[
      ...
      Attr.tabindex(0),
      Attr.on_keydown(key_handler(len_of_exp, local, info, parent)),
      Attr.classes(
        ["main"]
        @ (Option.is_some(cur_ap(info)) ? ["ap"] : [])
...
let view =
  ...
  View.{
    inline: view(local, parent, info, model, ~len_of_exp, ~view_ed),
    overlay: Some(overlay_view(info)),
    offside: Some(offside_view(info, local, view_term)),
    enter_left: None,
    enter_right: None,
  };
};
...
let methods = {
  init: (~copy_ed as _, term: Term.Any.t, ed) => (
    {
      OptUtil.Syntax.(
        switch (term) {
        | Exp(_)
        | Pat(_)
        /* Any, since grout don't have sorts rn */
        | Any(_) =>
          let+ ed = ed();
          {ed: ed};
  ...
  placeholder: (~ed_size, m: model('ed), _) =>
    ...
    ProjectorShape.inline(2 + ed_size(m.ed).row),
  update,
  mk_term: (~mk_term_ed, ~sort, ~prev, {ed}) => {
    //TODO(andrew): incoporate matt's fn above for to put probe on term?
    let (ed, t) = mk_term_ed(~sort, ed);
    ({ed: ed}, Calc.update(t, Fun.id, prev));
  },
  view,
  calculate: (~calculate_ed, ~common, {ed}) => {
