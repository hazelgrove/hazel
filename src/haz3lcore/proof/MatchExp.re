...
let rec match_exp =
  ...
  | (Undefined, _) => None
  | (Bool(b1), Bool(b2)) when b1 == b2 => Some(ctx)
  | (Bool(_), _) => None
  | (Int(i1), Int(i2)) when i1 == i2 => Some(ctx)
  | (Int(_), _) => None
  | (Float(f1), Float(f2)) when f1 == f2 => Some(ctx)
  | (Float(_), _) => None
  | (String(s1), String(s2)) when s1 == s2 => Some(ctx)
  | (String(_), _) => None
...
and match_pat = (pat_r: Pat.t, pat: Pat.t): option(alphas) =>
  ...
  | (Var(_), _) => None
  | (Int(i1), Int(i2)) when i1 == i2 => Some([])
  | (Int(_), _) => None
  | (Float(f1), Float(f2)) when f1 == f2 => Some([])
  | (Float(_), _) => None
  | (Bool(b1), Bool(b2)) when b1 == b2 => Some([])
  | (Bool(_), _) => None
  | (String(s1), String(s2)) when s1 == s2 => Some([])
  | (String(_), _) => None
  | (ListLit(xs), ListLit(ys)) when List.length(xs) == List.length(ys) =>
    ListUtil.fold_left_opt(
      (alphas, (x, y)) =>
