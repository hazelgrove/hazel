...
let rec subst =
  ...
  switch (term) {
  | Var(y) when y == x =>
    /* the FIRST copy carries the def's subtree ids (it travels —
       P7); later copies are minted fresh HERE, explicitly, rather
       than leaving duplicates for dedupe_ids to heal silently
       (tests assert the healer stays quiet) */
    let d = first^ ? def : strip_comments(refresh_ids(def));
    first := false;
    inserted(~parens=parens_for(e), d, e);
  | Let(p, d, body) =>
    let recursive =
...
let feed_plan =
  ...
  | Some((l, x, occ_pref)) =>
    ...
    | Let(_, def, body) =>
      ...
      | [_] when !free_in(x, def) => Some(Consume)
      | _ =>
        let occ =
          switch (occ_pref) {
          | Some(oid) =>
...
let feed_let_impl: impl = {
  ...
  prepare: (~info_map, ~target, program) =>
    ...
    | Some(Feed(l, def, occ)) =>
      ...
      /* STATIC parens (precedence tables — no printing, no reparse;
         the reparse identity stays a TEST oracle) */
      let parens =
        needs_parens(def)
