...
let move_into_if_stringlit = (char, z, id_gen) =>
  ...
  Form.is_string_delim(char)
    ? switch (move(Left, z, id_gen)) {
      | None => (z, id_gen)
      | Some((z, id_gen)) => (z |> set_caret(Inner(0, 0)), id_gen)
      }
    : (z, id_gen);
...
let go =
  ...
  | (Outer, (_, Some(_))) =>
    ...
    |> opt_regrold(Left)
    |> Option.map(((z, id_gen)) => move_into_if_stringlit(char, z, id_gen));
  | (Outer, (_, None)) =>
