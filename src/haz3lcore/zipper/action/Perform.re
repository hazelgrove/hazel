...
let go_z =
  ...
  | Move(d) =>
    Animation.request(
      ...
      @ List.map(Animation.Actions.move_scale, Animation.indicated_ids),
    );
  ...
  | Destruct(d) =>
    Animation.request(
      ...
      @ List.map(
          ...
          Animation.projector_ids(projectors)
          @ Animation.comment_ids(comments)
          @ Animation.token_ids(tiles),
        ),
    );
    z
    |> Destruct.go(d)
