...
let replace_markers =
    (~implicit_hole: string, ~shape=Grout.Convex, seg: Segment.t): Segment.t =>
  List.map(
...
let strip_implicit_holes =
    (~implicit_hole: string, ~shape=Grout.Convex, z: Zipper.t): Zipper.t =>
  Zipper.zip(z)
  |> replace_markers(~implicit_hole, ~shape)
  |> Zipper.unzip
...
let of_text =
  ...
  | None => None
  | Some(z) =>
    let z = strip_implicit_holes(~implicit_hole, z);
    /* the concave marker stands for an OPERATOR hole, so it comes back
       as concave grout rather than the convex default */
    Some(
