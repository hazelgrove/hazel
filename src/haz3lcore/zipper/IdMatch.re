 ...
 *
 * Id assignment is a performance concern only: IncrEval.reuse_check re-checks
 * `Exp.fast_equal(entry.prev_elab, info.elab_term)` before using a cache entry,
 * so a wrong match costs a wasted lookup, not a wrong answer. Duplicate ids are
 * the exception --- those break an invariant the rest of the editor (Measured,
 * Info map, selection) depends on --- hence the explicit check. */
