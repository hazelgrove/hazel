...
let forms: list((string, t)) = [
  ...
  ("lt", mk_infix("<", Exp, P.eqs)), //TODO: precedence
  ("gt", mk_infix(">", Exp, P.eqs)), //TODO: precedence
  ...
  ("ap_exp", mk(ii, ["(", ")"], mk_post(P.ap, Exp, [mk_child(Exp)]))),
  ("ap_pat", mk(ii, ["(", ")"], mk_post(P.ap, Pat, [mk_child(Pat)]))),
  (
    "let_",
    mk(
      ds,
      ...
      ["|", "=>"],
      {
        ...
        nibs:
          Nib.(
            {
              sort: Exp,
              shape: Shape.concave(~pad=true, ~prec=P.rule_sep, ()),
            },
            {
              sort: Exp,
              shape: Shape.concave(~pad=true, ~prec=P.rule_sep, ()),
  ...
  ("list_lit_pat", mk(ii, ["[", "]"], mk_op(Pat, [mk_child(Pat)]))),
  ("list_typ", mk(ii, ["[", "]"], mk_op(Typ, [mk_child(Typ)]))),
