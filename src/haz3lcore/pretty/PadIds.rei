/* How many ids a term's rendering consumes, and giving a term that many.

   ExpToSegment mints an id wherever a rendered piece needs one the term does
...
/* How many ids this type's rendering consumes. ExpToSegment.typ_to_pretty
...
/* Give every node the ids its rendering consumes, so the renderer uses these
