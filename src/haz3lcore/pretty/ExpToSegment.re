...
let rec exp_to_pretty = (~settings: Settings.t, exp: Exp.t): pretty => {
  ...
  | ListLit([x, ...xs]) =>
    ...
    let (id, ids) = (
      ...
      IdTagged.ids(exp) |> List.tl_exn |> PadIds.pad_ids(List.length(xs)),
    );
    let form = (x, xs) =>
      mk_form(
        ListLitExp,
  ...
  | DeferredAp(e, es) =>
    ...
    let (id, ids) = (
      IdTagged.ids(exp) |> List.hd_exn,
      IdTagged.ids(exp) |> List.tl_exn |> PadIds.pad_ids(List.length(es)),
    );
    wrap(
      exp,
      e
...
and pat_to_pretty = (~settings: Settings.t, pat: Pat.t): pretty => {
  ...
  | ListLit([x, ...xs]) =>
    ...
    let (id, ids) = (
      ...
      IdTagged.ids(pat) |> List.tl_exn |> PadIds.pad_ids(List.length(xs)),
    );
    wrap(
      pat,
      p_just([
