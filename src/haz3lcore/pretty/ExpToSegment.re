...
let pad_ids =
  ...
  let truncated =
    if (List.length(ids) < n) {
      ids @ List.init(n - List.length(ids), ~f=_ => Id.mk());
    } else {
...
let rec drv_exp_to_pretty =
  ...
  | Ctx([x, ...xs]) =>
    ...
    let ids =
      syntax |> IdTagged.ids |> List.tl_exn |> pad_ids(List.length(xs));
...
let rec exp_to_pretty = (~settings: Settings.t, exp: Exp.t): pretty => {
  ...
  | ListLit([x, ...xs]) =>
    ...
    let (id, ids) = (
      ...
      IdTagged.ids(exp) |> List.tl_exn |> pad_ids(List.length(xs)),
    );
    let form = (x, xs) =>
      mk_form(
        ListLitExp,
  ...
  | Parens({term: Fun(p, e, _, _), _} as inner_exp) =>
    ...
    let name =
      if (settings.hide_fixpoints && String.is_suffix(name, ~suffix="+")) {
  ...
  | DeferredAp(e, es) =>
    ...
    let (id, ids) = (
      IdTagged.ids(exp) |> List.hd_exn,
      IdTagged.ids(exp) |> List.tl_exn |> pad_ids(List.length(es)),
    );
    wrap(
      exp,
      e
      @ [
        mk_form(
          ...
          [
            ...
            @ List.concat(
                List.map2_exn(
                  ids |> List.tl_exn, es |> List.tl_exn, ~f=(id, e) =>
                  [mk_form(CommaExp, id, [])] @ e
                ),
              ),
          ],
        ),
  ...
  | Module(items) =>
    ...
    let ids =
      IdTagged.ids(exp) |> List.tl_exn |> pad_ids(List.length(items) - 1);
    let body =
      switch (items_pretty) {
      | [] => []
      | [first, ...rest] =>
...
and pat_to_pretty = (~settings: Settings.t, pat: Pat.t): pretty => {
  ...
  | ListLit([x, ...xs]) =>
    ...
    let (id, ids) = (
      ...
      IdTagged.ids(pat) |> List.tl_exn |> pad_ids(List.length(xs)),
    );
    wrap(
      pat,
      p_just([
...
and typ_to_pretty = (~settings: Settings.t, typ: Typ.t): pretty => {
  ...
  | Prod([t, ...ts]) =>
    ...
    wrap(
      ...
      @ List.concat(
          List.map2_exn(
            IdTagged.ids(typ) |> pad_ids(ts |> List.length), ts, ~f=(id, t) =>
  ...
  | Sig(items) =>
    ...
    /* Join items with semicolons and wrap in braces */
    let ids =
      IdTagged.ids(typ) |> List.tl_exn |> pad_ids(List.length(items) - 1);
    let body =
      switch (items_pretty) {
      | [] => []
      | [first, ...rest] =>
...
let uniquify_repeated_tiles = (seg: Segment.t): Segment.t => {
  let seen: Stdlib.Hashtbl.t((Id.t, int), unit) = Stdlib.Hashtbl.create(64);
  let rec go_seg = (seg: Segment.t): Segment.t => List.map(~f=go_piece, seg)
  and go_piece = (p: Piece.t): Piece.t =>
    switch (p) {
    | Tile(t) =>
      let dup =
        List.exists(~f=i => Stdlib.Hashtbl.mem(seen, (t.id, i)), t.shards);
      let id = dup ? Id.mk() : t.id;
      List.iter(
        ~f=i => Stdlib.Hashtbl.replace(seen, (id, i), ()),
        t.shards,
      );
      Tile({
        ...t,
