...
and typ_to_pretty = (~settings: Settings.t, typ: Typ.t): pretty => {
  ...
  | Forall(p, t) =>
    ...
    let+ p = pat_to_pretty(~settings: Settings.t, p)
    and+ t = go(t);
    [mk_form("forall-term", id, [p])] @ t;
  | Type(tp, t) =>
    let id = typ |> Typ.rep_id;
    let+ tv = tpat_to_pretty(~settings: Settings.t, tp)
    and+ t = go(t);
    [mk_form("forall-type", id, [tv])] @ t;
  | Equals(e1, e2) =>
    let id = typ |> Typ.rep_id;
    let+ e1 = exp_to_pretty(~settings: Settings.t, e1)
    and+ e2 = exp_to_pretty(~settings: Settings.t, e2);
...
and parenthesize_typ = (~show_filters: bool, typ: Typ.t): Typ.t => {
  ...
  | Forall(p, t) =>
    ...
    |> rewrap
  | Type(tv, t) =>
    Type(tv, parenthesize_typ(t) |> paren_typ_at(Precedence.type_binder))
    |> rewrap
  | Equals(e1, e2) =>
    Equals(
      parenthesize(~show_filters, e1) |> paren_at(Precedence.min),
      parenthesize(~show_filters, e2) |> paren_at(Precedence.min),
    )
    |> rewrap
  | Arrow(t1, t2) =>
    Arrow(
