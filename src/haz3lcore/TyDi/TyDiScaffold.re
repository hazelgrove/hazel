 ...
 * Skips Grout (holes), Whitespace, and Tile pieces other than commas.
 * e.g. [,, " ", ?]  → ","
 *      [,, " ", y=, ?] → ",y="  */
let scaffold_insertable = (content: Segment.t): Token.t =>
  String.concat(
    "",
    List.filter_map(
      (p: Piece.t) =>
...
let scaffold_segment_to_string = TyDi.buffer_to_string;
 ...
 * scaffold's structural text (commas and labels). Called before
 * dumping for statics so that statics sees the tuple structure. */
let reify_scaffold = (z: Zipper.t): Zipper.t =>
  if (is_scaffold_buffer(z)) {
    let insertable = scaffold_insertable(z.selection.content);
