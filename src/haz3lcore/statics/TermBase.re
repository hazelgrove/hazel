...
and Exp: {
  ...
  let rec term_of_menhir_ast = (exp: Haz3lmenhir.AST.exp): Exp.term => {
    ...
    | Filter(a, cond, body) =>
      ...
      Filter(StepperFilterKind.Filter({pat: dcond, act}), dbody);
...
and Pat: {
  ...
  let term_of_menhir_ast: Haz3lmenhir.AST.pat => term;
  let of_menhir_ast: Haz3lmenhir.AST.pat => t;
} = {
  [@deriving (show({with_path: false}), sexp, yojson)]
  type term =
    | Invalid(string)
  ...
  let rec term_of_menhir_ast = (pat: Haz3lmenhir.AST.pat): term => {
    switch (pat) {
  ...
  and of_menhir_ast = (pat: Haz3lmenhir.AST.pat): t => {
    IdTagged.fresh(term_of_menhir_ast(pat));
  };
...
and Typ: {
  ...
  let rec of_menhir_ast = (typ: Haz3lmenhir.AST.typ): t => {
    IdTagged.fresh(term_of_menhir_ast(typ));
  }
  and term_of_menhir_ast = (typ: Haz3lmenhir.AST.typ): term => {
    switch (typ) {
...
and TPat: {
  ...
  let fast_equal: (t, t) => bool;
  let of_menhir_ast: Haz3lmenhir.AST.tpat => t;
  let term_of_menhir_ast: Haz3lmenhir.AST.tpat => term;
} = {
  [@deriving (show({with_path: false}), sexp, yojson)]
  type term =
    | Invalid(string)
  ...
  let rec term_of_menhir_ast = (tpat: Haz3lmenhir.AST.tpat) => {
    switch (tpat) {
  ...
  and of_menhir_ast = (tpat: Haz3lmenhir.AST.tpat) => {
    IdTagged.fresh(term_of_menhir_ast(tpat));
  };
