...
module rec Typ: {
  /* TYPE_PROVENANCE: From whence does an unknown type originate?
     Is it generated from an unannotated pattern variable (TrueSyn),
  ...
  type type_provenance =
    ...
    | TypeHole
    | EmptyJoin
    | Join(type_provenance, type_provenance)
    | MatchedArrow(type_provenance)
    | MatchedProd(type_provenance)
    | MatchedList(type_provenance)
    | Internal;
  ...
  /* MODE: The (analytic) type information derived from a term's
     ...
     Ana(Unknown(TrueSyn)), and that this type is thus vestigial. */
  [@deriving (show({with_path: false}), sexp, yojson)]
  type mode =
    //| SynFun
    //| Syn
