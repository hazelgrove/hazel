...
let rec any_to_info_map =
  ...
  | Exp(e) =>
    ...
    (co, map, constraints);
  | Pat(p) =>
    let (_, _, map, constraints) = upat_to_info_map(~ctx, p);
...
and uexp_to_info_map =
  ...
  | Cons(e1, e2) =>
    let (mode_ele, match_constraints) =
      Typ.matched_list_mode(mode, Term.UExp.rep_id(uexp));
    let (ty1, free1, m1, constraints1) = go(~mode=mode_ele, e1);
  ...
  | ListLit(es) =>
    ...
    let constraints1 = List.map(((_, _, _, c)) => c, infos) |> List.flatten;
    let self: Typ.self =
    ...
    let free = Ctx.union(List.map(((_, f, _, _)) => f, infos));
    let m = union_m(List.map(((_, _, m, _)) => m, infos));
    add(~self, ~free, m, match_constraints @ constraints1);
  | Test(test) =>
  ...
  | Let(pat, def, body) =>
    ...
    let (_, ctx_pat_ana, m_pat, constraints3) =
      upat_to_info_map(~mode=Ana(ty_def), pat);
...
and upat_to_info_map =
  ...
  let anonymous = Typ.Just(Unknown(Anonymous));
  switch (term) {
  | Invalid(msg) =>
  ...
  | Cons(hd, tl) =>
    ...
    let (_, ctx, m_tl, constraints2) =
      upat_to_info_map(~ctx, ~mode=Ana(List(ty)), tl);
    add(
      ~self=Just(List(ty)),
      ~ctx,
      union_m([m_hd, m_tl]),
