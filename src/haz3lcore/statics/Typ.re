...
/*
 ...
 let rec eq = (t1: t, t2: t): bool => {
   ...
   | (List(_), _) => false
   | (Sum(sm1), Sum(sm2)) => ConstructorMap.equal(Option.equal(eq), sm1, sm2)
   | (Sum(_), _) => false
   | (Var(n1), Var(n2)) => n1 == n2
   | (Var(_), _) => false
   };
 ...
 let rec join = (~resolve=false, ctx: Ctx.t, ty1: t, ty2: t): option(t) => {
   ...
   | (Sum(sm1), Sum(sm2)) =>
     let (sorted1, sorted2) =
       ...
       ConstructorMap.same_tags_same_order(sm1, sm2)
         ? (sm1, sm2) : (ConstructorMap.sort(sm1), ConstructorMap.sort(sm2));
 ...
 let sum_entry = (tag: Constructor.t, tags: sum_map): option(sum_entry) =>
   List.find_map(
     fun
     | (t, typ) when Constructor.equal(t, tag) => Some((t, typ))
     | _ => None,
     tags,
   );
