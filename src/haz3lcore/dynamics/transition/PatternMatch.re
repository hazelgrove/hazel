...
let rec matches = (~indet=0, capture, dp: Pat.t, d: DHExp.t): match_result => {
  print_endline("");
  print_endline(String.make(indet, ' ') ++ "PatternMatch matches:");
  print_endline(String.make(indet, ' ') ++ "  Pattern: " ++ DHPat.show(dp));
  print_endline(
    String.make(indet, ' ') ++ "  Expression: " ++ DHExp.show(d),
  );
  let matches = matches(~indet=indet + 2, capture);
  ...
  let ret: match_result =
    ...
    | TupLabel(_, x) =>
      ...
      print_endline("x' ++ " ++ DHExp.show(x'));
      print_endline("x ++ " ++ DHPat.show(x));
      matches(x, x');
    | Tuple(ps) =>
    ...
    | Cast(p, t1, t2) =>
      ...
      print_endline(
        String.make(indet, ' ') ++ "  exp: " ++ DHExp.show(exp),
      );
      print_endline(
        String.make(indet, ' ')
        ++ "  Transitioned: "
        ++ DHExp.show(transitioned),
      );
      matches(p, transitioned);
    };
  print_endline(
    String.make(indet, ' ') ++ "  Result: " ++ show_match_result(ret),
  );
  print_endline("");
