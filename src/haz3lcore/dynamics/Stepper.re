...
module Step = {
  ...
  type t = EvaluatorStep.EvalObj.t;
...
module Stepper = {
  [@deriving (show({with_path: false}), sexp, yojson)]
  type t = Aba.t(Exp.t, Step.t);

  [@deriving (show({with_path: false}), sexp, yojson)]
  type persistent = list(Step.t);
};
