...
//     switch (r) {
//     | BoxedValue(d) =>
//       BoxedValue((f(d), Some((t, EvaluatorResult.BoxedValue(d))))) |> return
//     | Indet(d) => Indet((f(d), Some((t, EvaluatorResult.Indet(d))))) |> return
...
//     let* r = require(d);
//     switch (r) {
//     | BoxedValue(d) => Indet((f(d), Some((t, EvaluatorResult.Indet(d))))) |> return
//     | Indet(d) => Indet((f(d), Some((t, EvaluatorResult.Indet(d))))) |> return
//     | Expr(d) => Expr((f(d), None)) |> return
//     };
...
//   };
//   let combine = (f: EvaluatorMonad.t(CombineIndetValue.t('f, 't)), d) => {
//     let* f = f;
//     switch (f) {
//     | BoxedValue(f, Some(t)) =>
//       let* r = require(d);
...
//       switch (r) {
//       | BoxedValue(d) =>
//         `BoxedValue((f(d), `Some((t, BoxedValue.BoxedValue(d))))) |> return
//       | Indet(d) => `Indet((f(d), `None)) |> return
//       | Expr(d) => `Expr((f(d), `None)) |> return
//       };
//     | `Indet(f, `Some(t)) =>
...
//       switch (r) {
//       | BoxedValue(d) =>
//         `Indet((f(d), `Some((t, BoxedValue.BoxedValue(d))))) |> return
//       | Indet(d) => `Indet((f(d), `None)) |> return
//       | Expr(d) => `Expr((f(d), `None)) |> return
//       };
//     | `Indet(f, `None) =>
...
//     and+ r1 = d1
//     and+ r2 = d2;
//     let return = (r: StepperResult.t(DHExp.t)) => {
//       return(r);
//     };
//     switch (r1) {
//     | BoxedValue(TestLit(id)) =>
//       let* r = Evaluator.evaluate_test(env, id, d2);
//       switch (r) {
...
//       | None =>
//         switch (r2) {
//         | BoolLit(b2) => builtin(Evaluator.eval_bin_bool_op(op, b1, b2))
//         | d2 =>
...
//     | FloatLit(f1) =>
//       switch (d2) {
//       | FloatLit(f2) => builtin(Evaluator.eval_bin_float_op(op, f1, f2))
//       | d2 =>
...
//     | StringLit(s1) =>
//       switch (d2) {
//       | StringLit(s2) => builtin(Evaluator.eval_bin_string_op(op, s1, s2))
//       | d2 =>
