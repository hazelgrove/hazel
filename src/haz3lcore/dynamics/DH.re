...
module rec DHExp: {
  ...
  let rec ty_subst = (s: Typ.t, x: TypVar.t, exp: DHExp.t): t => {
    ...
    | Derive(t1, t2, t3) => Derive(re(t1), re(t2), re(t3))
    ...
    | BinPropOp(op, t1, t2) => BinPropOp(op, re(t1), re(t2))
    | Cons(t1, t2) => Cons(re(t1), re(t2))
    | ListConcat(t1, t2) => ListConcat(re(t1), re(t2))
    | Entail(t1, t2) => Entail(re(t1), re(t2))
    ...
    | InvalidDerivation(t, err) => InvalidDerivation(re(t), err)
