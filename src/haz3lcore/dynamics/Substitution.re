...
let rec subst_var =
        (m, d1: DHExp.t(IdTag.t), x: Var.t, d2: DHExp.t(IdTag.t))
        : DHExp.t(IdTag.t) => {
...
and subst_var_env =
    (m, d1: DHExp.t(IdTag.t), x: Var.t, env: ClosureEnvironment.t)
...
and subst_var_filter =
    (
      ...
      d1: DHExp.t(IdTag.t),
      x: Var.t,
      flt: TermBase.StepperFilterKind.t(IdTag.t),
    )
    : TermBase.StepperFilterKind.t(IdTag.t) => {
  flt |> TermBase.StepperFilterKind.map(subst_var(m, d1, x));
};

let subst = (m, env: Environment.t, d: DHExp.t(IdTag.t)): DHExp.t(IdTag.t) =>
  env
  |> Environment.foldo(
       (xd: (Var.t, DHExp.t(IdTag.t)), d2) => {
         let (x, d1) = xd;
         subst_var(m, d1, x, d2);
       },
       d,
