...
module type EV_MODE = {
  ...
  let req_all_final:
    (DHExp.t => result, int, list(DHExp.t)) => requirement(list(DHExp.t));
  let req_all_final_tuple:
    (DHExp.t => result, int, list(option(LabeledTuple.t)), list(DHExp.t)) =>
    requirement(list((option(LabeledTuple.t), DHExp.t)));
...
module Transition = (EV: EV_MODE) => {
  ...
  let transition = (req, state, env, d): 'a =>
    ...
    | TupLabel(_, _) =>
      ...
      Constructor;
    | Tuple(ds) =>
      let (dsp, dst) = List.split(ds);
      let. _ = otherwise(ds => Tuple(ds))
      and. _ = req_all_final_tuple(req(state, env), 0, dsp, dst);
      Constructor;
    | Prj(d1, n) =>
      ...
      Step({
        apply: () =>
          ...
          | Tuple(ds) =>
            let (_, x) = List.nth(ds, n);
            x;
          | Cast(_, Prod(ts), Prod(_)) when n < 0 || List.length(ts) <= n =>
            raise(EvaluatorError.Exception(InvalidProjection(n)))
          | Cast(d2, Prod(ts1), Prod(ts2)) =>
            let (_, ts1x) = List.nth(ts1, n);
            let (_, ts2x) = List.nth(ts2, n);
            Cast(Prj(d2, n), ts1x, ts2x);
