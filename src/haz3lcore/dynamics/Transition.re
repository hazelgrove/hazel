...
module Transition = (EV: EV_MODE) => {
  ...
  let transition = (req, mod_req, state, env, d): 'a =>
    switch (d) {
    | BoundVar(x) =>
    ...
    | Module(dp, d1, d2) =>
      let env_in = ClosureEnvironment.empty;
      let. _ = otherwise(d1 => Module(dp, d1, d2))
      and. d1' = req_final(mod_req(state, env, env_in), 0, d1);
      let.match env' = (env, matches(dp, d1'));
      Step({apply: () => Closure(env', d2), kind: ModuleBind, value: false});
    | Dot(d1, d2) =>
      let. _ = otherwise(d1 => Dot(d1, d2))
