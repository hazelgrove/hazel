...
let rec dhexp_of_uexp =
  ...
  | Some(InfoExp({mode, self, ctx, _})) =>
    ...
    let+ d: DHExp.t =
      ...
      | UnOp(Bool(Not), e) =>
        let+ d_scrut = dhexp_of_uexp(m, e, livelit_state);
        let d_rules =
          DHExp.[
      ...
      | TyAlias(_, _, e) => dhexp_of_uexp(m, e, livelit_state)
      };
...
let uexp_elab =
    (m: Statics.Map.t, uexp: Term.UExp.t, livelit_state: Id.Map.t(DHExp.t))
    : ElaborationResult.t =>
