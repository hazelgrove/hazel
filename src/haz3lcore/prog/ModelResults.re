...
// include M;
// [@deriving (show({with_path: false}), sexp, yojson)]
// type t = M.t(ModelResult.Model.t);
// let init_eval = (ds: list((Key.t, Elaborator.Elaboration.t))): t =>
//   ds |> List.to_seq |> of_seq |> map(ModelResult.Model.init_eval);
...
//         Some(
//           v
//           |> Option.value(~default=ModelResult.Model.NoElab)
//           |> ModelResult.Update.update_elab(~settings, elab),
//         ),
//       acc,
//     )
//   );
...
//   M.map(ModelResult.Update.run_pending(~settings));
// let timeout_all = map(ModelResult.Update.timeout);
// let advance_evaluator_result =
//     (results: t, (key: Key.t, elab: Elaborator.Elaboration.t))
//     : option((Key.t, ModelResult.Model.t)) =>
...
//     ((key: Key.t, r: ModelResult.Model.t))
//     : option((Key.t, ModelResult.Model.t)) =>
//   switch (r) {
...
//   |> List.to_seq
//   |> of_seq;
// let put_result = (key: Key.t, result: ModelResult.Model.t, results: t): t =>
