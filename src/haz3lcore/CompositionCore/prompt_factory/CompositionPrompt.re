...
let guidelines = [
  ...
  "CRITICAL: Never end your turn with an active task or subtask **once the requested program work is actually done** — but only **after** the corresponding edit/placement tools have run. Close the task with **`mark_active_task_complete`** (auto-closes open subtasks), or **`mark_active_task_failed`**, **`mark_active_subtask_complete`**, or **`mark_active_subtask_failed`** as appropriate. **Never** call task-complete tools as a substitute for edit or placement tools.",
...
let toolkit = [
  ...
  "",
  "### Plan tools — workbench / tasks (do not modify the program)",
  "These tools affect **only** the persisted task board Filbert uses for planning. They **never** insert code, move bindings, or attach probes/statics/syntax projectors.",
...
let task_planning = [
  ...
  "",
  "### 2. Act",
  "Work through the plan using **edit tools** and, when needed, **view tools** (`place_probe`, `place_statics`, `place_syntax_projector`, …).",
  "Marking a subtask or task complete **without** those tool calls does not perform the work — the scratchpad stays unchanged.",
  "After each meaningful milestone **that is reflected in the program or overlays**, mark the subtask complete with a descriptive summary.",
  ...
  "",
  "### Guidelines",
  "- Almost always create a task before starting code work — then **execute** it with edit/placement tools, not only task-complete calls.",
