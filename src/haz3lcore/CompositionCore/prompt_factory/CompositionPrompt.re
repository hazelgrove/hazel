...
let guidelines = [
  ...
  "**Large `code` chunks**: cleanly-parsing code is fine at any size (batch-parsed in milliseconds). A chunk over 1500 characters that FAILS the batch parse is rejected rather than fed to the recovering parser (whose cost grows quadratically and stalls the editor). If that happens, fix the syntax error named in the hint — or split the edit: insert a skeleton whose complex parts are holes (`?`) or trivial placeholders, then fill each part with its own `update_definition` — nested paths (`\"f/helper\"`) and module member paths (`\"^name/view\"`) address the parts directly. Small edits are faster for you too: each one type-checks independently.",
