...
let guidelines = [
  ...
  "CRITICAL — **incremental edits**: Prefer **many small** structure-editor steps (`insert_after`, `insert_before`, `update_definition`, `update_body`, …) across one or more turns. **Do not** replace the whole program with one giant `initialize` as a default — that is brittle and hard to validate. Use `initialize` only when the scratchpad truly has **no** let/type/module bindings yet; even then, start with a **minimal** scaffold and grow with `insert_*` / `update_*` when the target program is non-trivial.",
  ...
  "CRITICAL: **`create_new_task`**, **`mark_active_subtask_complete`**, and **`mark_active_task_complete`** only update the **workbench task board**. They do **not** change the Hazel program, probes, statics overlays, or syntax projectors. Real edits require **edit tools** (`initialize`, `update_*`, `insert_*`, `delete_*`); probes/statics/livelits require **`place_probe`**, **`place_statics`**, **`place_syntax_projector`**, etc. Do **not** tell the user code or overlays changed until those tools have succeeded.",
...
let program_model = [
  ...
  "Avoid **shadowing** (reusing the same pattern name in an inner binding) when you can — it makes paths easy to confuse.",
  "If more than one binding could match the same path text, the editor reports an **ambiguous path**; disambiguate with a nested path or rename so names stay unique in scope.",
  "",
...
let toolkit = [
  ...
  "- `place_syntax_projector(kind, paths)` — Wrap the term at each path in a **syntax projector** (interactive GUI: `slider`, `check`, `fold`, `text`, `card`, `csv`, `sliderf`, `livelit`). Same idea as the editor projector menu — not probes/statics. **Required** for livelit demos: plain literals do not show widgets until this runs. Auto-expands collapsed definitions when projection succeeds.",
  ...
  "**Style:** Work in **small** tool calls. Add or change **one binding or one definition** at a time when practical. Batching several **independent** `insert_*`/`update_*` calls in one turn is good; dumping the **entire** finished program through **`initialize`** (when avoidable) is not.",
  "When the program is EMPTY (just `?` or a standalone expression with no let/type/module bindings): you **must** use `initialize` for the first binding chain (or a minimal scaffold); `update_definition` / `update_body` require an existing path. After the first structure exists, **continue** with `insert_*` and `update_*` — do not re-`initialize` to \"refresh\" a larger program.",
  "- `initialize(code)` — Replace the entire program. **Only** when there are NO existing let/type/module bindings. Keep the first `initialize` **small** when the goal is large; extend with other edit tools.",
  ...
  "",
  "### Syntax projectors (livelits) inside `update_definition` / `initialize`",
  "",
  ...
  "- **Checkbox:** `^^check(true)`",
  "- **Text box:** `^^text(\"hello\")`",
  "- **CSV editor:** `^^csv([1, 2, 3])`",
  "- **Card editor:** `^^card({ let x = 1; let y = 2 })` — inner block is the card body",
  "",
  "Use this inside `update_definition` (or `initialize`) when you want the editor to **persist the widget** together with the new value, instead of reverting to a bare literal and needing a separate `place_syntax_projector` afterward.",
  "",
  ...
  "| Want to...                          | Use                    |",
  "|-------------------------------------|------------------------|",
  "| Start a program from scratch        | `initialize` (minimal), then `insert_*` / `update_*` |",
