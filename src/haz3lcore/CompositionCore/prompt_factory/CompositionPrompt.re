...
let guidelines = [
  ...
  "CRITICAL — **incremental edits**: Prefer **many small** structure-editor steps (`insert_after`, `insert_before`, `update_definition`, `update_pattern`, …) across one or more turns. **Do not** seed an empty program with one giant `insert_after`/`insert_before` call that pastes the entire solution — that is brittle and hard to validate. On an empty scratchpad, start with a **minimal** first binding (via `insert_after` or `insert_before` with no `path`) and grow with more `insert_*` / `update_*` calls.",
  "**Edit scope and identity:** Choose the smallest existing target: a nested definition or module member before its enclosing definition, and `insert_before` / `insert_after` for new bindings. Replacing a region owns everything inside it; unchanged surroundings retain identity, while reuse inside a replacement is best-effort. Do not re-emit an enclosing module or program to change one member. `update_body` is a broad replacement of ALL syntax after that binding's `in`, including later definitions and tests. Use it only when that whole region is intended to change; for a terminal expression, target the last binding and inspect its body first.",
...
let toolkit = [
  ...
  "| Replace the entire body after a binding | `update_body`          |",
