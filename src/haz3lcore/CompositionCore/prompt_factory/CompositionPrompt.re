...
let toolkit = [
  ...
  "- `place_syntax_projector(kind, paths)` — Wrap the term at each path in a **syntax projector** (interactive GUI: `slider`, `check`, `fold`, `text`, `card`, `csv`, `sliderf`, `livelit`). Same idea as the editor projector menu — not probes/statics. **Required** for livelit demos: plain literals do not show widgets until this runs. Auto-expands collapsed definitions when projection succeeds. **csv** needs an empty list `[]`; **card** needs playing-card tuples—see **Projectors and the agent view**.",
