let self = [
  ...
  "- New types are defined in the form 'type <name> = ... in <body>'",
  "- List types are constructed in the form '[<type>]'.",
  "- Sum types are constructed in the form '+ <constructor1>(<params1>) + <constructor2>(<params2>) + ...' where '(<params>)' is an optional list of type parameters for the constructor. For example, an optional integer type would be constructed as '+ None + Some(Int)'.",
  "- Pattern matching is written as 'case <exp> | <pat1> => <exp1> | <pat2> => <exp2> | ... end' - note that there is NO 'of' keyword. Each '| <pat> => <exp>' is a rule and there may be zero or more rules - note the '=>' arrow. Cases MUST END in an 'end' keyword. DO NOT USE any other keyword besides 'case' to do pattern matching.  DO NOT USE a 'with' or 'of' keyword with 'case', just start the list of rules after the first expression.",
  ...
  "- Tests are written in the form 'test <bool exp> end' where <bool exp> is an expression evaluating to true or false.  Tests MUST END in an 'end' keyword. Multiple tests may be sequenced together using a semicolon: 'test <bool exp> end; test <bool exp> end; ...'",
