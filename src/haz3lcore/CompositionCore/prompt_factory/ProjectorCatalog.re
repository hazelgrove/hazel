...
let livelit_line = (k: ProjectorKind.t): option(string) =>
  ...
  | Chart =>
    Some(
      "- **chart** — D3-rendered **read-only** visualization (bar, grouped bar, pie, line, scatter) of a `Chart` ADT value. Reuses the table recognizer, so the payload is a **list of labeled tuples**; numeric cells must be **Float** literals (no implicit `Int` coercion — convert explicitly).",
    )
  | Fold
  | Probe
  | Statics => None
...
let blurb_for_composition_prompt: list(string) = {
  ...
  @ [
    ...
    "`kind` must be a menu projector name (`fold`, `slider`, `sliderf`, `check`, `text`, `card`, `csv`, `table`, `chart`, `livelit`) — not `probe` or `statics` (those use the probe/statics tools).",
    "",
...
let blurb_for_compaction: list(string) = [
  ...
  "**Livelits** are alternate UIs for literals (checkbox, slider/sliderf, text, card, csv, table, chart, livelit, plus fold in the menu).",
