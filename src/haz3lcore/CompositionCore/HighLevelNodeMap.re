...
/* Resolve a parsed path to a node ID by walking segments. Handles the
   structured segment forms (`#n` positional index, `$` final binding)
   that name-path matching can't express. */
...
let path_to_id_opt = (node_map: t, path: string): option(Id.t) => {
  ...
  | ([], _) =>
    /* No name-path match: try structured resolution (#n index, $ final) */
    resolve_path(node_map, parse_path(path))
...
let path_to_id = (node_map: t, path: string): Id.t => {
  ...
  let matches = matches_for_path(node_map, split_path(base));
  switch (matches, occurrence) {
  | ([], _) when resolve_path(node_map, parse_path(path)) != None =>
    /* Structured form (#n index, $ final binding) */
    Option.get(resolve_path(node_map, parse_path(path)))
  | ([], _) =>
