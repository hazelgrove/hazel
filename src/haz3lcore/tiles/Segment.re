...
let rec remold = (~shape=Nib.Shape.concave(), seg: t, s: Sort.t) =>
  ...
  | ModuleSignatureEntry => remold_module_signature_entry(shape, seg)
  }
...
and remold_module_signature_entry = (shape, seg: t): t =>
  switch (seg) {
  | [] => []
  | [hd, ...tl] =>
    switch (hd) {
    | Secondary(_)
    | Grout(_) => [hd, ...remold_module_signature_entry(shape, tl)]
    | Projector(p) => [
        hd,
        ...remold_module_signature_entry(snd(ProjectorCore.shapes(p)), tl),
      ]
    | Tile(t) =>
      switch (remold_tile(ModuleEntry, shape, t)) {
      | None => [
          Tile(t),
          ...remold_module_signature_entry(snd(Tile.shapes(t)), tl),
        ]
      | Some(t) when !Tile.has_end(Right, t) =>
        let (_, r) = Tile.nibs(t);
        let remolded = remold(~shape=r.shape, tl, r.sort);
      ...
      | Some(t) =>
        ...
        | (_, {shape, sort: Typ}) =>
          let (remolded, shape, rest) =
            remold_typ_uni(shape, tl, [Sort.ModuleEntry]);
          [Piece.Tile(t), ...remolded]
          @ remold_module_signature_entry(shape, rest);
        | _ => [
            Tile(t),
            ...remold_module_signature_entry(snd(Tile.shapes(t)), tl),
          ]
