      ...
      ~f=
        ((l, child, r), children) => {
          let child =
            if (l
                + 1 == r
                && List.nth_exn(remolded.mold.in_, l)
                != List.nth_exn(t.mold.in_, l)) {
              remold(child, List.nth_exn(remolded.mold.in_, l));
            } else {
              child;
            };
          [child, ...children];
    ...
    | Tile(t) =>
      ...
      | Some(t)
          when
            ...
            && List.exists(~f=y => Sort.equal(Sort.Mod, y), parent_sorts) => (
          [],
          shape,
          seg,
        )
...
let first_string =
  ...
  | [Piece.Grout(_), ..._] => "?"
  | [Piece.Tile(t), ..._] => t.label |> List.hd_exn;
