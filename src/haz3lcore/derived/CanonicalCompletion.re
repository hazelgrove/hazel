...
let finish_display =
  ...
  let edge =
    ...
    | Tile(t) =>
      ...
      switch (sh) {
      | None => None
      | Some(i) => Some((Tile.token(t, i), mark_mem(t.id, Some(i))))
      };
    };
  ...
  let piece_prefix_op = (p: Piece.t): bool =>
    switch (p) {
    | Tile(t) => Mold.is_prefix_op(Tile.mold(t))
    | _ => false
    };
  ...
  and pad_piece = (~hot: bool, p: Piece.t): Piece.t =>
    ...
    | Tile(t) =>
      ...
      let bound = (k: int) => {
        ...
        (Tile.token(t, i), mark_mem(t.id, Some(i)));
      };
      let children =
        t.children
        |> List.mapi((k, c) => {
...
/* Zipper-facing queries (which records the caret owns, what Tab does)
   live in CompletionQuery; the ghost-splicing helpers above are the
   display fork's and read the caret only through its atoms. */
