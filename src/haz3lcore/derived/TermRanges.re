// open Util;
  ...
  //     res;
  //   };
  // let rec term_segs = (seg: Segment.t): Id.Map.t(Segment.t) => {
  //   let union = Id.Map.union((_, seg: Segment.t, _) => Some(seg));
  //   let rec go = (skel: Skel.t): Id.Map.t(Segment.t) => {
  //     let root = Aba.map_a(List.nth(seg), Skel.root(skel));
  //     let unichild_map =
  //       switch (skel) {
  //       | Op(_) => empty
  //       | Pre(_, r) => go(r)
  //       | Post(l, _) => go(l)
  //       | Bin(l, _, r) => union(go(l), go(r))
  //       };
  //     let map =
  //       Aba.get_bs(root)
  //       |> List.map(go)
  //       |> List.fold_left(union, unichild_map);
  //     let (l, r) = Skel.range(skel);
  //     let range = ListUtil.sublist((l, r + 1), seg);
  //     Aba.get_as(root)
  //     |> List.map(Piece.id)
  //     |> List.fold_left((map, id) => Id.Map.add(id, range, map), map);
  //   };
  //   seg
  //   |> List.concat_map(
  //        fun
  ...
  //      )
  //   |> List.fold_left(
  //        (map, kid) => union(map, term_segs(kid)),
  //        go(Segment.skel(seg)),
  //      );
  // };
