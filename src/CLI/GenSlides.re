 ...
 *   2. Checks each file parses, then embeds its text (PersistentZipper)
...
/* Check if a relative path matches any of the given filter prefixes */
let matches_filters = (filters: list(string), rel_path: string): bool =>
  switch (filters) {
  | [] => true /* No filter = match all */
  | _ =>
    List.exists(
      filter => {
        let flen = String.length(filter);
        let rlen = String.length(rel_path);
        rlen >= flen && String.sub(rel_path, 0, flen) == filter;
      },
      filters,
    )
  };

/* Scan output directory for all existing .ml module names */
let find_existing_modules = (): list(string) =>
  try({
    let entries = Sys.readdir(output_dir) |> Array.to_list;
    List.filter_map(
      entry =>
        if (Filename.check_suffix(entry, ".ml")) {
          Some(Filename.chop_suffix(entry, ".ml"));
        } else {
          None;
        },
      entries,
...
/* Generate a single ML file for a .hz program. The slide IS the program
   text: it is embedded verbatim and parsed at load (of_slide_text), the
   same path the doc/B2T2 slides take. Parsing here only checks that the
   source is loadable. */
...
let generate_aggregation_module = (modules: list((string, string))): unit => {
  ...
  let content =
    ...
    ++ "let all: list((string, Haz3lcore.PersistentZipper.t)) = [\n"
    ++ module_refs
    ++ "\n];\n";
...
/* Generate aggregation module from all .ml files on disk */
let generate_aggregation_from_disk = (): unit => {
  let modules = find_existing_modules();
  let module_refs =
    List.map(name => "  " ++ name ++ ".out,", modules) |> String.concat("\n");
...
let generate = (filters: list(string)): unit => {
  ...
  let hz_files =
    switch (filters) {
    | [] => hz_files
    | _ => List.filter(matches_filters(filters), hz_files)
    };
  ...
  switch (filters) {
  | [] =>
    /* Full rebuild: AllStudy.re from what we just generated */
    generate_aggregation_module(generated_modules)
  | _ =>
    /* Filtered rebuild: AllStudy.re from all .ml files on disk */
    generate_aggregation_from_disk()
  };
  generate_dune_file();
