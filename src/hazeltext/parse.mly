%{
  ...
  let mk_inj tag expr =
    UHExp.Inj(InjErrStatus.NotInHole, tag, expr)
  ...
  let mk_rule pat expr =
    let pat = UHPat.mk_OpSeq pat in
    UHExp.Rule(NotRedundant, pat, expr)

  let mk_match expr rules = UHExp.new_match expr rules
...
%left COMMA
%left COLON
%nonassoc LBRACK CASE LPAREN IDENT WILD FUN INJ EMPTY_HOLE INT FLOAT TRUE FALSE
%nonassoc app
...
expr_:
  ...
  | CASE block rule+ END { mk_match $2 $3 }
  ...
  | INJ LBRACK tag RBRACK LPAREN block RPAREN { mk_inj $3 (Some $6) }
...
tag:
  IDENT {
    if UHTag.is_tag_name $1 then
      UHTag.Tag(NotInTagHole, $1)
    else
      let (it, _) = UHTag.new_TagHole IDGen.init in
      it
  }
  | EMPTY_HOLE {
    let (it, _) = UHTag.new_TagHole IDGen.init in
