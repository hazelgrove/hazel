  ...
  Module Dynamics.
      ...
      Module Evaluator.
        ...
        *)
        ...
        Definition is_var (d : DHExp.t) : bool :=
          match d with
          | DHExp.BoundVar _
          | DHExp.FreeVar _ _ _ _ => true
          | _ => false
          end.

        Fixpoint cannot_step (fuel : Fuel.t) (d : DHExp.t) : bool :=
          match fuel with
          | Fuel.Kicked => false
          | Fuel.More fuel => is_var d || is_final fuel d
          end.
        ...
        Fixpoint decompose (fuel : Fuel.t) (d : DHExp.t) : EvalCtx.t * DHExp.t :=
          ...
          | DHExp.Let x d1 d2 =>
            match cannot_step fuel d1 with
            | false =>
              let (ctx, d') := decompose fuel d1 in
              (EvalCtx.Let1 x ctx d2, d')
            | true =>
              match cannot_step fuel d2 with
              | false =>
          ...
          | DHExp.Case d1 (x,d2) (y,d3) =>
            ...
            | true =>
              ...
              | true =>
                match cannot_step fuel d3 with
                | false =>
        ...
        Definition instruction_step (fuel : Fuel.t) (d : DHExp.t) : option DHExp.t :=
          match fuel with
          | Fuel.Kicked => None
          | Fuel.More fuel =>
          match d with
          ...
          | DHExp.NumLit _
          | DHExp.Inj _ _ _
          | DHExp.EmptyHole _ _ _ => None
          | DHExp.Let x d1 d2 => Some (DHExp.subst fuel d1 x d2)
          | DHExp.Ap d1 d2 =>
            match d1 with
            | DHExp.Lam x _ d1 => Some (DHExp.subst fuel d2 x d1)
            | DHExp.Cast d1 (HTyp.Arrow t1 t2 as s) (HTyp.Arrow t1' t2' as s') =>
              if HTyp.eq s s' then
                None
              else
               Some (DHExp.Cast (DHExp.Ap d1 (DHExp.Cast d2 t1' t1)) t2 t2')
            | _ => None
            end
          | DHExp.BinNumOp op d1 d2 =>
            match (d1, d2) with
            | (DHExp.NumLit n1, DHExp.NumLit n2) => Some (DHExp.NumLit (eval_bin_num_op op n1 n2))
            | _ => None
            end
          | DHExp.Case d1 (x,d2) (y,d3) =>
            match (d1, d2, d3) with
