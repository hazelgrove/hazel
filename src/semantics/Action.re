...
type t =
  ...
  | MoveToNextHole
  | MoveToPrevHole
  | UpdateApPalette(UHExp.HoleRefs.m_hole_ref(PaletteSerializedModel.t))
  | Delete
  | Backspace
  | Construct(shape);
...
let syn_zpat_fix_holes =
  ...
  let p = ZPat.erase(zp);
  switch (UHExp.syn_pat_fix_holes(ctx, u_gen, false, p)) {
  | None => None
  | Some((p, ty, ctx, u_gen)) =>
...
let ana_zpat_fix_holes =
  ...
  let path = Path.of_zpat(zp);
  let p = ZPat.erase(zp);
  switch (UHExp.ana_pat_fix_holes(ctx, u_gen, false, p, ty)) {
  | None => None
  | Some((p, ctx, u_gen)) =>
    switch (Path.follow_pat(path, p)) {
    | None => None
...
let make_and_syn_OpSeqZ_pat =
  ...
  switch (UHExp.syn_skel_pat_fix_holes(ctx, u_gen, false, skel, seq)) {
...
let make_and_ana_OpSeqZ_pat =
  ...
  let seq = OperatorSeq.opseq_of_exp_and_surround(p0, surround);
  let skel = Associator.associate_pat(seq);
  switch (UHExp.ana_skel_pat_fix_holes(ctx, u_gen, false, skel, seq, ty)) {
...
let rec perform_syn_pat =
  ...
  | (_, ZPat.Deeper(_, ZPat.OpSeqZ(_, zp0, surround))) =>
    ...
    | UHPat.Pat(_, UHPat.OpSeq(skel, seq)) =>
      switch (UHExp.syn_skel_pat(ctx, skel, seq, Some(i))) {
...
and perform_ana_pat =
  ...
  | (Construct(SInj(side)), ZPat.CursorP(cursor_side, p1)) =>
    ...
    | Some((tyL, tyR)) =>
      ...
      switch (UHExp.ana_pat_fix_holes(ctx, u_gen, false, p1, ty1)) {
      | None => None
      | Some((p1, ctx, u_gen)) =>
        let zp =
          ZPat.Deeper(
    ...
    | None =>
      switch (UHExp.syn_pat_fix_holes(ctx, u_gen, false, p1)) {
      | None => None
      | Some((p1, _, ctx, u_gen)) =>
  ...
  | (_, ZPat.Deeper(_, ZPat.OpSeqZ(_, zp0, surround))) =>
    ...
    switch (ZPat.erase(zp)) {
    | UHPat.Pat(_, UHPat.OpSeq(skel, seq)) =>
      switch (UHExp.ana_skel_pat(ctx, skel, seq, ty, Some(i))) {
      | Some((ctx, Some(mode))) =>
        switch (mode) {
        | UHExp.AnalyzedAgainst(ty0) =>
          switch (perform_ana_pat(ctx, u_gen, a, zp0, ty0)) {
...
let zexp_syn_fix_holes =
  ...
  let e = ZExp.erase(ze);
  switch (UHExp.syn_fix_holes(ctx, u_gen, e)) {
  | Some((e', ty, u_gen')) =>
...
let zexp_ana_fix_holes =
  ...
  let path = Path.of_zexp(ze);
  let e = ZExp.erase(ze);
  switch (UHExp.ana_fix_holes(ctx, u_gen, e, ty)) {
  | Some((e', u_gen')) =>
    switch (Path.follow_e(path, e')) {
...
let make_and_syn_OpSeqZ =
  ...
  switch (UHExp.syn_skel_fix_holes(ctx, u_gen, false, skel, seq)) {
...
let rec perform_syn =
  ...
  | (
    ...
    | None => None
    | Some((e1, ty1, u_gen)) =>
      switch (UHExp.ana_pat_fix_holes(ctx, u_gen, false, p, ty1)) {
      | None => None
      | Some((p, ctx, u_gen)) =>
        switch (UHExp.syn_fix_holes(ctx, u_gen, e2)) {
        | None => None
        | Some((e2, ty, u_gen)) =>
          let ze =
            ZExp.Deeper(
              NotInHole,
              ZExp.LineItemZL(
                ZExp.LetLineZP(ZPat.CursorP(After, p), None, e1),
                e2,
              ),
            );
          Some((ze, ty, u_gen));
  ...
  | (
      ...
      ZExp.Deeper(
        _,
        ZExp.LineItemZL(
          ZExp.LetLineZP(ZPat.CursorP(After, _) as zp, Some(_), e1),
          e2,
        ),
      ),
    )
  ...
  | (
    ...
    switch (UHExp.ana_pat_fix_holes(ctx, u_gen, false, p, HTyp.Hole)) {
    | None => None
    | Some((p, ctx, u_gen)) =>
      switch (UHExp.syn_fix_holes(ctx, u_gen, e1)) {
      | None => None
  ...
  | (
      ...
      ZExp.Deeper(
        err_status,
        ZExp.LineItemZL(ZExp.LetLineZP(zp, Some(uty1), e1), e2),
      ),
    ) =>
  ...
  | (
    ...
    | Some(palette_defn) =>
      ...
      switch (UHExp.ana(expansion_ctx, expansion, expansion_ty)) {
      | Some(_) =>
        Some((
          ZExp.CursorE(
            After,
  ...
  | (_, ZExp.Deeper(_, ZExp.LineItemZL(ZExp.LetLineZP(zp, ann, e1), e2))) =>
    switch (ann) {
    | Some(uty1) =>
      let ty1 = UHTyp.expand(uty1);
      ...
      | Some((zp, ctx2, u_gen)) =>
        let p = ZPat.erase(zp);
        let ctx1 = UHExp.ctx_for_let(ctx, p, ty1, e1);
        switch (UHExp.ana_fix_holes(ctx1, u_gen, e1, ty1)) {
        | None => None
        | Some((e1, u_gen)) =>
          switch (UHExp.syn_fix_holes(ctx2, u_gen, e2)) {
          | None => None
          | Some((e2, ty, u_gen)) =>
            let ze =
              ZExp.Deeper(
  ...
  | (_, ZExp.Deeper(_, ZExp.LineItemZL(ZExp.LetLineZA(p, zann, e1), e2))) =>
        ...
        | Some((e1, u_gen)) =>
          ...
          | Some((e2, ty, u_gen)) =>
            let ze =
              ZExp.Deeper(
                NotInHole,
                ZExp.LineItemZL(ZExp.LetLineZA(p, zann, e1), e2),
              );
            Some((ze, ty, u_gen));
          }
        };
  ...
  | (_, ZExp.Deeper(_, ZExp.LineItemZL(ZExp.LetLineZE(p, ann, ze1), e2))) =>
    switch (ann) {
    | Some(ann_ty) =>
      let ty1 = UHTyp.expand(ann_ty);
      let ctx1 = UHExp.ctx_for_let(ctx, p, ty1, ZExp.erase(ze1));
      ...
      | Some((ze1, u_gen)) =>
        let ze =
          ZExp.Deeper(
            NotInHole,
            ZExp.LineItemZL(ZExp.LetLineZE(p, ann, ze1), e2),
          );
        Some((ze, ty, u_gen));
      };
    | None =>
      ...
      | Some(ctx2) =>
        ...
        | Some((ze2, ty, u_gen)) =>
          let ze =
            ZExp.Deeper(
              NotInHole,
              ZExp.LineItemZE(UHExp.LetLine(p, ann, e1), ze2),
            );
          Some((ze, ty, u_gen));
        }
      }
  ...
  | (_, ZExp.Deeper(_, ZExp.OpSeqZ(_, ze0, surround))) =>
    ...
    | UHExp.Tm(_, UHExp.OpSeq(skel, seq)) =>
      switch (UHExp.syn_skel(ctx, skel, seq, Some(i))) {
      | Some((ty, Some(mode))) =>
        switch (mode) {
        | UHExp.AnalyzedAgainst(ty0) =>
  ...
  | (
    ...
    let (next_lbl, z_nat_map) = z_hole_data;
...
and perform_ana =
  ...
  | (
    ...
    zexp_ana_fix_holes(ctx, u_gen, ze', ty);
  | (Delete, ZExp.Deeper(_, ZExp.AscZ1(ZExp.CursorE(After, e1), _))) =>
    switch (UHExp.ana_fix_holes(ctx, u_gen, e1, ty)) {
    | Some((e1', u_gen)) =>
      let ze' = ZExp.CursorE(After, e1');
      Some((ze', u_gen));
    | None => None
  ...
  | (
    ...
    | Some((e1, ty1, u_gen)) =>
      ...
      | None => None
      | Some((p, ctx, u_gen)) =>
        switch (UHExp.ana_fix_holes(ctx, u_gen, e2, ty)) {
        | None => None
        | Some((e2, u_gen)) =>
          let ze =
            ZExp.Deeper(
              NotInHole,
              ZExp.LineItemZL(
                ZExp.LetLineZP(ZPat.place_After(p), None, e1),
                e2,
              ),
            );
          Some((ze, u_gen));
  ...
  | (
    ...
    | Some((ty1, ty2)) =>
      ...
      | None => None
      | Some((p, ctx, u_gen)) =>
        switch (UHExp.ana_fix_holes(ctx, u_gen, e1, ty2)) {
        | None => None
        | Some((e1, u_gen)) =>
  ...
  | (Construct(SLam), ZExp.CursorE(_, e)) =>
    switch (HTyp.matched_arrow(ty)) {
    | Some((_, ty2)) =>
      switch (UHExp.ana_fix_holes(ctx, u_gen, e, ty2)) {
      | None => None
      | Some((e, u_gen)) =>
        let (zp, u_gen) = ZPat.new_EmptyHole(u_gen);
  ...
  | (Construct(SInj(side)), ZExp.CursorE(cursor_side, e1)) =>
    ...
    | Some((tyL, tyR)) =>
      let ty1 = pick_side(side, tyL, tyR);
      switch (UHExp.ana_fix_holes(ctx, u_gen, e1, ty1)) {
      | None => None
      | Some((e1, u_gen)) =>
        let ze =
          ZExp.Deeper(
  ...
  | (_, ZExp.Deeper(_, ZExp.LineItemZL(ZExp.LetLineZP(zp, ann, e1), e2))) =>
    ...
    | Some(uty1) =>
      ...
      | Some((zp, ctx2, u_gen)) =>
        ...
        | None => None
        | Some((e1, u_gen)) =>
          switch (UHExp.ana_fix_holes(ctx2, u_gen, e2, ty)) {
          | None => None
          | Some((e2, u_gen)) =>
            let ze =
              ZExp.Deeper(
  ...
  | (_, ZExp.Deeper(_, ZExp.CaseZE(ze1, rules))) =>
    switch (UHExp.syn(ctx, ZExp.erase(ze1))) {
    | None => None
    | Some(ty1) =>
      switch (perform_syn(ctx, a, (ze1, ty1, u_gen))) {
      | None => None
      | Some((ze1, ty1, u_gen)) =>
        switch (UHExp.ana_rules_fix_holes(ctx, u_gen, false, rules, ty1, ty)) {
        | None => None
        | Some((rules, u_gen)) =>
  ...
  | (_, ZExp.Deeper(_, ZExp.OpSeqZ(_, ze0, surround))) =>
    ...
    switch (ZExp.erase(ze)) {
    | UHExp.Tm(_, UHExp.OpSeq(skel, seq)) =>
      switch (UHExp.ana_skel(ctx, skel, seq, ty, Some(i))) {
      | Some(Some(mode)) =>
        switch (mode) {
        | UHExp.AnalyzedAgainst(ty0) =>
          switch (perform_ana(u_gen, ctx, a, ze0, ty0)) {
...
and perform_ana_subsume =
    (u_gen: MetaVarGen.t, ctx: Contexts.t, a: t, ze: ZExp.t, ty: HTyp.t)
    : option((ZExp.t, MetaVarGen.t)) =>
  switch (UHExp.syn(ctx, ZExp.erase(ze))) {
  | Some(ty1) =>
    switch (perform_syn(ctx, a, (ze, ty1, u_gen))) {
    | Some((ze', ty1', u_gen')) =>
