...
let rec split n seq =
  ...
  | (_, ExpOpExp (_, _, _)) ->
    None
  | (_, SeqOpExp (seq', op, e)) ->
    let length' = seq_length seq' in
    if n < length' then
      begin match split n seq' with
      | Some (e', surround) ->
