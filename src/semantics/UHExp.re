...
and syn_line_items = (ctx, lis) =>
  switch (lis) {
  | [] => Some(ctx)
  | [li, ...lis] =>
    switch (syn_line_item(ctx, li)) {
    | None => None
    | Some(ctx) => syn_line_items(ctx, lis)
    }
  }
...
and syn_fix_holes' = (ctx, u_gen, renumber_empty_holes, e) =>
  ...
  | LineItems(lis, e1) =>
    switch (syn_fix_holes_line_items(ctx, u_gen, renumber_empty_holes, lis)) {
    | None => None
    | Some((lis, ctx, u_gen)) =>
      switch (syn_fix_holes_internal(ctx, u_gen, renumber_empty_holes, e1)) {
      | None => None
      | Some((e1, ty, u_gen)) => Some((LineItems(lis, e1), ty, u_gen))
      }
    }
...
and syn_fix_holes_line_items = (ctx, u_gen, renumber_empty_holes, lis) =>
  switch (lis) {
  | [] => Some((lis, ctx, u_gen))
  | [li, ...lis] =>
    ...
    | None => None
    | Some((li, ctx, u_gen)) =>
      syn_fix_holes_line_items(ctx, u_gen, renumber_empty_holes, lis)
    }
  }
...
and ana_fix_holes' = (ctx, u_gen, renumber_empty_holes, e, ty) =>
  ...
  | LineItems(lis, e1) =>
    ...
    | Some((lis, ctx, u_gen)) =>
      ...
      | None => None
      | Some((e1, u_gen)) => Some((NotInHole, LineItems(lis, e1), u_gen))
      }
    }
  | Lam(p, ann, e1) =>
    switch (HTyp.matched_arrow(ty)) {
