...
and syn_skel =
  ...
  | BinOp(InHole(WrongLength, u), Comma as op, skel1, skel2) =>
    let skel_not_in_hole = Skel.BinOp(NotInHole, op, skel1, skel2);
    switch (syn_skel(ctx, skel_not_in_hole, seq, monitor)) {
    | None => None
    | Some((ty, mode)) => Some((Hole, mode))
    };
...
let rec syn_fix_holes_pat =
  ...
  switch (p) {
  | Pat(_, p') =>
    switch (syn_fix_holes_pat'(ctx, u_gen, ~renumber_empty_holes, p')) {
    ...
    | (p', ty, ctx, u_gen) => (Pat(NotInHole, p'), ty, ctx, u_gen)
    }
  | Parenthesized(p) =>
    let (p, ty, ctx, u_gen) =
...
and ana_fix_holes_pat' =
  ...
  | OpSeq(skel, seq) =>
    switch (
      ana_fix_holes_pat_skel(ctx, u_gen, ~renumber_empty_holes, skel, seq, ty)
    ) {
    | (Placeholder(_), _, _, _) =>
