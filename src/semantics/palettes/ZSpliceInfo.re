...
type t('exp, 'zexp) = {next: SpliceInfo.splice_name, zsplice_map: zsplice_map('exp, 'zexp), splice_order: list(SpliceInfo.splice_name) };
let erase = (zpsi : t('exp, 'zexp), erase_z): SpliceInfo.t('exp) => 
  SpliceInfo.({ next: zpsi.next, splice_map: Util.ZNatMap.erase(zpsi.zsplice_map, erase_z), splice_order: zpsi.splice_order }); 
let select_opt = 
