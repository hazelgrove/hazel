...
/*
 let rec _steps_of_first_line__zblock =
   fun
   | BlockZL((prefix, CursorL(OnText(0), EmptyLine), suffix), EmptyHole(_)) => {
     if (suffix |> List.for_all(li => li == EmptyLine)) {
       let start_of_contiguous_empty_lines =
         fold_right_i(
           ((i, prefix_line), (start, broke_contiguity)) =>
             broke_contiguity
             ? (start, broke_contiguity)
             : (
               prefix_line == EmptyLine
               ? (i, false)
               : (start, true)
             ),
           prefix,
           (prefix |> List.length, false),
         );
       Some([start_of_contiguous_empty_lines, ...steps]);
     } else {
       None;
     };
     }
   | BlockZE(leading, CursorE(_, EmptyHole(_))) => {
     let start_of_contiguous_empty_lines =
       fold_right_i(
         ((i, prefix_line), (start, broke_contiguity)) =>
           broke_contiguity
         ...
         leading,
         (leading |> List.length, false),
       );
     Some([start_of_contiguous_empty_lines, ...steps]);
   }
   | BlockZL((prefix, zline, _), _) =>
     cons_opt(prefix |> List.length, zline |> _steps_of_first_line__zline)
 and _steps_of_first_line__zline =
   fun
   | CursorL(_, _)
   | LetLineZP(_, _, _)
   | LetLineZA(_, _, _) => None
   | ExpLineZ(ze) => ze |> _steps_of_first_line__zexp(~steps)
   | LetLineZE(_, _, zdef) => cons_opt(2, zdef |> _steps_of_first_line__zblock(~steps))
 and _steps_of_first_line__zexp =
   fun
   | CursorE(_, _)
   | LamZP(_, _, _, _)
   | LamZA(_, _, _, _)
   ...
   | OpSeqZ(_, ztm, surround) =>
     cons_opt(
       surround |> OperatorSeq.surround_prefix_length,
       ztm |> _steps_of_first_line__zexp)
   | ParenthesizedZ(zblock)
   | InjZ(zblock)
   | CaseZE(_, zblock, _, _) =>
     cons_opt(
       0,
       zblock |> _steps_of_first_line__zblock)
   | LamZE(_, _, _, zbody) =>
     cons_opt(
       2,
       zbody |> _steps_of_first_line__zblock
     )
   | CaseZR(_, _, (prefix, RuleZE(_, zclause), _), _) =>
     cons_opt(
       (prefix |> List.length) + 1,
       cons_opt(
         2,
         zclause |> _steps_of_first_line__zblock
       )
     );

 let steps_of_first_line_of_current_trivial_block = (zblock) =>
   _steps_of_first_line__zblock(zblock);

 let rec _add_empty_lines__block = (~steps_of_first_line, ~num_empty_lines) =>
   fun
   | Block(leading, conclusion) =>
     switch (steps_of_first_line) {
     | [] => assert(false)
     | [n] =>
       let empty_lines = (EmptyLine |> replicate(num_empty_lines);
       switch (leading |> ZList.split_at(n)) {
       | None => Block(leading @ empty_lines), conclusion)
       | Some((prefix, line, suffix), conclusion) =>
         Block(prefix @ [line, ...empt] @ )
       }
 ...
 let add_empty_lines_to_trivial_block = (~steps_of_first_line, ~num_empty_lines, zblock) => {
   let path = of_zblock(zblock);
   let new_block =
     _add_empty_lines__block(~steps_of_first_line, ~num_empty_lines, zblock |> ZExp.erase_block);
   // TODO add note about invariant that ensures cursor returns to intended position
   follow_block(path, new_block);
 }
 */
