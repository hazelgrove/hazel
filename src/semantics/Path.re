...
let rec holes_seq = (seq, holes_tm, offset, steps, holes) : hole_list => 
  ...
  | OperatorSeq.SeqOpExp(seq1, op, e2) =>
    let holes = holes_tm(e2, [offset + OperatorSeq.seq_length(seq1), ...steps], holes)
...
let rec holes_pat = (p, steps, holes) : hole_list => 
  ...
  | UHPat.Pat(_, UHPat.OpSeq(skel, seq)) => holes_seq(seq, holes_pat, 0, steps, holes)
  };
...
let rec holes_e = (e, steps, holes) : hole_list => 
  ...
  | UHExp.Tm(_, UHExp.Let(p, ann, e1, e2)) =>
    let holes = holes_e(e2, [3, ...steps], holes);
    let holes = holes_e(e1, [2, ...steps], holes);
    let holes = 
      switch (ann) {
      | Some(uty) => holes_uty(uty, [1, ...steps], holes)
...
let string_of_zhole_list = ({holes_before, hole_selected, holes_after}) => {
  "{ holes_before: " ++ string_of_hole_list(holes_before) ++ ", \n" ++
  "  hole_selected: " ++ 
    Util.string_of_opt(
      Util.string_of_pair(string_of_hole_desc, string_of_steps), 
      hole_selected) ++ ", \n" ++ 
  "  holes_after: " ++ string_of_hole_list(holes_after) ++ "}\n"
};
...
let holes_suffix = (holes_fn, suffix, prefix_len, steps) : hole_list => 
  switch (suffix) {
  | OperatorSeq.ExpSuffix(_, e) => holes_fn(e, [prefix_len + 1, ...steps], [])
  | OperatorSeq.SeqSuffix(_, seq) => holes_seq(seq, holes_fn, prefix_len + 1, steps, [])
  };
...
let holes_OpSeqZ = (holes_fn, zholes_fn, z0, surround, steps) => { 
  let (holes_prefix, holes_suffix) = holes_surround(holes_fn, surround, steps);
...
let rec holes_ze = (ze, steps) : zhole_list => 
  ...
  | ZExp.Deeper(_, ze) =>
    switch (ze) {
    | ZExp.AscZ1(ze1, uty) =>
      let { holes_before, hole_selected, holes_after } = holes_ze(ze1, [0, ...steps]);
      { holes_before,
        hole_selected,
        holes_after: holes_after @ holes_uty(uty, [1, ...steps], []) }
    | ZExp.AscZ2(e1, zty1) => 
      let { holes_before, hole_selected, holes_after } = holes_zty(zty1, [1, ...steps]);
      { holes_before: holes_e(e1, [0, ...steps], []) @ holes_before,
        hole_selected,
        holes_after }
    | ZExp.LetZP(zp, ann, e1, e2) =>
      let { holes_before, hole_selected, holes_after } = holes_zpat(zp, [0, ...steps]);
      let holes_ann = 
        switch(ann) { 
      ...
      { holes_before,
        hole_selected,
        holes_after: holes_after @ holes_ann @ holes_e1 @ holes_e2 } 
    | ZExp.LetZA(p, zann, e1, e2) =>
      let { holes_before, hole_selected, holes_after } = holes_zty(zann, [1, ...steps]);
      let holes_p = holes_pat(p, [0, ...steps], []);
      let holes_e1 = holes_e(e1, [2, ...steps], []);
      let holes_e2 = holes_e(e2, [3, ...steps], []);
      { holes_before: holes_p @ holes_before,
        hole_selected,
        holes_after: holes_after @ holes_e1 @ holes_e2 }
    | ZExp.LetZE1(p, ann, ze1, e2) =>
      let { holes_before, hole_selected, holes_after } = holes_ze(ze1, [2, ...steps]);
      let holes_p = holes_pat(p, [0, ...steps], []);
      let holes_ann = 
        switch(ann) { 
        | Some(uty) => holes_uty(uty, [1, ...steps], [])
      ...
      { holes_before: holes_p @ holes_ann @ holes_before,
        hole_selected,
        holes_after: holes_after @ holes_e2 }
    | ZExp.LetZE2(p, ann, e1, ze2) => 
      let { holes_before, hole_selected, holes_after } = holes_ze(ze2, [3, ...steps]);
      let holes_p = holes_pat(p, [0, ...steps], []);
      let holes_ann = 
        switch(ann) { 
        | Some(uty) => holes_uty(uty, [1, ...steps], [])
      ...
      let holes_e1 = holes_e(e1, [2, ...steps], []);
      { holes_before: holes_p @ holes_ann @ holes_e1 @ holes_before,
        hole_selected,
        holes_after }
    ...
    | ZExp.CaseZR(e1, zrules) =>
      let { holes_before, hole_selected, holes_after } = holes_zrules(zrules, steps);
...
let rec steps_to_hole_z = (zhole_list, u): option(steps) => {
  ...
  | None => 
    ...
    | Some((PatHole(u'), steps)) => 
      switch (MetaVar.eq(u, u')) {
      | true => Some(steps)
      | false => steps_to_hole(holes_after, u)
      }
    | Some((TypeHole, _))
...
let opt_steps_to_opt_path = cursor_side => fun
| None => None
