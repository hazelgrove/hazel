...
let mk_all = (~instructor_mode): all => {
  ...
  //let explainThisMessages = LocalStorage.ExplainThisMessages.export();
  //print_endline("ExplainThisMessages OK");
  ...
  let log = Log.export();
  {settings, /*explainThisMessages,*/ scratch, school, log};
};
...
let import_all = (data, ~specs) => {
  ...
  //LocalStorage.ExplainThisMessages.import(all.explainThisMessages);
