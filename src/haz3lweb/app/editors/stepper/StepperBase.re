...
module Model = {
  ...
  let init_step =
    MissingStep({common: init_step_common, next_steps: Calc.Pending});
...
module Update = {
  ...
  and update_step =
    ...
    | (StepForward(i), Model.MissingStep(m)) =>
      switch (List.nth_opt(m.next_steps |> Calc.get_saved([]), i)) {
      | Some(next) =>
        Model.SingleStep(
          Model.{
            common: init_step_common,
  ...
  and update_single_step =
    ...
    | NextStepUpdate(a) =>
      ...
      ({...model, next_step: new_next_step}: Model.single_step);
    };
  }
  and update_induction_step =
    ...
    | ScrutUpdate(a) =>
      ...
      ({...model, scrut: new_scrut}: Model.induction_step);
    ...
    | AddCase =>
      ...
      ({...model, cases: model.cases @ [new_case]}: Model.induction_step)
      |> return;
    | RemoveCase(i) =>
      switch (ListUtil.remove_nth(i, model.cases)) {
      | Some(new_cases) =>
        ({...model, cases: new_cases}: Model.induction_step) |> return
      | None => model |> return_quiet
      }
    | NextStepUpdate(a) =>
      let* new_next_step = update_step(~settings, a, model.next_step);
      ({...model, next_step: new_next_step}: Model.induction_step);
    };
  };
  ...
  let rec calculate_stepper =
    ...
    calculate_stepper'(~settings, elab, {cached_settings, cached_elab, root})
    |> fst;
  }
  ...
  and calculate_missing_step =
    ...
    | None =>
      ...
      (
        Model.MissingStep({common, next_steps: next_steps |> Calc.save}),
        exp,
      );
    };
  }
          ...
          let+ (filer_action, eo) =
            List.find_opt(
              ((_, eo: EvaluatorStep.EvalObj.t)) =>
                eo.d_loc.ids == evalobj.d_loc.ids,
              next_steps,
            );
          let hidden =
            switch (filer_action) {
...
module Selection = {
  ...
  and handle_key_event_missing_step =
    ...
    |> Option.map((x) => (Update.EditorAction(x): Update.missing_step));
  }
  and handle_key_event_single_step =
    ...
    | Here(a) =>
      ...
      |> Option.map((x) => (Update.EditorAction(x): Update.single_step));
    | Next(a) =>
      handle_key_event_step(~selection=a, ~event, model.next_step)
      |> Option.map((x) => (Update.NextStepUpdate(x): Update.single_step))
    }
  and handle_key_event_induction_step =
    ...
    | Next(a) =>
      handle_key_event_step(~selection=a, ~event, model.next_step)
      |> Option.map((x) => (Update.NextStepUpdate(x): Update.induction_step))
    };
};
    ...
    let editor =
      StepperEditor.View.view(
        ...
        StepperEditor.Model.{
          ...
          taken_steps: [model.evalobj.d_loc |> Exp.rep_id],
          next_steps: [],
        },
      );
    let next_step =
