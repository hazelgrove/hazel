...
let get_doc =
    (
      ~docs: ExplainThisModel.t,
      info: info,
      // TODO(zhiyao): change this info to be a combination of info and result
  ...
  let get_message =
      (
        ...
        // TODO(zhiyao): Doing something for different verifications results
        ~explanation: option(string)=?,
        group: ExplainThisForm.group,
      )
      // TODO(zhiyao): extend group for verifications results
  ...
  | Deduction(Some({res: Incorrect(failure), _})) =>
    ...
    | FailTest(s, operation) =>
      ...
      | Subst2((v1, x1), (v2, x2), e) =>
        ...
        let v2_id = List.nth(v2.self.ids, 0);
      ...
      | ConsHasTy2((x1, t1), (x2, t2), l) =>
        let x1_id = List.nth(x1.self.ids, 0);
        let t1_id = List.nth(t1.self.ids, 0);
        let x2_id = List.nth(x2.self.ids, 0);
  ...
  | Code(info) =>
        ...
        | Fun(pat, body, _, _) =>
          ...
          | Tuple([]) =>
            if (FunctionExp.function_triv_exp.id
              ...
              get_message(
                ~colorings=
                  FunctionExp.function_triv_exp_coloring_ids(
                    ~pat_id,
                    ~body_id,
                  ),
          ...
          | Var(var) =>
            if (FunctionExp.function_var_exp.id
              ...
              get_message(
                ~colorings=
                  FunctionExp.function_var_exp_coloring_ids(
                    ~pat_id,
                    ~body_id,
                  ),
                ~format=
          ...
          | Tuple(elements) =>
            ...
            | 3 =>
              let doc_id =
                get_specificity_level(FunctionExp.functions_tuple3);
          ...
          | Constructor(v, _) =>
            if (FunctionExp.function_ctr_exp.id
              ...
              get_message(
                ~colorings=
                  FunctionExp.function_ctr_exp_coloring_ids(
                    ~pat_id,
                    ~body_id,
                  ),
                ~format=
        ...
        | Let(pat, def, body) =>
          ...
          | Int(i) =>
            if (LetExp.let_int_exp.id
                == get_specificity_level(LetExp.lets_int)) {
              get_message(
                ~colorings=
          ...
          | Float(f) =>
            if (LetExp.let_float_exp.id
              ...
              get_message(
                ~colorings=
                  LetExp.let_float_exp_coloring_ids(
                    ~pat_id,
                    ~def_id,
                    ~body_id,
                  ),
          ...
          | Bool(b) =>
            if (LetExp.let_bool_exp.id
              ...
              get_message(
                ~colorings=
                  LetExp.let_bool_exp_coloring_ids(
                    ~pat_id,
                    ~def_id,
                    ~body_id,
                  ),
          ...
          | String(s) =>
            if (LetExp.let_str_exp.id
                == get_specificity_level(LetExp.lets_str)) {
              get_message(
                ~colorings=
          ...
          | Var(var) =>
            if (LetExp.let_var_exp.id
                == get_specificity_level(LetExp.lets_var)) {
              get_message(
                ~colorings=
          ...
          | Constructor(v, _) =>
            if (LetExp.let_ctr_exp.id
                == get_specificity_level(LetExp.lets_ctr)) {
              get_message(
                ~colorings=
      ...
      | Constructor(con, _) =>
        get_message(
          ~format=
            Some(
              msg =>
                Printf.sprintf(Scanf.format_from_string(msg, "%s"), con),
            ),
          TerminalPat.ctr(con),
        )
      | Cast(pat, typ, _) =>
    ...
    | Some(Secondary(s)) =>
      ...
      | Secondary(Whitespace) =>
        simple("A semantic void, pervading but inert")
      | Secondary(Comment) =>
        ...
        @ [
          section(
            ...
            ~title=
              ...
              | Deduction(Some({rule: Some(rule), _})) => Rule.show(rule)
              | Deduction(_) => "Unknown Rule"
              },
