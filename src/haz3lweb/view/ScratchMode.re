...
module Model = {
  ...
  let unpersist = (~settings, (current, slides), ~root) => {
    current,
    scratchpads:
      List.mapi(
        (i, m) =>
          (
            string_of_int(i),
            CellEditor.Model.unpersist(~settings, m, ~root),
          ),
        slides,
      ),
  };
  ...
  let unpersist_documentation = (~settings, (current, slides), ~root) => {
    current,
    scratchpads:
      List.map(
        ((s, m)) => (s, CellEditor.Model.unpersist(~settings, m, ~root)),
        slides,
      ),
  };
};
