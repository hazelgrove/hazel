  ...
  | [x, ...xs] =>
    ...
    | Inj(err, tag, body) =>
      ...
      | 0 =>
        ...
        |> Option.map(ztag => ZPat.InjZT(err, ztag, body))
      | 1 =>
        body
        |> follow((xs, cursor))
        |> Option.map(zbody => ZPat.InjZP(err, tag, zbody))
      | _ => None
      }
    | TypeAnn(err, op, ann) =>
      switch (x) {
...
and holes_zoperand =
  ...
  | InjZT(err, ztag, body) =>
    ...
    let ztag_holes = CursorPath_Tag.holes_z(ztag, [0, ...rev_steps]);
    let all_holes = {
      ...ztag_holes,
      holes_after: ztag_holes.holes_after @ body_holes,
    };
    switch (err) {
    | NotInHole => all_holes
    | InHole(_, u) => {
