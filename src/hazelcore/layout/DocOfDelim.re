...
let open_Parenthesized = (~caret=?, steps) =>
  doc(~caret?, ~path=(steps, 0), "(");
let close_Parenthesized = (~caret=?, steps) =>
  doc(~caret?, ~path=(steps, 1), ")");
...
let close_Inj = (~caret=?, steps) => doc(~caret?, ~path=(steps, 1), ")");
...
let close_Case = (~caret=?, steps) => doc(~caret?, ~path=(steps, 1), "end");
let close_Case_ann = (~caret=?, steps) =>
  doc(~caret?, ~path=(steps, 1), "end :");

let bar_Rule = (~caret=?, steps) => doc(~caret?, ~path=(steps, 0), "|");
let arrow_Rule = (~caret=?, steps) =>
  doc(~caret?, ~path=(steps, 1), LangUtil.caseArrowSym);

let let_LetLine = (~caret=?, steps) =>
  doc(~caret?, ~path=(steps, 0), "let");
let colon_LetLine = (~caret=?, steps) =>
  doc(~caret?, ~path=(steps, 1), ":");
let eq_LetLine = (~caret=?, steps) => doc(~caret?, ~path=(steps, 2), "=");
let in_LetLine = (~caret=?, steps) => doc(~caret?, ~path=(steps, 3), "in");
