...
let doc_of_TyLam = (stp_doc: doc, sbody_doc: doc): doc => {
  let lam_delim = Doc.Text(LangUtil.lamSym);
  let type_text_delim = Doc.Text("(type ");
  let dot_delim = Doc.Text(").");
  Doc.hcats([lam_delim, type_text_delim, stp_doc, dot_delim, sbody_doc]);
};
...
let doc_of_separated_tpat =
  ...
  let tp_doc = (~may_wrap as _: bool) =>
    (
      switch (tp) {
      | Hole(u) => doc_of_EmptyHole(u)
      | Var(x) => doc_of_Var(x)
      }
    )
    |> Doc.tag({steps, node_shape: TPat(tp)});
  tp_doc |> apply_seps(surround);
};
...
and doc_of_exp = (~may_wrap: bool, ~steps: CursorPath.steps, e: UHExp.t): doc => {
  let doc =
    ...
    | TyLam(_, tp, body) =>
      ...
      doc_of_TyLam(stp_doc, sbody_doc);
    | Lam(_, p, ann, body) =>
      let sp_doc =
        doc_of_separated_pat(
          ~surround=Both(Doc.empty, Doc.empty),
