...
let pad_child =
    (~inline_padding as (left, right): (ddoc, ddoc), child: ddoc): ddoc =>
  Choice([
...
let ddoc_of_Lam =
  ...
  switch (sann_doc) {
  | None => DDoc.(Choice([Cat([lam_delim, sp_doc, dot_delim, sbody_doc])]))
  | Some(sann_doc) =>
...
let ddoc_of_Case =
  ...
  combos2(space_or_linebreak, space_or_linebreak) |> List.map(scrut_left);
...
let ddoc_of_LetLine =
  ...
  let delim_group_1: ddoc = {
    ...
    let doc =
      switch (ann_doc) {
      | None =>
        DDoc.(Choice([Cat([let_delim, p_doc |> pad_child, eq_delim])]))
      | Some(ann_doc) =>
  ...
  Choice([
    ...
    Box([delim_group_1, Cat([indent, def_doc]), delim_group_2]),
  ]);
};
