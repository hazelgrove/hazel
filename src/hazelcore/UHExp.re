...
let case =
    (~err: CaseErrStatus.t=CaseNotInHole, scrut: t, rules: rules): operand =>
...
let new_InvalidText =
  ...
  let (operand, u_gen, _) = new_InvalidText_and_id(u_gen, t);
  (operand, u_gen);
};
...
let new_EmptyHole = (u_gen: MetaVarGen.t): (operand, MetaVarGen.t) => {
  let (op, u_gen, _) = new_EmptyHole_and_id(u_gen);
  (op, u_gen);
};
...
and set_err_status_operand = (err, operand) =>
  ...
  | Case(_, scrut, rules) => Case(CaseNotInHole, scrut, rules)
...
and is_complete_operand = (operand: 'operand): bool => {
  ...
  | Case(InconsistentBranches(_), _, _) => false
  | Case(CaseNotInHole, body, rules) =>
