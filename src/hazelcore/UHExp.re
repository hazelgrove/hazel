...
/* let rec fix_indices = (ctx: Context.t, e: t): t => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
...
/*     () => */
/*     fix_indices_block(Context.initial, e) */
/*   ) */

/* and fix_indices_block = (ctx: Context.t, block: block): block => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
/*       ("ctx", () => Context.sexp_of_t(ctx)), */
/*       ("block", () => sexp_of_block(block)), */
/*     ], */
/*     ~result_sexp=sexp_of_block, */
/*     () => */
/*     List.fold_left( */
/*       ((ctx, new_block), line) => { */
/*         let (ctx, new_line) = fix_indices_line(ctx, line); */
/*         (ctx, [new_line, ...new_block]); */
/*       }, */
/*       (ctx, []), */
/*       block, */
/*     ) */
...
/* and fix_indices_line = (ctx: Context.t, line: line): (Context.t, line) => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
/*       ("ctx", () => Context.sexp_of_t(ctx)), */
/*       ("line", () => sexp_of_line(line)), */
/*     ], */
/*     ~result_sexp= */
/*       ((ctx, line)) => List([Context.sexp_of_t(ctx), sexp_of_line(line)]), */
/*     () => */
/*       switch (line) { */
/*       | EmptyLine */
/*       | CommentLine(_) => (ctx, line) */
/*       | LetLine(p, e) => */
/*         let (ctx, p) = fix_indices_pat(ctx, p); */
/*         let e = fix_indices(ctx, e); */
/*         (ctx, LetLine(p, e)); */
/*       | ExpLine(opseq) => */
/*         let opseq = fix_indices_opseq(ctx, opseq); */
/*         (ctx, ExpLine(opseq)); */
/*       | TyAliasLine(tp, ty) => */
/*         let (ctx, tp) = fix_indices_tpat(ctx, tp); */
/*         let ty = fix_indices_typ(ctx, ty); */
/*         (ctx, TyAliasLine(tp, ty)); */
/*       }, */
/*   ) */

/* and fix_indices_opseq = (ctx: Context.t, OpSeq(skel, seq): opseq): opseq => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
/*       ("ctx", () => Context.sexp_of_t(ctx)), */
/*       ("opseq", () => sexp_of_opseq(OpSeq(skel, seq))), */
/*     ], */
/*     ~result_sexp=sexp_of_opseq, */
/*     () => { */
/*       let (skel, seq) = fix_indices_skel(ctx, skel, seq); */
/*       OpSeq(skel, seq); */
/*     }, */
/*   ) */

/* and fix_indices_skel = (ctx: Context.t, skel: skel, seq: seq): (skel, seq) => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
/*       ("ctx", () => Context.sexp_of_t(ctx)), */
/*       ("skel", () => sexp_of_skel(skel)), */
/*       ("seq", () => sexp_of_seq(seq)), */
/*     ], */
/*     ~result_sexp= */
/*       ((skel, seq)) => List([sexp_of_skel(skel), sexp_of_seq(seq)]), */
/*     () => */
/*       switch (skel) { */
/*       | Placeholder(n) => */
/*         let en = Seq.nth_operand(n, seq); */
/*         let en = fix_indices_operand(ctx, en); */
/*         let seq = Seq.update_nth_operand(n, en, seq); */
/*         (skel, seq); */
/*       | BinOp(status, op, skel1, skel2) => */
/*         let (skel1, seq) = fix_indices_skel(ctx, skel1, seq); */
/*         let (skel2, seq) = fix_indices_skel(ctx, skel2, seq); */
/*         let skel = Skel.BinOp(status, op, skel1, skel2); */
/*         (skel, seq); */
/*       }, */
/*   ) */

/* and fix_indices_operand = (ctx: Context.t, operand: operand): operand => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
/*       ("ctx", () => Context.sexp_of_t(ctx)), */
/*       ("operand", () => sexp_of_operand(operand)), */
/*     ], */
/*     ~result_sexp=sexp_of_operand, */
/*     () => */
/*     switch (operand) { */
/*     | EmptyHole(_) */
/*     | InvalidText(_) */
...
/*       let e = fix_indices(ctx, e); */
/*       Inj(status, side, e); */
/*     | Case(status, scrut, rules) => */
/*       let scrut = fix_indices(ctx, scrut); */
/*       let rules = fix_indices_rules(ctx, rules); */
/*       Case(status, scrut, rules); */
/*     | Parenthesized(e) => */
/*       let e = fix_indices(ctx, e); */
/*       Parenthesized(e); */
...
/* and fix_indices_rules = (ctx: Context.t, rules: rules): rules => */
/*   Log.fun_call( */
/*     __FUNCTION__, */
/*     ~args=[ */
/*       ("ctx", () => Context.sexp_of_t(ctx)), */
/*       ("rules", () => sexp_of_rules(rules)), */
/*     ], */
/*     ~result_sexp=sexp_of_rules, */
/*     () => */
/*     List.map(fix_indices_rule(ctx), rules) */
/*   ) */

/* and fix_indices_rule = (ctx: Context.t, Rule(p, e): rule): rule => { */
/*   let (ctx, p) = fix_indices_pat(ctx, p); */
/*   let e = fix_indices(ctx, e); */
/*   Rule(p, e); */
/* } */

/* and fix_indices_pat = (ctx: Context.t, p: UHPat.t): (Context.t, UHPat.t) => */
/*   if (UHPat.defines_var(p)) { */
/*     let x = UHPat.defines_var_name(p) |> Option.value(~default="!!"); */
/*     let ctx = Context.add(ctx, x, Hole); */
/*     (ctx, p); */
/*   } else { */
/*     (ctx, p); */
/*   }; */
