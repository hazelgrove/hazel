...
module DHPat = {
  ...
  let rec syn_elab =
    ...
    | Var(NotInHole, InVarHole(Keyword(k), u), _) =>
      Elabs(IntermediateKW(u, 0, k), Hole, ctx, delta)
  ...
  and ana_elab =
    ...
    | Var(NotInHole, InVarHole(Free, _), _) => raise(UHPat.FreeVarInPat)
    | Var(NotInHole, InVarHole(Keyword(k), u), _) =>
      Elabs(IntermediateKW(u, 0, k), ty, ctx, delta)
    | Var(NotInHole, NotInVarHole, x) =>
...
module DHExp = {
  ...
  type t =
    ...
    | NonEmptyHole(ErrStatus.HoleReason.t, MetaVar.t, inst_num, env, t)
    | IntermediateKW(MetaVar.t, inst_num, env, IntermediateKeyword.t)
  ...
  let constructor_string = (d: t): string =>
    ...
    | IntermediateKW(_, _, _, _) => "Keyword"
  ...
  let rec renumber_result_only_pat =
    ...
    | IntermediateKW(u, _, k) =>
      /* TODO: see above */
      let sigma = Environment.empty;
      let (i, hii) = HoleInstanceInfo.next(hii, u, sigma, path);
      (IntermediateKW(u, i, k), hii);
    | Inj(side, dp1) =>
      let (dp1, hii) = renumber_result_only_pat(path, hii, dp1);
      (Inj(side, dp1), hii);
    | Pair(dp1, dp2) =>
