...
and place_before_operand =
  fun
  | (Hole | Unit | Int | Bool | Parenthesized(_) | List(_)) as operand =>
...
and move_cursor_left_zoperand =
  ...
  | CursorT(OnDelim(_, Before), Hole | Unit | Int | Bool) => None
...
and move_cursor_right_zoperand =
  ...
  | CursorT(OnDelim(_, After), Hole | Unit | Int | Bool) => None
