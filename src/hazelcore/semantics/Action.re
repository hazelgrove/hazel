...
module Exp = {
  ...
  let mk_ana_text =
    ...
    | Some(
        IntLit(_) | FloatLit(_) | BoolLit(_) | Underscore | Var(_) |
        LivelitName(_),
      ) =>
  ...
  and syn_perform_operand =
    ...
    let ret_with_ty = f => {
      ...
      let ty_opt =
        zoperand' |> ZExp.erase_zoperand |> Statics.Exp.syn_operand(ctx);
      switch (ty_opt) {
      | None => Outcome.Failed
      | Some(ty) => f(ty)
      };
    ...
    | (
        ...
        when zoperand |> ZExp.is_after_zoperand =>
      let (zhole, u_gen) = u_gen |> ZExp.new_EmptyHole;
      syn_perform_operand'(ctx, keyword_action(k), (zhole, u_gen));
    ...
    | (_, ParenthesizedZ(zbody)) =>
      ret_with_ty(ty =>
        ...
        | Failed => Failed
        | CursorEscaped(side) =>
          syn_perform_operand'(ctx, escape(side), (zoperand, u_gen))
