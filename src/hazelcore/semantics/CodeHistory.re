...
let empty: t = {actions: [], action_count: 0, undo_count: 0, snapshots: []} /* Add a new action to the history, dropping any existing * history which has been undone by the user. */;
...
let construct_code = (_history: t): ZExp.t => {
  ...
  ZExp.place_before_exp(
    NumLit(NotInHole, 7) /* Undoes the last performed action, and returns the corresponding expression. */,
  );
};
...
let undo = (history: t): (t, option(ZExp.t)) =>
  ...
  if (history.undo_count < history.action_count) {
    ...
    (
      history,
      None /* Redoes the next action, and returns the corresponding expression. */,
    );
  };
