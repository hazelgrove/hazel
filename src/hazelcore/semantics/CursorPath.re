    ...
    let rec go: Skel.t(_) => zhole_list = (
      ...
      | BinOp(err, op, skel1, skel2) => {
          ...
          if (n == preceding_operand_index) {
            ...
            let binop_holes =
              binop_hole |> Opt.map_default(~default=[], hole => [hole]);
            if (n < preceding_operand_index) {
...
module Typ = {
  ...
  /*
   let rec term_steps_of_z = (zty: ZTyp.t): steps = term_steps_of_zopseq(zty)
   and term_steps_of_zopseq =
     fun
     | CursorT(_, _) => []
     | ListZ(zbody)
     | ParenthesizedZ(zbody) => [0, ...term_steps_of_z(zbody)]
    ...
    | [x, ...xs] =>
      ...
      | Parenthesized(body) =>
        ...
        | 0 =>
          body
          |> follow((xs, cursor))
          |> Opt.map(zbody => ZTyp.ParenthesizedZ(zbody))
        | _ => None
        }
      | List(body) =>
        switch (x) {
        | 0 =>
          body |> follow((xs, cursor)) |> Opt.map(zbody => ZTyp.ListZ(zbody))
        | _ => None
        }
      }
    }
    ...
    | [x, ...xs] =>
      ...
      | List(body) =>
        ...
        | 0 =>
          body
          |> follow_steps(~side, xs)
          |> Opt.map(zbody => ZTyp.ListZ(zbody))
        | _ => None
        }
      }
    }
  and follow_steps_operator =
    ...
    | [] =>
      switch (side) {
      | Before => Some(operator |> ZTyp.place_before_operator)
      | After => Some(operator |> ZTyp.place_after_operator)
      }
    };
...
module Pat = {
    ...
    | [x, ...xs] =>
      ...
      | Parenthesized(body) =>
        ...
        | 0 =>
          body
          |> follow((xs, cursor))
          |> Opt.map(zbody => ZPat.ParenthesizedZ(zbody))
        | _ => None
        }
      | Inj(err, side, body) =>
        switch (x) {
        | 0 =>
          body
          |> follow((xs, cursor))
          |> Opt.map(zbody => ZPat.InjZ(err, side, zbody))
        | _ => None
        }
      }
    }
    ...
    | [x, ...xs] =>
      ...
      | Inj(err, inj_side, body) =>
        ...
        | 0 =>
          body
          |> follow_steps(~side, xs)
          |> Opt.map(zbody => ZPat.InjZ(err, inj_side, zbody))
        | _ => None
        }
      }
    }
  and follow_steps_operator =
    ...
    | [] =>
      switch (side) {
      | Before => Some(operator |> ZPat.place_before_operator)
      | After => Some(operator |> ZPat.place_after_operator)
      }
    | [_, ..._] => None
    };
...
module Exp = {
  ...
  and follow_line =
    ...
    | ([x, ...xs], LetLine(p, ann, def)) =>
      ...
      | 0 =>
        ...
        |> Opt.map(zp => ZExp.LetLineZP(zp, ann, def))
      | 1 =>
        switch (ann) {
        | None => None
        | Some(ann) =>
          ...
          |> Opt.map(zann => ZExp.LetLineZA(p, zann, def))
        }
      | 2 =>
        def
        |> follow((xs, cursor))
        |> Opt.map(zdef => ZExp.LetLineZE(p, ann, zdef))
      | _ => None
      }
    }
  and follow_operator =
    ...
    | [x, ...xs] =>
      ...
      | Parenthesized(body) =>
        ...
        | 0 =>
          body
          |> follow((xs, cursor))
          |> Opt.map(zbody => ZExp.ParenthesizedZ(zbody))
        | _ => None
        }
      | Lam(err, p, ann, body) =>
        switch (x) {
        | 0 =>
          p
          |> Pat.follow((xs, cursor))
          |> Opt.map(zp => ZExp.LamZP(err, zp, ann, body))
        | 1 =>
          switch (ann) {
          | None => None
          | Some(ann) =>
            ann
            |> Typ.follow((xs, cursor))
            |> Opt.map(zann => ZExp.LamZA(err, p, zann, body))
          }
        | 2 =>
          body
          |> follow((xs, cursor))
          |> Opt.map(zbody => ZExp.LamZE(err, p, ann, zbody))
        | _ => None
        }
      | Inj(err, side, body) =>
        switch (x) {
        | 0 =>
          body
          |> follow((xs, cursor))
          |> Opt.map(zbody => ZExp.InjZ(err, side, zbody))
        | _ => None
        }
      | Case(err, scrut, rules, ann) =>
        ...
        | 0 =>
          scrut
          |> follow((xs, cursor))
          |> Opt.map(zscrut => ZExp.CaseZE(err, zscrut, rules, ann))
        | _ when x == List.length(rules) + 1 =>
          ...
          | Some(ann) =>
            ann
            |> Typ.follow((xs, cursor))
            |> Opt.map(zann => ZExp.CaseZA(err, scrut, rules, zann))
          }
        | _ =>
          ...
          | Some(split_rules) =>
            ...
            |> Opt.map(zrules => ZExp.CaseZR(err, scrut, zrules, ann))
          }
        }
  ...
  and follow_rule =
    ...
    | [x, ...xs] =>
      ...
      | 0 =>
        p
        |> Pat.follow((xs, cursor))
        |> Opt.map(zp => ZExp.RuleZP(zp, clause))
      | 1 =>
        clause
        |> follow((xs, cursor))
        |> Opt.map(zclause => ZExp.RuleZE(p, zclause))
      | _ => None
      }
    };
  ...
  and follow_steps_operator =
    ...
    | [] =>
      switch (side) {
      | Before => Some(operator |> ZExp.place_before_operator)
      | After => Some(operator |> ZExp.place_after_operator)
      }
    }
  and follow_steps_operand =
    ...
    | [x, ...xs] =>
      ...
      | Inj(err, inj_side, body) =>
        ...
        | 0 =>
          body
          |> follow_steps(~side, xs)
          |> Opt.map(zbody => ZExp.InjZ(err, inj_side, zbody))
        | _ => None
        }
      | Case(err, scrut, rules, ann) =>
        switch (x) {
...
/*
 let rec term_steps_Pat.of_z: ZPat.t => steps =
   fun
   | CursorP(_, _) => []
   | InjZ(_, _, zbody)
   | ParenthesizedZ(zbody) => [0, ...term_steps_Pat.of_z(zbody)]
   | OpSeqZ(_, ztm, surround) => [
       Seq.surround_prefix_length(surround),
       ...term_steps_Pat.of_z(ztm),
     ];
 ...
 and term_steps_of_zline =
   ...
   | LetLineZP(zp, _, _) => [0, ...term_steps_Pat.of_z(zp)]
   | LetLineZA(_, zann, _) => [1, ...Typ.term_steps_of_z(zann)]
 ...
 and term_steps_of_zexp =
   ...
   | LamZP(_, zp, _, _) => [0, ...term_steps_Pat.of_z(zp)]
   | LamZA(_, _, zann, _) => [1, ...Typ.term_steps_of_z(zann)]
   ...
   | CaseZA(_, _, rules, zann) => [
       1 + List.length(rules),
       ...Typ.term_steps_of_z(zann),
     ]
   | ApPaletteZ(_, _, _, _) => []
 and term_steps_of_zrule =
   fun
   | CursorR(_, _) => []
   | RuleZP(zp, _) => [0, ...term_steps_Pat.of_z(zp)]
