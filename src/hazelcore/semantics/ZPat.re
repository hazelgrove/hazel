...
let valid_cursors_operand: UHPat.operand => list(CursorPosition.t) =
  CursorPosition.(
    ...
    | IntLit(_, n) => text_cursors(IntUtil.num_digits_str(n))
    | FloatLit(_, f) => text_cursors(FloatUtil.num_digits_str(f))
...
and is_after_zoperand =
  ...
  | CursorP(cursor, IntLit(_, n)) =>
    cursor == OnText(IntUtil.num_digits_str(n))
  | CursorP(cursor, FloatLit(_, f)) =>
    cursor == OnText(FloatUtil.num_digits_str(f))
...
and place_after_operand = operand =>
  ...
  | IntLit(_, n) => CursorP(OnText(IntUtil.num_digits_str(n)), operand)
  | FloatLit(_, f) => CursorP(OnText(FloatUtil.num_digits_str(f)), operand)
...
and move_cursor_left_zoperand =
  ...
  | CursorP(
      OnDelim(_, _),
      Var(_, _, _) | BoolLit(_, _) | NumLit(_, _) | FloatLit(_, _),
    ) =>
