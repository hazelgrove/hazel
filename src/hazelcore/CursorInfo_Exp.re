...
let rec extract_cursor_term = (exp: ZExp.t): cursor_term => {
  extract_from_zline(ZList.prj_z(exp), None);
}
and extract_from_zline =
    (zline: ZExp.zline, suffix: option((UHExp.t, int))): cursor_term => {
  switch (zline) {
  | CursorL(cursor_pos, uex_line) => Line(cursor_pos, uex_line, suffix)
...
and extract_from_zexp_operand = (zexp_operand: ZExp.zoperand): cursor_term => {
  ...
  | CaseZR(_, scrut, zrules) => extract_from_zrules(zrules, scrut)
  | ApPaletteZ(_, _, _, _) => failwith("ApPalette is not implemented")
  };
}
and extract_from_zrules = (zrules: ZExp.zrules, scrut: UHExp.t): cursor_term => {
  let zrule = ZList.prj_z(zrules);
  let index = List.length(ZList.prj_prefix(zrules)) + 1;
  extract_from_zrule(zrule, scrut, index);
}
and extract_from_zrule =
    (zrule: ZExp.zrule, scrut: UHExp.t, index: int): cursor_term => {
  switch (zrule) {
  | CursorR(cursor_pos, uex_rule) => Rule(cursor_pos, uex_rule, index, scrut)
  | RuleZP(zpat, _) => CursorInfo_Pat.extract_cursor_term(zpat)
  | RuleZE(_, zexp) => extract_cursor_term(zexp)
  };
}
and extract_from_zexp_opseq =
    (ZOpSeq(skel, zseq) as zopseq: ZExp.zopseq): cursor_term => {
  switch (zseq) {
...
and syn_cursor_info_line =
  ...
  | CursorL(_, line) =>
    let suffix_opt =
      switch (line) {
      | LetLine(_) =>
        Some((suffix, List.nth(steps, List.length(steps) - 1) + 1))
      | _ => None
      };
    Some(
      CursorNotOnDeferredVarPat(
        CursorInfo_common.mk(
          OnLine,
          ctx,
          extract_from_zline(zline, suffix_opt),
        ),
      ),
    );
  | ExpLineZ(ze) =>
  ...
  | ZOperator((_, Comma), _) =>
    ...
    |> Option.map(ty =>
         CursorInfo_common.mk(
           Synthesized(ty),
           ctx,
           extract_from_zexp_opseq(zopseq),
         )
       )
  | _ =>
    ...
    | ZOperator(_) =>
      ...
      |> Option.map(ty =>
           CursorInfo_common.mk(
             Synthesized(ty),
             ctx,
             extract_from_zexp_opseq(ZOpSeq(skel, zseq)),
           )
         )
    };
  } else {
...
and syn_cursor_info_zoperand =
  ...
  let mk = typed => CursorInfo_common.mk(typed, ctx, cursor_term);
  switch (zoperand) {
  | CursorE(_, InvalidText(_)) => Some(mk(SynInvalid))
  | CursorE(_, Var(_, InVarHole(Keyword(k), _), _)) =>
    Some(mk(SynKeyword(k)))
  | CursorE(_, Var(_, InVarHole(Free, _), _)) => Some(mk(SynFree))
  | CursorE(_, Case(InconsistentBranches(rule_types, _), _, _)) =>
    Some(mk(SynInconsistentBranches(rule_types, steps)))
  | CursorE(_, e) =>
...
and ana_cursor_info_zopseq =
    ...
    : option(CursorInfo.t) => {
  let cursor_term = extract_from_zexp_opseq(zopseq);
  let mk = typed => CursorInfo_common.mk(typed, ctx, cursor_term);
  switch (zseq) {
  | ZOperator((_, Comma), _) =>
    // cursor on tuple comma
...
and ana_cursor_info_skel =
    ...
    : option(CursorInfo.t) => {
  let cursor_term = extract_from_zexp_opseq(ZOpSeq(skel, zseq));
  let mk = typed => CursorInfo_common.mk(typed, ctx, cursor_term);
...
and ana_cursor_info_zoperand =
  ...
  | CursorE(_, e) =>
    switch (e) {
    /* in hole */
    | Var(_, InVarHole(Keyword(k), _), _) => Some(mk(AnaKeyword(ty, k)))
    | Var(_, InVarHole(Free, _), _) => Some(mk(AnaFree(ty)))
    | InvalidText(_) => Some(mk(AnaInvalid(ty)))
    ...
    | ApPalette(InHole(TypeInconsistent, _), _, _, _) =>
      ...
      | None => None
      | Some(ty') => Some(mk(AnaTypeInconsistent(ty, ty')))
      };
    ...
    | Lam(NotInHole, p, body) =>
      ...
      mk(AnaAnnotatedLambda(ty, Arrow(ty_p, ty_body)));
    /* zipper cases */
    }
  ...
  | CursorR(_) =>
    Some(
      CursorInfo_common.mk(
        OnRule,
        ctx,
        extract_from_zrule(zrule, scrut, rule_index),
      ),
    )
  | RuleZP(zp, clause) =>
    switch (
  ...
  | RuleZE(p, zclause) =>
    ...
    | Some(ctx) =>
      ...
      | (true, Some({typed, ctx, uses, _})) =>
        ...
        switch (uses) {
        | None => Some(CursorInfo_common.mk(typed, ctx, cursor_term))
        | Some(uses) =>
          Some(CursorInfo_common.mk(~uses, typed, ctx, cursor_term))
        };
      };
    }
  }
