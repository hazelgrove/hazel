...
and ana_inj_arg =
    (ctx: Contexts.t, arg_opt: option(UHExp.t), ty_opt: option(HTyp.t))
    : option(unit) =>
...
and syn_fix_holes_operand =
  ...
  | Inj(_, tag, arg_opt) =>
    ...
    let (arg_opt', ty_opt, id_gen) =
      switch (arg_opt) {
      | None => (arg_opt, None, id_gen)
      | Some(arg) =>
        let (arg', ty_arg, id_gen) =
          syn_fix_holes(ctx, id_gen, ~renumber_empty_holes, arg);
        (Some(arg'), Some(ty_arg), id_gen);
      };
    (Inj(NotInHole, tag', arg_opt'), Sum(Elided(tag', ty_opt)), id_gen);
  | Match(_, scrut, rules) =>
    ...
    | Some(common_type) =>
      ...
      (Match(match_err, scrut, new_rules), common_type, id_gen);
    };
  };
}
...
and ana_fix_holes_operand =
  ...
  | Inj(_, tag, arg_opt) =>
    let fix_holes_inj_arg =
        (id_gen: IDGen.t, ty_opt: option(HTyp.t))
        : (option(UHExp.t), IDGen.t) => {
    ...
    // AInjHole
    | Hole =>
      let (arg_opt', id_gen) = fix_holes_inj_arg(id_gen, None);
      (Inj(NotInHole, tag', arg_opt'), id_gen);
    | Sum(_) =>
      ...
      | Some(tymap) =>
        ...
        | Some(ty_opt) =>
          ...
          | (Some(_), Some(_))
          | (None, None) =>
            let (arg_opt', id_gen) = fix_holes_inj_arg(id_gen, ty_opt);
            (Inj(NotInHole, tag', arg_opt'), id_gen);
          // AInjUnexpectedArg
          | (Some(_), None) =>
            let (arg_opt', id_gen) = fix_holes_inj_arg(id_gen, None);
            let (u, id_gen) = IDGen.next_hole(id_gen);
            (Inj(InHole(UnexpectedArg, u), tag', arg_opt'), id_gen);
          // AInjExpectedArg
          | (None, Some(_)) =>
            let (u, id_gen) = IDGen.next_hole(id_gen);
            (Inj(InHole(ExpectedArg, u), tag', arg_opt), id_gen);
          }
        }
      | None =>
        let (u, id_gen) = IDGen.next_hole(id_gen);
  ...
  | Match(_, scrut, rules) =>
    ...
    (Match(match_err, scrut, new_rules), id_gen);
  }
