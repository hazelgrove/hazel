      ...
      let correct_rule_types =
        List.for_all2(
          (rule_ty, rule) => {
            ...
            | None => false
            | Some(syn_ty) => HTypUtil.eq(ctx, rule_ty, syn_ty)
            }
          },
          rule_types,
          rules,
...
and ana_skel =
  ...
  | BinOp(
    ...
    | None => None
    | Some(ty') => HTypUtil.consistent(ctx, ty, ty') ? Some() : None
    }
  }
and ana_operand =
...
and syn_fix_holes_line =
  ...
  | DefineLine(tp, uty) =>
    ...
    | TyVar(_, id) =>
      ...
      | Some(_) => (
          DefineLine(tp, uty),
          Contexts.extend_tyvarctx(ctx, (id, Kind.Singleton(ty))),
          u_gen,
        )
      | None => (DefineLine(tp, uty), ctx, u_gen)
      };
    };
  }
...
and ana_fix_holes_operand =
  ...
  | Lam(_, p, ann, def) =>
    ...
    | Some((ty1_given, ty2)) =>
      ...
      | Some(uty1) =>
        let ty1_ann = UHTyp.expand(Contexts.tyvars(ctx), uty1);
        if (HTypUtil.consistent(ctx, ty1_ann, ty1_given)) {
          let (p, ctx, u_gen) =
