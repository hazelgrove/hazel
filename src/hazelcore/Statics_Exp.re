...
let recursive_let_id =
  ...
  | (
    ...
    | None => None
    | Some(ty_p) => Option.map(_ => x, HTyp.matched_arrow(ty_p))
    }
  | _ => None
  };
};
...
let extend_let_def_ctx =
  ...
  | Some(id) =>
    switch (Statics_Pat.syn_moded(ctx, p)) {
    | None => ctx
    | Some(ty_p) => Contexts.extend_gamma(ctx, (id, ty_p))
    }
  };
};
...
and syn_fix_holes_operand =
  ...
  | InvalidText(_) => (e, Unknown(Internal), id_gen)
  ...
  | ListNil(_) => (e_nih, List(Unknown(Internal)), id_gen)
  | Parenthesized(body) =>
    let (block, ty, id_gen) =
...
and ana_fix_holes_block =
  ...
  | Unknown(ModeSwitch) =>
    let (block, _, id_gen) =
      syn_fix_holes_block(ctx, id_gen, ~renumber_empty_holes, block);
    (block, id_gen);
  | _ => ana_fix_holes_block'(ctx, id_gen, ~renumber_empty_holes, block, ty)
  }
and ana_fix_holes_block' =
    (
      ctx: Contexts.t,
...
and ana_fix_holes_opseq =
  ...
  | Unknown(ModeSwitch) =>
    ...
    (opseq, id_gen);
  | _ => ana_fix_holes_opseq'(ctx, id_gen, ~renumber_empty_holes, opseq, ty)
  }
and ana_fix_holes_opseq' =
    (
      ctx: Contexts.t,
...
and ana_fix_holes_operand =
  ...
  switch (ty) {
  | Unknown(ModeSwitch) =>
    let (operand, _, id_gen) =
      syn_fix_holes_operand(ctx, id_gen, ~renumber_empty_holes, e);
    (operand, id_gen);
  | _ => ana_fix_holes_operand'(ctx, id_gen, ~renumber_empty_holes, e, ty)
  }
and ana_fix_holes_operand' =
    (
      ctx: Contexts.t,
  ...
  | Case(_, scrut, rules) =>
    let (scrut, ty_scrut, id_gen) =
      syn_fix_holes(ctx, id_gen, ~renumber_empty_holes, scrut);
    let (rules, id_gen) =
      ana_fix_holes_rules(ctx, id_gen, rules, ty_scrut, ty);
