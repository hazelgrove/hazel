...
and syn_operand = (ctx: Contexts.t, operand: UHExp.operand): option(HTyp.t) =>
  ...
  | Deferral(InHole(TypeInconsistent, _))
  ...
  | Deferral(NotInHole) => Some(List(Hole))
...
and syn_fix_holes_operand =
  ...
  | Deferral(_) => (e_nih, List(Hole), u_gen)
  | Parenthesized(body) =>
    let (block, ty, u_gen) =
...
and ana_fix_holes_operand =
  ...
  | Deferral(_) =>
    ...
    | None =>
      let (u, u_gen) = MetaVarGen.next(u_gen);
      (Deferral(InHole(TypeInconsistent, u)), u_gen);
    }
  | Parenthesized(body) =>
    let (body, u_gen) =
