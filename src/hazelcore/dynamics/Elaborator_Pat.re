module ElaborationResult = {
  ...
  let bind =
      (x: t, ~f: ((DHPat.t, HTyp.t, Contexts.t, MetaVarGen.t, Delta.t)) => t)
      : t =>
    switch (x) {
    | DoesNotElaborate => DoesNotElaborate
    | Elaborates(dp, ty, ctx, u_gen, delta) => f((dp, ty, ctx, u_gen, delta))
    };
};
...
let rec syn_elab =
        (ctx: Contexts.t, u_gen: MetaVarGen.t, delta: Delta.t, p: UHPat.t)
        : ElaborationResult.t =>
  syn_elab_opseq(ctx, u_gen, delta, p)
and syn_elab_opseq =
    (
      ctx: Contexts.t,
      u_gen: MetaVarGen.t,
...
and syn_elab_skel =
  ...
  switch (skel) {
  | Placeholder(n) =>
    syn_elab_operand(ctx, u_gen, delta, seq |> Seq.nth_operand(n))
  ...
  | BinOp(InHole(WrongLength as reason, u), Comma as op, skel1, skel2) =>
    ...
    | Elaborates(dp, _, ctx, u_gen, delta) =>
      ...
      let ty = HTyp.Hole;
      Elaborates(NonEmptyHole(reason, u, 0, dp), ty, ctx, u_gen, delta);
    };
  | BinOp(InHole(WrongLength, _), _, _, _) => DoesNotElaborate
  | BinOp(NotInHole, Comma, _, _) =>
    switch (UHPat.get_tuple_elements(skel)) {
    | [skel1, skel2, ...tail] =>
      let%bind (dp1, ty1, ctx, u_gen, delta) =
        syn_elab_skel(ctx, u_gen, delta, skel1, seq);
      let%bind (dp2, ty2, ctx, u_gen, delta) =
        syn_elab_skel(ctx, u_gen, delta, skel2, seq);
      tail
      |> ListUtil.map_with_accumulator_opt(
           ((dp_acc, ctx, u_gen, delta), skel) => {
             syn_elab_skel(ctx, u_gen, delta, skel, seq)
             |> ElaborationResult.to_option
             |> Option.map(((dp, ty, ctx, u_gen, delta)) =>
                  ((DHPat.Pair(dp_acc, dp), ctx, u_gen, delta), ty)
                )
           },
           (DHPat.Pair(dp1, dp2), ctx, u_gen, delta),
         )
      |> Option.map((((dp_acc, ctx, u_gen, delta), tys)) =>
           (dp_acc, HTyp.Prod([ty1, ty2, ...tys]), ctx, u_gen, delta)
         )
  ...
  | BinOp(NotInHole, Space, skel1, skel2) =>
    ...
    | DoesNotElaborate => DoesNotElaborate
    | Elaborates(dp1, _, ctx, u_gen, delta) =>
      switch (syn_elab_skel(ctx, u_gen, delta, skel2, seq)) {
      | DoesNotElaborate => DoesNotElaborate
      | Elaborates(dp2, _, ctx, u_gen, delta) =>
        let dp = DHPat.Ap(dp1, dp2);
        Elaborates(dp, Hole, ctx, u_gen, delta);
      }
    }
  | BinOp(NotInHole, Cons, skel1, skel2) =>
    switch (syn_elab_skel(ctx, u_gen, delta, skel1, seq)) {
    | DoesNotElaborate => DoesNotElaborate
    | Elaborates(dp1, ty1, ctx, u_gen, delta) =>
      let ty = HTyp.List(ty1);
      switch (ana_elab_skel(ctx, u_gen, delta, skel2, seq, ty)) {
      | DoesNotElaborate => DoesNotElaborate
      | Elaborates(dp2, _, ctx, u_gen, delta) =>
        let dp = DHPat.Cons(dp1, dp2);
        Elaborates(dp, ty, ctx, u_gen, delta);
      };
    }
  }
and syn_elab_operand =
  ...
  | Inj(InHole(TypeInconsistent as reason, u), _, _) =>
    ...
    | Elaborates(dp, _, ctx, u_gen, delta) =>
      ...
      let delta =
        MetaVarMap.add(u, Delta.Hole.Pattern(HTyp.Hole, gamma), delta);
      Elaborates(NonEmptyHole(reason, u, 0, dp), Hole, ctx, u_gen, delta);
    };
  ...
  | InvalidText(u, t) =>
    ...
    let delta = MetaVarMap.add(u, Delta.Hole.Pattern(ty, gamma), delta);
    Elaborates(dp, ty, ctx, u_gen, delta);
  | Wild(NotInHole) => Elaborates(Wild, Hole, ctx, u_gen, delta)
  | Var(NotInHole, InVarHole(Free, _), _) => raise(UHPat.FreeVarInPat)
  | Var(NotInHole, InVarHole(Keyword(k), u), _) =>
    Elaborates(Keyword(u, 0, k), Hole, ctx, u_gen, delta)
  | Var(NotInHole, NotInVarHole, x) =>
    let ctx = Contexts.extend_gamma(ctx, (x, Hole));
    Elaborates(Var(x), Hole, ctx, u_gen, delta);
  | IntLit(NotInHole, n) =>
  ...
  | ListNil(NotInHole) => Elaborates(ListNil, List(Hole), ctx, u_gen, delta)
  | Parenthesized(p1) => syn_elab(ctx, u_gen, delta, p1)
  | Inj(NotInHole, side, p) =>
    switch (syn_elab(ctx, u_gen, delta, p)) {
    | DoesNotElaborate => DoesNotElaborate
    | Elaborates(dp1, ty1, ctx, u_gen, delta) =>
  ...
  | TypeAnn(_, p1, ty1) =>
    switch (Elaborator_Typ.syn(ctx, u_gen, delta, ty1)) {
    | None => DoesNotElaborate
    | Some((ty1, _, _, _, _)) => ana_elab_operand(ctx, u_gen, delta, p1, ty1)
    }
  }
and ana_elab =
    (
    ...
    : ElaborationResult.t =>
  ana_elab_opseq(ctx, u_gen, delta, p, ty)
and ana_elab_opseq =
    (
      ctx: Contexts.t,
      u_gen: MetaVarGen.t,
  ...
  | Some(skel_tys) =>
    ...
    |> List.fold_left(
         (
           acc: option((list(DHPat.t), Contexts.t, MetaVarGen.t, Delta.t)),
           (skel: UHPat.skel, ty: HTyp.t),
         ) =>
           switch (acc) {
           | None => None
           | Some((rev_dps, ctx, u_gen, delta)) =>
             switch (ana_elab_skel(ctx, u_gen, delta, skel, seq, ty)) {
             | DoesNotElaborate => None
             | Elaborates(dp, _, ctx, u_gen, delta) =>
               Some(([dp, ...rev_dps], ctx, u_gen, delta))
             }
           },
...
and ana_elab_skel =
  ...
  | Placeholder(n) =>
    ...
    ana_elab_operand(ctx, u_gen, delta, pn, ty);
  | BinOp(InHole(TypeInconsistent as reason, u), op, skel1, skel2) =>
    let skel_not_in_hole = Skel.BinOp(NotInHole, op, skel1, skel2);
    switch (syn_elab_skel(ctx, u_gen, delta, skel_not_in_hole, seq)) {
    | DoesNotElaborate => DoesNotElaborate
  ...
  | BinOp(NotInHole, Space, skel1, skel2) =>
    switch (ana_elab_skel(ctx, u_gen, delta, skel1, seq, Hole)) {
    | DoesNotElaborate => DoesNotElaborate
    | Elaborates(dp1, _ty1, ctx, u_gen, delta) =>
      switch (ana_elab_skel(ctx, u_gen, delta, skel2, seq, Hole)) {
      | DoesNotElaborate => DoesNotElaborate
      | Elaborates(dp2, _ty2, ctx, u_gen, delta) =>
        let dp = DHPat.Ap(dp1, dp2);
        Elaborates(dp, Hole, ctx, u_gen, delta);
      }
    }
...
and ana_elab_operand =
  ...
  | Var(NotInHole, InVarHole(Free, _), _) => raise(UHPat.FreeVarInPat)
  | Var(NotInHole, InVarHole(Keyword(k), u), _) =>
    Elaborates(Keyword(u, 0, k), ty, ctx, u_gen, delta)
  | Var(NotInHole, NotInVarHole, x) =>
    let ctx = Contexts.extend_gamma(ctx, (x, ty));
    Elaborates(Var(x), ty, ctx, u_gen, delta);
  | Wild(NotInHole) => Elaborates(Wild, ty, ctx, u_gen, delta)
  | InvalidText(_, _)
  | IntLit(NotInHole, _)
  | FloatLit(NotInHole, _)
  | BoolLit(NotInHole, _) => syn_elab_operand(ctx, u_gen, delta, operand)
  | ListNil(NotInHole) =>
    switch (HTyp.matched_list(ty)) {
    | None => DoesNotElaborate
    | Some(ty_elt) =>
      Elaborates(ListNil, HTyp.List(ty_elt), ctx, u_gen, delta)
    }
  | Parenthesized(p) => ana_elab(ctx, u_gen, delta, p, ty)
  | Inj(NotInHole, side, p1) =>
    switch (HTyp.matched_sum(ty)) {
    | None => DoesNotElaborate
    | Some((tyL, tyR)) =>
      let ty1 = InjSide.pick(side, tyL, tyR);
      switch (ana_elab(ctx, u_gen, delta, p1, ty1)) {
      | DoesNotElaborate => DoesNotElaborate
      | Elaborates(dp1, ty1, ctx, u_gen, delta) =>
        let ty =
          switch (side) {
        ...
        Elaborates(Inj(side, dp1), ty, ctx, u_gen, delta);
      };
    }
  | TypeAnn(NotInHole, op, _) => ana_elab_operand(ctx, u_gen, delta, op, ty)
  };
