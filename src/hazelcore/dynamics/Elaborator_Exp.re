...
and syn_elab_skel =
  ...
  | BinOp(InHole(WrongLength as reason, u), Comma as op, skel1, skel2) =>
    ...
    | Elaborates(d, _, delta) =>
      ...
      Elaborates(NonEmptyHole(reason, u, 0, d), Hole, delta);
    };
...
and syn_elab_operand =
  ...
  | Case(InconsistentBranches(rule_types, u), scrut, rules) =>
    ...
    | Elaborates(d1, pat_ty, delta) =>
      ...
      | Some((drs, delta)) =>
        ...
        Elaborates(InconsistentBranches(u, 0, d), Hole, delta);
      };
    } /* not in hole */
  | EmptyHole(u) =>
    let gamma = Contexts.gamma(ctx);
    let d = DHExp.EmptyHole(u, 0);
    let ty = HTyp.Hole;
    let delta = MetaVarMap.add(u, (Delta.ExpressionHole, ty, gamma), delta);
    Elaborates(d, ty, delta);
  | InvalidText(u, t) =>
    let gamma = Contexts.gamma(ctx);
    let d = DHExp.InvalidText(u, 0, t);
    let ty = HTyp.Hole;
    let delta = MetaVarMap.add(u, (Delta.ExpressionHole, ty, gamma), delta);
    Elaborates(d, ty, delta);
  ...
  | Var(NotInHole, InVarHole(reason, u), x) =>
    ...
    let d =
      switch (reason) {
      | Free => DHExp.FreeVar(u, 0, x)
      | ExpandingKeyword(k) => DHExp.ExpandingKeyword(u, 0, k)
      };
...
and ana_elab_skel =
  ...
  | BinOp(InHole(TypeInconsistent as reason, u), op, skel1, skel2) =>
    ...
    | Elaborates(d1, _, delta) =>
      ...
      let delta =
        MetaVarMap.add(u, (Delta.ExpressionHole, ty, gamma), delta);
      let d = DHExp.NonEmptyHole(reason, u, 0, d1);
      Elaborates(d, Hole, delta);
    };
  | BinOp(NotInHole, Cons, skel1, skel2) =>
...
and ana_elab_operand =
  ...
  | Case(StandardErrStatus(InHole(TypeInconsistent as reason, u)), _, _) =>
    ...
    | Elaborates(d, _, delta) =>
      ...
      let delta =
        MetaVarMap.add(u, (Delta.ExpressionHole, ty, gamma), delta);
      Elaborates(NonEmptyHole(reason, u, 0, d), ty, delta);
    };
  ...
  | Var(NotInHole, InVarHole(reason, u), x) =>
    ...
    let d: DHExp.t =
      switch (reason) {
      | Free => FreeVar(u, 0, x)
      | ExpandingKeyword(k) => ExpandingKeyword(u, 0, k)
      };
    Elaborates(d, ty, delta);
