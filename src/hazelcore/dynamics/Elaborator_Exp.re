...
and syn_elab_operand =
  ...
  | Var(NotInHole, InVarHole(reason, u), x) =>
    ...
    let d =
      ...
      | Keyword(k) => DHExp.ExpandingKeyword(u, 0, sigma, k)
      };
  ...
  | StringLit(NotInHole, s) =>
    let parsed = s |> UnescapedStringParser.from_string;
    Elaborates(StringLit(parsed), String, delta);
  | ListNil(NotInHole) =>
...
and ana_elab_operand =
  ...
  | Var(NotInHole, InVarHole(reason, u), x) =>
    ...
    let d: DHExp.t =
      ...
      | Keyword(k) => ExpandingKeyword(u, 0, sigma, k)
      };
    Elaborates(d, ty, delta);
