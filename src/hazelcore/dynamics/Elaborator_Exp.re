...
module M = (S: Statics_Exp_Sig.S) : SElab => {
  ...
  let mk_statics_ctx = ctx => (
    Contexts.gamma(ctx),
    VarMap.map(
      ((_, (def, dtys))) =>
        (def, List.map(((x, (_, ty))) => (x, ty), dtys)),
  ...
  and syn_elab_skel =
    ...
    | BinOp(InHole(WrongLength as reason, u), Comma as op, skel1, skel2) =>
      ...
      switch (
        syn_elab_skel(~livelit_holes, ctx, delta, skel_not_in_hole, seq)
      ) {
      | DoesNotElaborate => DoesNotElaborate
    ...
    | BinOp(NotInHole, Space, skel1, skel2) =>
      ...
      | _ =>
        let ctx' = ctx |> map_livelit_ctx(((_, ty)) => ty);
        switch (S.syn_skel(ctx', skel1, seq)) {
        | None => DoesNotElaborate
        | Some(ty1) =>
  ...
  and syn_elab_operand =
    ...
    | Lam(InHole(TypeInconsistent(_) as reason, u), _, _)
  ...
  and syn_elab_rules =
    ...
    let ctx' = ctx |> map_livelit_ctx(((_, ty)) => ty);
    switch (S.syn_rules(ctx', rules, pat_ty)) {
    | None => None
    | Some(glb) =>
      let elabed_rule_info =
        List.fold_left(
          (b, r) =>
            ...
            | Some((drs, delta)) =>
              switch (
                syn_elab_rule(~livelit_holes, ctx, delta, r, pat_ty, glb)
              ) {
              | None => None
              | Some((dr, delta)) =>
                let drs = drs @ [dr];
      ...
      |> List.fold_left(
           (
             ...
             | Some((rev_ds, rev_tys, delta)) =>
               switch (
                 ana_elab_skel(~livelit_holes, ctx, delta, skel, seq, ty)
               ) {
               | DoesNotElaborate => None
               | Elaborates(d, ty, delta) =>
