  ...
  | LetLine(p, ann, def) =>
    ...
    | Some(uty1) =>
      ...
      | Expands(d1, ty1', delta) =>
        ...
        switch (Dynamics_Pat.ana_expand(ctx, delta, p, ty1)) {
        | Dynamics_Pat.ExpandResult.DoesNotExpand => LinesDoNotExpand
...
and syn_expand_operand =
    (ctx: Contexts.t, delta: Delta.t, operand: UHExp.operand): ExpandResult.t =>
  switch (operand) {
  /* in hole */
  ...
  | Lam(NotInHole, p, ann, body) =>
    ...
    switch (Dynamics_Pat.ana_expand(ctx, delta, p, ty1)) {
    | Dynamics_Pat.ExpandResult.DoesNotExpand => ExpandResult.DoesNotExpand
    | Expands(dp, _, ctx, delta) =>
...
and syn_expand_rule =
  ...
  switch (Dynamics_Pat.ana_expand(ctx, delta, p, pat_ty)) {
  | DoesNotExpand => None
  | Expands(dp, _, ctx, delta) =>
...
and ana_expand =
    (ctx: Contexts.t, delta: Delta.t, e: UHExp.t, ty: HTyp.t): ExpandResult.t =>
  ...
  | Lam(NotInHole, p, ann, body) =>
    ...
    | Some((ty1_given, ty2)) =>
      ...
      | Some(uty1) =>
        ...
        | true =>
          switch (Dynamics_Pat.ana_expand(ctx, delta, p, ty1_ann)) {
          | Dynamics_Pat.ExpandResult.DoesNotExpand => ExpandResult.DoesNotExpand
      ...
      | None =>
        switch (Dynamics_Pat.ana_expand(ctx, delta, p, ty1_given)) {
        | Dynamics_Pat.ExpandResult.DoesNotExpand => ExpandResult.DoesNotExpand
...
and ana_expand_rule =
  ...
  let UHExp.Rule(p, clause) = r;
  switch (Dynamics_Pat.ana_expand(ctx, delta, p, pat_ty)) {
  | Dynamics_Pat.ExpandResult.DoesNotExpand => None
  | Expands(dp, _, ctx, delta) =>
...
and renumber_result_only_rules =
  ...
  |> List.fold_left(
       (b, r: DHExp.rule) => {
         ...
         switch (r) {
         | Rule(dp, d) =>
           let (dp, hii) = Dynamics_Pat.renumber_result_only(path, hii, dp);
           let (d, hii) = renumber_result_only(path, hii, d);
