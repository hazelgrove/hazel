open Sexplib.Std /* hole instance numbers are all 0 after expansion and during evaluation --
...
let rec evaluate = (d: DHExp.t): result =>
  ...
  | Let(dp, d1, d2) =>
    ...
    | BoxedValue(d1)
    | Indet(d1) =>
      switch (Expansion.Exp.matches(dp, d1)) {
      | Indet => Indet(d)
      | DoesNotMatch => Indet(d)
      | Matches(env) => evaluate(Expansion.Exp.subst(env, d2))
      }
    }
  | FixF(x, _, d1) => evaluate(Expansion.Exp.subst_var(d, x, d1))
  ...
  | Ap(d1, d2) =>
    ...
    | BoxedValue(Lam(dp, _, d3)) =>
      ...
      | BoxedValue(d2)
      | Indet(d2) =>
        switch (Expansion.Exp.matches(dp, d2)) {
        | DoesNotMatch => Indet(d)
        | Indet => Indet(d)
        | Matches(env) =>
          /* beta rule */
          evaluate(Expansion.Exp.subst(env, d3))
        }
      }
...
and evaluate_case =
  ...
  | Indet(scrut) =>
    ...
    | Some(Rule(dp, d)) =>
      switch (Expansion.Exp.matches(dp, scrut)) {
      | Indet =>
        let case = DHExp.Case(scrut, rules, current_rule_index);
        switch (inconsistent_info) {
        | None => Indet(ConsistentCase(case))
      ...
      | Matches(env) => evaluate(Expansion.Exp.subst(env, d))
      | DoesNotMatch =>
