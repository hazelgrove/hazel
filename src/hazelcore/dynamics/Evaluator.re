...
let rec ground_cases_of = (tyvars: TyVarCtx.t, ty: HTyp.t): ground_cases =>
  switch (ty) {
  | TyVarHole(_, _)
  | Hole => Hole
  | Bool
  ...
  | List(Hole) => Ground
  | TyVar(idx, _) =>
    switch (TyVarCtx.tyvar_with_idx(tyvars, idx)) {
    | (_, Singleton(_, ty2)) => ground_cases_of(tyvars, ty2)
    | (_, KHole) => Hole
    | (_, Type) => Ground
    }
  | Prod(tys) =>
    if (List.for_all(HTyp.equiv(HTyp.Hole), tys)) {
      Ground;
    } else {
...
and matches_cast_Cons =
  ...
  | Cast(_, _, _) => DoesNotMatch
  // TODO: (eric) are type alias lines unmatchable?
  | TyAlias(_) => DoesNotMatch
  | BoundVar(_) => DoesNotMatch
  | FreeVar(_, _, _, _) => Indet
  | InvalidText(_) => Indet
