...
module Evaluator = {
  ...
  let rec evaluate = (d: DHExp.t): result =>
    ...
    | Let(dp, d1, d2) =>
      ...
      | BoxedValue(d1)
      | Indet(d1) =>
        switch (Dynamics_Exp.matches(dp, d1)) {
        | Indet => Indet(d)
        | DoesNotMatch => Indet(d)
        | Matches(env) => evaluate(Dynamics_Exp.subst(env, d2))
        }
      }
    | FixF(x, _, d1) => evaluate(Dynamics_Exp.subst_var(d, x, d1))
    ...
    | Ap(d1, d2) =>
      ...
      | BoxedValue(Lam(dp, _, d3)) =>
        ...
        | BoxedValue(d2)
        | Indet(d2) =>
          switch (Dynamics_Exp.matches(dp, d2)) {
          | DoesNotMatch => Indet(d)
          | Indet => Indet(d)
          | Matches(env) =>
            /* beta rule */
            evaluate(Dynamics_Exp.subst(env, d3))
          }
        }
  ...
  and evaluate_case =
    ...
    | Indet(scrut) =>
      ...
      | Some(Rule(dp, d)) =>
        switch (Dynamics_Exp.matches(dp, scrut)) {
        | Indet =>
          let case = DHExp.Case(scrut, rules, current_rule_index);
          switch (inconsistent_info) {
          | None => Indet(ConsistentCase(case))
        ...
        | Matches(env) => evaluate(Dynamics_Exp.subst(env, d))
        | DoesNotMatch =>
          evaluate_case(
            inconsistent_info,
            scrut,
