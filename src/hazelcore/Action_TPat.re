module TyIdUtil = {
  let insert = (k, s, x) => StringUtil.insert(k, s, x);
  let delete = (k, x) => StringUtil.delete(k, x);
  let backspace = (k, x) => StringUtil.backspace(k, x);
  let is_empty = x => x |> StringUtil.is_empty;
};
