  ...
  | Inj(NotInHole, tag, arg_opt) =>
    let (ctx', ty_opt) =
      ...
      | Some(arg) =>
        switch (syn(ctx, arg)) {
        | None => (ctx, None)
        | Some((ty_arg, ctx')) => (ctx', Some(ty_arg))
        }
      };
...
and ana_operand =
  ...
  | Inj(NotInHole, tag, arg_opt) =>
    ...
    | Sum(_) =>
      ...
      // AInjTagErr
      | None =>
        ana_inj_arg(ctx, arg_opt, arg_opt |> Option.map(_ => HTyp.Hole))
      };
    | _ => None
    }
...
and inj_arg_valid =
  ...
  switch (arg_opt) {
  | None => Some(ctx)
  | Some(arg) => ana(ctx, arg, HTyp.Hole)
  }
and ana_inj_arg =
    (ctx: Contexts.t, arg_opt: option(UHPat.t), ty_opt: option(HTyp.t))
    : option(Contexts.t) =>
  switch (arg_opt, ty_opt) {
  | (Some(arg), Some(ty)) => ana(ctx, arg, ty)
  | (None, None) => Some(ctx)
  | (_, _) => None
  };
...
and syn_fix_holes_operand =
  ...
  | Inj(_, tag, arg_opt) =>
    let (tag', id_gen) =
      Statics_Tag.syn_fix_holes(ctx, id_gen, ~renumber_empty_holes, tag);
    let (arg_opt', ty_opt, ctx, id_gen) =
      switch (arg_opt) {
      | None => (arg_opt, None, ctx, id_gen)
      | Some(arg) =>
        let (arg', ty_arg, ctx, id_gen) =
          syn_fix_holes(ctx, id_gen, ~renumber_empty_holes, arg);
        (Some(arg'), Some(ty_arg), ctx, id_gen);
      };
    let ty = HTyp.Sum(Elided(tag', ty_opt));
    (Inj(NotInHole, tag', arg_opt'), ty, ctx, id_gen);
  | TypeAnn(_, op, ann) =>
...
and ana_fix_holes_operand =
  ...
  | Inj(_, tag, arg_opt) =>
    let fix_holes_inj_arg =
        (ctx: Contexts.t, id_gen: IDGen.t, ty_opt: option(HTyp.t))
        : (option(UHPat.t), Contexts.t, IDGen.t) => {
      let ty_arg = ty_opt |> Option.value(~default=HTyp.Hole);
      switch (arg_opt) {
      | None => (None, ctx, id_gen)
      | Some(arg) =>
        let (arg', ctx', id_gen) =
          ana_fix_holes(~renumber_empty_holes, ctx, id_gen, arg, ty_arg);
        (Some(arg'), ctx', id_gen);
      };
    };
    let (tag', id_gen) =
      Statics_Tag.ana_fix_holes(~renumber_empty_holes, ctx, id_gen, tag, ty);
    switch (ty) {
    // AInjHole
    | Hole =>
      let (arg_opt', ctx, id_gen) = fix_holes_inj_arg(ctx, id_gen, None);
      (Inj(NotInHole, tag', arg_opt'), ctx, id_gen);
    | Sum(_) =>
      ...
      | Some(tymap) =>
        ...
        | Some(ty_opt) =>
          ...
          | (Some(_), Some(_))
          | (None, None) =>
            let (arg_opt', ctx, id_gen) =
              fix_holes_inj_arg(ctx, id_gen, ty_opt);
            (Inj(NotInHole, tag', arg_opt'), ctx, id_gen);
          // AInjUnexpectedArg
          | (Some(_), None) =>
            let (arg_opt', ctx, id_gen) =
              fix_holes_inj_arg(ctx, id_gen, None);
            let (u, id_gen) = IDGen.next_hole(id_gen);
            (Inj(InHole(UnexpectedArg, u), tag', arg_opt'), ctx, id_gen);
          // AInjExpectedArg
          | (None, Some(_)) =>
            let (u, id_gen) = IDGen.next_hole(id_gen);
            (Inj(InHole(ExpectedArg, u), tag', arg_opt), ctx, id_gen);
          }
        }
      | None =>
        let (u, id_gen) = IDGen.next_hole(id_gen);
        let (arg_opt', ctx, id_gen) = fix_holes_inj_arg(ctx, id_gen, None);
        let status =
          InjErrStatus.InHole(ExpectedTypeNotConsistentWithSums, u);
        (UHPat.Inj(status, tag, arg_opt'), ctx, id_gen);
      }
    | _ =>
      let (arg_opt', ctx, id_gen) = fix_holes_inj_arg(ctx, id_gen, None);
      let (u, id_gen) = IDGen.next_hole(id_gen);
      let status = InjErrStatus.InHole(ExpectedTypeNotConsistentWithSums, u);
      (Inj(status, tag, arg_opt'), ctx, id_gen);
    };
...
let ana_fix_holes_z =
  ...
  let zp =
    ...
    |> OptUtil.get(() =>
         failwith(
           "P ana_fix_holes did not preserve path "
           ++ Sexplib.Sexp.to_string(CursorPath.sexp_of_t(path)),
         )
       );
  (zp, ctx, id_gen);
