...
and hooks_line =
  ...
  | ExpLine(opseq) => hooks_opseq(rev_steps, opseq, hs)
  }
and hooks_opseq =
    (
      rev_steps: CursorPath.rev_steps,
...
let prev_hook_steps_z = (ze: ZExp.t): option(CursorPath.steps) => {
  let hooks = hooks_z(ze, []);
  CursorPath_common.prev_hook_steps(hooks);
};
let prev_hook_steps_zline = (zline: ZExp.zline): option(CursorPath.steps) => {
  let hooks = hooks_zline(zline, []);
  CursorPath_common.prev_hook_steps(hooks);
};

let next_hook_steps_z = (ze: ZExp.t): option(CursorPath.steps) => {
  let hooks = hooks_z(ze, []);
  CursorPath_common.next_hook_steps(hooks);
};
let next_hook_steps_zline = (zline: ZExp.zline): option(CursorPath.steps) => {
  let hooks = hooks_zline(zline, []);
  CursorPath_common.next_hook_steps(hooks);
};
