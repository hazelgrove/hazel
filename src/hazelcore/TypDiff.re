...
let rec mk =
  ...
  switch (HTyp.unsafe(ty1), HTyp.unsafe(ty2)) {
  | (Hole, _)
  | (_, Hole)
  | (TyVarHole(_), _)
  | (_, TyVarHole(_))
  ...
  | (Sum(ty1, ty2), Sum(ty1', ty2')) =>
    let (steps1, steps1') =
      diff_subtypes(0, (HTyp.of_unsafe(ty1), HTyp.of_unsafe(ty1')));
    let (steps2, steps2') =
      diff_subtypes(1, (HTyp.of_unsafe(ty2), HTyp.of_unsafe(ty2')));
  ...
  | (Prod(tys), Prod(tys')) =>
    if (List.length(tys) != List.length(tys')) {
      ...
      |> List.mapi((i, (ty, ty')) =>
           diff_subtypes(i, (HTyp.of_unsafe(ty), HTyp.of_unsafe(ty')))
         )
      |> List.split
      |> TupleUtil.map2(List.flatten);
    }
  | (List(ty), List(ty')) =>
    diff_subtypes(0, (HTyp.of_unsafe(ty), HTyp.of_unsafe(ty')))
  | (Int, _)
  | (Float, _)
  | (Bool, _)
  | (Arrow(_), _)
