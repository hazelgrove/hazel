let mk_tag = (text: string, id_gen: IDGen.t): (UHTag.t, IDGen.t) =>
  UHTag.fix_holes(Tag(NotInTagHole, text), UHTag.Set.empty, id_gen);

let mk_text =
    (caret_index: int, text: string, id_gen: IDGen.t): (ZTag.t, IDGen.t) => {
  let cursor = CursorPosition.OnText(caret_index);
  let (tag, id_gen) = mk_tag(text, id_gen);
  (ZTag.CursorTag(cursor, tag), id_gen);
};

let insert_text =
    ((caret_index: int, insert_text: string), text: string, id_gen: IDGen.t)
    : ActionOutcome.t((ZTag.t, IDGen.t)) => {
  let text_len = String.length(insert_text);
  let new_text = text |> StringUtil.insert(caret_index, insert_text);
  Succeeded(mk_text(caret_index + text_len, new_text, id_gen));
};

let delete_text =
    (caret_index: int, text: string, id_gen: IDGen.t)
    : ActionOutcome.t((ZTag.t, IDGen.t)) =>
  caret_index == String.length(text)
    ? CursorEscaped(After)
    : Succeeded(
        mk_text(caret_index, text |> StringUtil.delete(caret_index), id_gen),
      );
...
let backspace_text =
  ...
  caret_index == 0
    ...
    : {
      ...
      Succeeded(mk_text(caret_index - 1, new_text, id_gen));
    };

let move =
    (id_gen: IDGen.t, a: Action.t, ztag: ZTag.t)
    : ActionOutcome.t((ZTag.t, IDGen.t)) =>
  switch (a) {
  | MoveTo(path) =>
    switch (CursorPath_Tag.follow(path, ZTag.erase(ztag))) {
    | None => Failed
    | Some(ztag) => Succeeded((ztag, id_gen))
    }
...
let perform =
  ...
  | (
      Construct(
        SAnn | SLet | SLine | SFun | SList | SListNil | SInj | SMatch |
        SParenthesized |
        SCloseParens |
        SCloseBraces |
        SCloseSquareBracket |
  ...
  | (Backspace, CursorTag(OnDelim(_0, After), EmptyTagHole(_) as operand)) =>
    ZTag.is_after(ztag)
      ? Succeeded((ZTag.place_before(operand), id_gen))
      : CursorEscaped(Before)

  | (Delete, CursorTag(OnDelim(_0, Before), EmptyTagHole(_) as operand)) =>
    ZTag.is_before(ztag)
      ? Succeeded((ZTag.place_after(operand), id_gen))
      : CursorEscaped(After)
  ...
  | (Backspace, CursorTag(OnText(j), Tag(_, t))) =>
    ...
    | (1, 1) =>
      let (tag_hole, id_gen) = UHTag.new_TagHole(id_gen);
      Succeeded((ZTag.place_before(tag_hole), id_gen));
    | (_, _) => backspace_text(j, t, id_gen)
    }
  ...
  | (Construct(SOp(SSpace)), CursorTag(OnText(j), Tag(_) as tag)) =>
    switch (j) {
    | 0 => Succeeded((ZTag.place_after(tag), id_gen))
