...
and kequiv = (ctx, k1, k2) =>
  ...
  | (Singleton(ty1), Singleton(ty2))
      when kcequiv(ctx, ty1, ty2, Kind.Type) =>
    true
  | (KHole | Type | Singleton(_), _)
  | (_, KHole | Type | Singleton(_)) => false
  }
...
and kcequiv = (ctx, ty1, ty2, k) => {
  ...
  | (TyVar(idx1, _), ty2) =>
    ...
    | (_, KHole | Type) => failwith("impossible for bounded type variables (currently)")
    }
  | (ty1, TyVar(idx2, _)) =>
  ...
  | (Sum(ty1, ty3), Sum(ty2, ty4)) =>
    kcequiv(ctx, ty1, ty3, Kind.Type) && kcequiv(ctx, ty2, ty4, Kind.Type) && kequiv(ctx, k, Kind.Type)
