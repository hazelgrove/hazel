...
module Pat = {
  ...
  and syn_cursor_info_zoperand =
    ...
    | CursorP(_, Var(_, InVarHole(Duplicate, _), _, x)) =>
      Some(
        CursorNotOnDeferred(
          mk(PatSynDuplicate(x), ctx, extract_from_zpat_operand(zoperand)),
        ),
      )
  ...
  and ana_cursor_info_zoperand =
    ...
    | CursorP(_, operand) =>
      ...
      | Var(NotInHole, InVarHole(Keyword(k), _), _, _) =>
        Some(
          CursorNotOnDeferred(mk(PatAnaKeyword(ty, k), ctx, cursor_term)),
        )
      | Var(NotInHole, InVarHole(Duplicate, _), _, x) =>
        Some(
          CursorNotOnDeferred(mk(PatAnaDuplicate(ty, x), ctx, cursor_term)),
        )
      // not in hole
      ...
      | Wild(NotInHole)
      | ListNil(NotInHole) =>
        Some(CursorNotOnDeferred(mk(PatAnalyzed(ty), ctx, cursor_term)))
      | IntLit(NotInHole, _) =>
        Some(
          CursorNotOnDeferred(
      ...
      | Parenthesized(body) =>
        ...
        |> OptUtil.map(_ =>
             CursorNotOnDeferred(mk(PatAnalyzed(ty), ctx, cursor_term))
           )
      }
...
module Exp = {
  ...
  and syn_cursor_info_line =
    ...
    switch (zline) {
    | CursorL(_) =>
      Some(CursorNotOnDeferred(mk(OnLine, ctx, extract_from_zline(zline))))
  ...
  and syn_cursor_info_zopseq =
    ...
    | ZOperator((_, Comma), _) =>
      ...
      |> OptUtil.map(ty =>
           CursorNotOnDeferred(
             mk(Synthesized(ty, None), ctx, extract_from_zexp_zseq(zseq)),
           )
         )
    | _ =>
      // cursor within tuple element
  ...
  and syn_cursor_info_zoperand =
    ...
    | CursorE(_, Var(_, InVarHole(Keyword(k), _), _)) =>
      Some(CursorNotOnDeferred(mk(SynKeyword(k), ctx, cursor_term)))
    | CursorE(_, Var(_, InVarHole(Free, _), _)) =>
      Some(CursorNotOnDeferred(mk(SynFree, ctx, cursor_term)))
    | CursorE(_, Case(InconsistentBranches(rule_types, _), _, _)) =>
      Some(
        CursorNotOnDeferred(
          mk(SynInconsistentBranches(rule_types, steps), ctx, cursor_term),
        ),
      )
    ...
    | CursorE(_, e) =>
      ...
      | Some(ty) =>
        Some(
          CursorNotOnDeferred(mk(Synthesized(ty, None), ctx, cursor_term)),
        )
      }
    | ParenthesizedZ(zbody) =>
  ...
  and ana_cursor_info_zopseq =
    ...
    | ZOperator((_, Comma), _) =>
      ...
      switch (err) {
      | NotInHole =>
        Some(CursorNotOnDeferred(mk(Analyzed(ty, None), ctx, cursor_term)))
      | InHole(WrongLength, _) =>
      ...
      | InHole(TypeInconsistent, _) =>
        ...
        |> Option.map(ty' =>
             CursorNotOnDeferred(
               mk(AnaTypeInconsistent(ty, ty', None), ctx, cursor_term),
             )
           );
      };
    | _ =>
    ...
    if (ZOpSeq.skel_is_rooted_at_cursor(skel, zseq)) {
      ...
      | ZOperator(_) =>
        ...
        | NotInHole =>
          ...
          |> OptUtil.map(_ =>
               CursorNotOnDeferred(mk(Analyzed(ty, None), ctx, cursor_term))
             )
        | InHole(WrongLength, _) =>
          failwith(__LOC__ ++ ": n-tuples handled at opseq level")
        | InHole(TypeInconsistent, _) =>
  ...
  and ana_cursor_info_zoperand =
    ...
    | CursorE(_, e) =>
      ...
      | Var(_, InVarHole(Keyword(k), _), _) =>
        Some(CursorNotOnDeferred(mk(AnaKeyword(ty, k), ctx, cursor_term)))
      | Var(_, InVarHole(Duplicate, _), _) => assert(false)
      | Var(_, InVarHole(Free, _), _) =>
        Some(CursorNotOnDeferred(mk(AnaFree(ty), ctx, cursor_term)))
      ...
      | ApPalette(NotInHole, _, _, _) =>
        ...
        | Some(ty') =>
          Some(
            CursorNotOnDeferred(mk(AnaSubsumed(ty, ty'), ctx, cursor_term)),
          )
        }
      | ListNil(NotInHole)
      | Inj(NotInHole, _, _)
  ...
  and syn_cursor_info_rule =
    ...
    switch (zrule) {
    | CursorR(_) =>
      Some(CursorNotOnDeferred(mk(OnRule, ctx, extract_from_zrule(zrule))))
    | RuleZP(zp, clause) =>
