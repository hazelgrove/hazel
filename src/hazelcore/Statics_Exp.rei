...
let syn_nth_type_mode:
  (Contexts.t, int, UHExp.opseq, MetaVarGen.t) => option(Statics.type_mode);
/**
 * Get type mode of nth operand of an opseq in analytic position
 */
let ana_nth_type_mode:
  (Contexts.t, int, UHExp.opseq, HTyp.t, MetaVarGen.t) =>
...
let syn_block:
  (Contexts.t, UHExp.block, MetaVarGen.t) => option((HTyp.t, MetaVarGen.t));
let syn_lines:
...
let syn_rules:
  (Contexts.t, UHExp.rules, HTyp.t, MetaVarGen.t) =>
  option((HTyp.t, MetaVarGen.t));
let syn_rule:
  (Contexts.t, UHExp.rule, HTyp.t, MetaVarGen.t) =>
  option((HTyp.t, MetaVarGen.t));

/**
 * Under context `ctx`, `ana(ctx, p, ty)` analyzes `e` against `ty`
 */
let ana: (Contexts.t, UHExp.t, HTyp.t, MetaVarGen.t) => option(unit);
let ana_skel:
  (Contexts.t, UHExp.skel, UHExp.seq, HTyp.t, MetaVarGen.t) => option(unit);
let ana_splice_map:
  (Contexts.t, UHExp.splice_map, MetaVarGen.t) => option(Contexts.t);
...
let recursive_let_id:
  (Contexts.t, UHPat.t, UHExp.t, MetaVarGen.t) =>
