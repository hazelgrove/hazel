...
let syn_nth_type_mode:
  (Contexts.t, MetaVarGen.t, int, UHExp.opseq) => option(Statics.type_mode);
/**
 * Get type mode of nth operand of an opseq in analytic position
 */
let ana_nth_type_mode:
  (Contexts.t, MetaVarGen.t, int, UHExp.opseq, HTyp.t) =>
 ...
 * (if possible)
 */
let syn: (Contexts.t, MetaVarGen.t, UHExp.t) => option(HTyp.t);
let syn_block: (Contexts.t, MetaVarGen.t, UHExp.block) => option(HTyp.t);
let syn_lines:
  (Contexts.t, MetaVarGen.t, list(UHExp.line)) => option(Contexts.t);
let syn_opseq: (Contexts.t, MetaVarGen.t, UHExp.opseq) => option(HTyp.t);
let syn_skel:
  (Contexts.t, MetaVarGen.t, UHExp.skel, UHExp.seq) => option(HTyp.t);
let syn_operand: (Contexts.t, MetaVarGen.t, UHExp.operand) => option(HTyp.t);
let syn_rules:
  (Contexts.t, MetaVarGen.t, UHExp.rules, HTyp.t) => option(HTyp.t);
let syn_rule:
  (Contexts.t, MetaVarGen.t, UHExp.rule, HTyp.t) => option(HTyp.t);

/**
 * Under context `ctx`, `ana(ctx, p, ty)` analyzes `e` against `ty`
 */
let ana: (Contexts.t, MetaVarGen.t, UHExp.t, HTyp.t) => option(unit);
let ana_skel:
  (Contexts.t, MetaVarGen.t, UHExp.skel, UHExp.seq, HTyp.t) => option(unit);
let ana_splice_map:
  (Contexts.t, MetaVarGen.t, UHExp.splice_map) => option(Contexts.t);
...
let joined_pattern_type:
  (Contexts.t, MetaVarGen.t, list(UHExp.rule)) => option(HTyp.t);
...
let extend_let_def_ctx:
  (Contexts.t, MetaVarGen.t, UHPat.t, UHExp.t) => (Contexts.t, MetaVarGen.t);
...
let recursive_let_id:
  (Contexts.t, MetaVarGen.t, UHPat.t, UHExp.t) => option(Var.t);
