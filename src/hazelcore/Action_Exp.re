    ...
    () => {
      ...
      | (Backspace, CursorL(OnDelim(k, After), LetLine(p, def))) =>
        if (k == 1) {
          ...
          let new_ze =
            k == 2 ? def |> ZExp.place_after : def |> ZExp.place_before; // changed 3 to 2?
      ...
      | (Construct(SCommentLine), CursorL(_, EmptyLine)) =>
        let new_zblock = (
          [],
          ZExp.CursorL(OnText(0), CommentLine("")),
          [],
        );
        mk_result(u_gen, new_zblock);
      ...
      | (
        ...
        let com_aft =
          String.sub(comment, loca, String.length(comment) - loca);
        let new_zblock = (
      ...
      | (SwapUp | SwapDown, CursorL(_) | LetLineZP(_) | TyAliasLineP(_)) =>
        Failed
      | (SwapLeft, CursorL(_))
      ...
      | (_, TyAliasLineT(tp, zty)) =>
        ...
        | Succeeded((new_zty, ctx, u_gen)) =>
          switch (
            Elaborator_Typ.syn_elab(ctx, Delta.empty, ZTyp.erase(new_zty))
          ) {
          | None => Failed
          | Some((_, k, _)) =>
  ...
  Log.fun_call(
    ...
    ~result_sexp=
      ActionOutcome.sexp_of_t(((zrules, u_gen)) =>
        List([ZExp.sexp_of_zrules(zrules), MetaVarGen.sexp_of_t(u_gen)])
      ),
    () => {
...
and ana_perform =
  ...
  Log.fun_call(
    ...
    ~id=u_gen,
    ~result_sexp=
      ActionOutcome.sexp_of_t(((ze, u_gen)) =>
