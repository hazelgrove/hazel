...
and syn_perform_operand =
  ...
  | (
      ...
      CursorE(
        OnText(_) | OnOp(_),
        EmptyHole(_) | ListNil(_) | Fun(_) | Inj(_) | Case(_) | Subscript(_) |
        Parenthesized(_),
      ) |
  ...
  | (Backspace, CursorE(OnText(j), StringLit(_, s))) =>
    if (j == 0) {
      ...
      let new_text = s |> EscapedString.apply(StringUtil.backspace(j));
  ...
  | (Construct(SOp(sop)), CursorE(OnText(j), StringLit(_, s)))
      when
        !ZExp.is_before_zoperand(zoperand)
        && !ZExp.is_after_zoperand(zoperand) =>
    syn_split_text(ctx, id_gen, j, sop, EscapedString.to_string(s))
  ...
  | (Construct(SChar(s)), CursorE(OnText(j), StringLit(_, x))) =>
    let new_text = x |> EscapedString.apply(StringUtil.insert(j, s));
    let text_cursor = CursorPosition.OnText(j + 1);
    let new_ze =
      ZExp.ZBlock.wrap(CursorE(text_cursor, UHExp.stringlit(new_text)));
    Succeeded(SynDone((new_ze, HTyp.String, id_gen)));
  ...
  | (Construct(SQuote), CursorE(OnDelim(_), EmptyHole(_))) =>
    ...
    let ze =
      ZExp.ZBlock.wrap(
        CursorE(text_cursor, UHExp.stringlit(EscapedString.empty)),
      );
    Succeeded(SynDone((ze, HTyp.String, id_gen)));
  ...
  | (
    ...
    syn_perform_operand(
      ctx,
      a,
      (CursorE(OnText(j), operand), HTyp.String, id_gen),
    );
  ...
  | (
      ...
      CursorE(
        OnDelim(_, _),
        ListNil(_) | Parenthesized(_) | Inj(_, _, _) | Fun(_, _, _) |
        Case(_, _, _) |
        Subscript(_),
      ),
    ) =>
  ...
  | (Construct(SSubscript), CursorE(_, operand))
    ...
    let (ctx, id_gen, ze) =
      switch (operand) {
      | EmptyHole(_) =>
        let (hole1, id_gen) = id_gen |> UHExp.new_EmptyHole;
        let (hole2, id_gen) = id_gen |> UHExp.new_EmptyHole;
        let ze =
          ZExp.ZBlock.wrap(
            SubscriptZE1(
              NotInHole,
  ...
  | (_, SubscriptZE1(_, zs, n1, n2)) =>
    ...
    | Succeeded((zs, id_gen)) =>
      switch (ZExp.is_zopseq(zs)) {
      /* If an operator was constructed on [zs], subscripting should be
       * applied to suffix, not whole expression.
       *
       * TODO: Doesn't work when splitting text (handle more specific ZOperand
       * case)?
       */
      ...
      | None =>
        let new_ze = ZExp.ZBlock.wrap(SubscriptZE1(NotInHole, zs, n1, n2));
        Succeeded(SynDone((new_ze, ty, id_gen)));
      }
    }
  | (_, SubscriptZE2(_, s, zn1, n2)) =>
    switch (ana_perform(ctx, a, (zn1, id_gen), Int)) {
    | Failed => Failed
    | CursorEscaped(side) =>
      syn_perform_operand(
        ctx,
    ...
    | Succeeded((zn1, id_gen)) =>
      let new_ze = ZExp.ZBlock.wrap(SubscriptZE2(NotInHole, s, zn1, n2));
      Succeeded(SynDone((new_ze, ty, id_gen)));
    }
  | (_, SubscriptZE3(_, s, n1, zn2)) =>
    switch (ana_perform(ctx, a, (zn2, id_gen), Int)) {
    | Failed => Failed
    | CursorEscaped(side) =>
      syn_perform_operand(
        ctx,
    ...
    | Succeeded((zn2, id_gen)) =>
      let new_ze = ZExp.ZBlock.wrap(SubscriptZE3(NotInHole, s, n1, zn2));
      Succeeded(SynDone((new_ze, ty, id_gen)));
    }
...
and ana_perform_operand =
  ...
  | (
    ...
    ana_perform_operand(ctx, a, (CursorE(OnText(j), operand), id_gen), ty);
  /* Disallow quotes directly next to anything */
  | (
      Construct(SQuote),
      CursorE(
        OnDelim(_, _),
        ListNil(_) | Fun(_, _, _) | Inj(_, _, _) | Case(_, _, _) |
        Subscript(_) |
        Parenthesized(_),
      ),
    ) =>
  ...
  | (Construct(SOp(sop)), CursorE(OnText(j), StringLit(_, s)))
      when
        !ZExp.is_before_zoperand(zoperand)
        && !ZExp.is_after_zoperand(zoperand) =>
    ana_split_text(ctx, id_gen, j, sop, EscapedString.to_string(s), ty)
  ...
  | (Construct(SSubscript), CursorE(_, operand))
    ...
    let (ctx, id_gen, ze) =
      switch (operand) {
      | EmptyHole(_) =>
        let (hole1, id_gen) = UHExp.new_EmptyHole(id_gen);
        let (hole2, id_gen) = UHExp.new_EmptyHole(id_gen);
        let ze =
          ZExp.ZBlock.wrap(
            SubscriptZE1(
              NotInHole,
