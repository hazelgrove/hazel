...
let mk_SynExpandsToMatch = (~id_gen, ~prefix=[], ~suffix=[], ~scrut, ()) =>
  SynExpands({kw: Match, id_gen, prefix, suffix, subject: scrut});
...
let mk_AnaExpandsToMatch = (~id_gen, ~prefix=[], ~suffix=[], ~scrut, ()) =>
  AnaExpands({kw: Match, id_gen, prefix, suffix, subject: scrut});
...
let zcase_of_scrut_and_suffix =
  ...
  | ([ExpLine(OpSeq(_, S(EmptyHole(_), E)))], []) =>
    ...
    (ZExp.MatchZE(StandardErrStatus(NotInHole), zscrut, [rule]), id_gen);
  | (_, []) =>
...
let syn_split_text =
  ...
  | (ExpandingKeyword(kw), Some(Space), rshape) =>
    ...
    Succeeded(
      ...
      | Match => mk_SynExpandsToMatch(~id_gen, ~scrut=subject, ())
      | Fun => mk_SynExpandsToFun(~id_gen, ~param=subject, ())
      },
    );
...
let ana_split_text =
  ...
  | (ExpandingKeyword(kw), Some(Space), rshape) =>
    ...
    Succeeded(
      ...
      | Match => mk_AnaExpandsToMatch(~id_gen, ~scrut=subject, ())
      | Fun => mk_AnaExpandsToFun(~id_gen, ~param=subject, ())
      },
    );
  | (lshape, Some(op), rshape) =>
    let (loperand, id_gen) = UHExp.text_operand(id_gen, lshape);
...
let rec syn_perform =
  ...
  | Succeeded(SynExpands({kw: Match, prefix, subject, suffix, id_gen})) =>
    let (zmatch, id_gen) =
  ...
  | Succeeded(SynExpands({kw: Let, prefix, subject, suffix, id_gen})) =>
    ...
    Succeeded(Statics_Exp.syn_fix_holes_z(ctx, id_gen, new_ze));
  | Succeeded(SynExpands({kw: Fun, prefix: _, subject, suffix: _, id_gen})) =>
...
and syn_perform_block =
  ...
  | MoveToNextHole
  | MoveLeft
  | MoveRight => syn_move(ctx, a, (zblock, ty, id_gen))
...
and syn_perform_line =
  ...
  /* let x =|> 2   ==>   let x = |2 */
  | (Delete, CursorL(OnDelim(_, After as side), _)) => escape(id_gen, side)
...
and syn_perform_opseq =
  ...
  | (MoveTo(_) | MoveToPrevHole | MoveToNextHole | MoveLeft | MoveRight, _) =>
    syn_move(ctx, a, (ZExp.ZBlock.wrap'(zopseq), ty, id_gen))

  /* Deletion */

  | (Delete, ZOperator((OnOp(After as side), _), _))
  | (Backspace, ZOperator((OnOp(Before as side), _), _)) =>
    syn_perform_opseq(ctx, Action_common.escape(side), (zopseq, ty, id_gen))
  ...
  | (Delete, ZOperator((OnOp(Before), op), surround)) =>
    ...
    syn_perform(ctx, Backspace, (new_ze, ty, id_gen)) |> wrap_in_SynDone;
  ...
  | (Construct(SOp(SSpace)), ZOperator(zoperator, _))
    ...
    syn_perform_opseq(ctx, MoveRight, (zopseq, ty, id_gen))
  ...
  /*
   Pressing <Parenthesized> after a constant injection is handled by
   syn_perform_operand.
   */
  | (
      Construct(SParenthesized),
      ZOperand(
        CursorE(OnDelim(1, After), Inj(_, _, None)) as zoperand,
        surround,
      ),
    ) =>
    ...
    | Failed
    | CursorEscaped(_)
    | Succeeded(SynExpands(_)) => Failed
    | Succeeded(SynDone((new_zexp, _, id_gen))) =>
      let (zexp, id_gen) = resurround_z(id_gen, new_zexp, surround);
      Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, zexp)));
    }
...
and syn_perform_operand =
  ...
  | (
      ...
      CursorE(
        OnText(_) | OnOp(_),
        EmptyHole(_) | ListNil(_) | Fun(_) | Inj(_) | Match(_) |
        Parenthesized(_),
      ),
    ) =>
    Failed
  ...
  /* Movement handled at top level */
  | (MoveTo(_) | MoveToPrevHole | MoveToNextHole | MoveLeft | MoveRight, _) =>
    syn_move(ctx, a, (ZExp.ZBlock.wrap(zoperand), ty, id_gen))
  ...
  /*
   ...
   1|  =(Inj)=>  inj[ |? ]( 1 )
   */
  | (Construct(SInj), CursorE(_, e)) =>
    let (ztag, id_gen) = ZTag.new_TagHole(id_gen);
    let arg_opt =
      switch (e) {
      | EmptyHole(_) => None
      | _ => Some(UHExp.Block.wrap(e))
      };
    let ze = ZExp.ZBlock.wrap(ZExp.InjZT(NotInHole, ztag, arg_opt));
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));

  /*
   Pressing <Parenthesized> after a constant injection wraps the injection
   around an expression hole.

   inj[ _ ]|  =(Parenthesized)=>  inj[ _ ]( |? )
   */
  | (
      Construct(SParenthesized),
      CursorE(OnDelim(1, After), Inj(status, tag, None)),
    ) =>
    let (zhole, id_gen) = ZExp.new_EmptyHole(id_gen);
    let zarg = ZList.singleton(ZExp.ExpLineZ(ZOpSeq.wrap(zhole)));
    let ze = ZExp.ZBlock.wrap(ZExp.InjZE(status, tag, zarg));
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[ _ |>]( 2 )  ==>  |2
   */
  | (Delete, CursorE(OnDelim(0 | 1, Before), Inj(_, _, Some(arg)))) =>
    let ze = ZExp.place_before(arg);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  | (Delete, CursorE(OnDelim(0, After), Inj(_, tag, arg_opt))) =>
    let zinj = ZExp.InjZT(NotInHole, ZTag.place_before(tag), arg_opt);
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));

  /*
   Pressing <Delete> after the opening delimiter of an injection argument moves
   the cursor forward.

   inj[ _ ](|> _ )  ==>  inj[ _ ]( |_ )
   */
  | (Delete, CursorE(OnDelim(1, After), Inj(_, tag, Some(arg)))) =>
    let zinj = ZExp.InjZE(NotInHole, tag, ZExp.place_before(arg));
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[ _ ]( 2 |>)  ==>  inj[ _ ]|
   */
  | (Delete, CursorE(OnDelim(2, Before), Inj(_, tag, Some(_)))) =>
    let zinj = ZExp.place_after_operand(Inj(NotInHole, tag, None));
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[<| _ ]( ? )  ==>  |?
   */
  | (Backspace, CursorE(OnDelim(_, After), Inj(_, _, None)))
  | (
      Backspace,
      CursorE(
        OnDelim(0, After),
  ...
  /*
   ...
   inj[<| _ ]( 2 )  ==>  |2
   */
  | (Backspace, CursorE(OnDelim(0, After), Inj(_, _, Some(arg)))) =>
    let ze = ZExp.place_before(arg);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  | (Backspace, CursorE(OnDelim(1, Before), Inj(_, tag, arg_opt))) =>
    let zinj = ZExp.InjZT(NotInHole, ZTag.place_after(tag), arg_opt);
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[ _ ](<| 2 )  ==>  inj[ _ ]|
   */
  | (Backspace, CursorE(OnDelim(1, After), Inj(_, tag, _))) =>
    let zinj = ZExp.place_after_operand(Inj(NotInHole, tag, None));
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[ _ ]( _ <|)  ==>  inj[ _ ]( _| )
   */
  | (Backspace, CursorE(OnDelim(_2, Before), Inj(_, tag, Some(arg)))) =>
    let zinj = ZExp.InjZE(NotInHole, tag, ZExp.place_after(arg));
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[ _ ]( 2 )<|  ==>  inj[ _ ]|
   */
  | (Backspace, CursorE(OnDelim(_2, After), Inj(_, tag, _))) =>
    let zinj = ZExp.place_after_operand(Inj(NotInHole, tag, None));
    let ze = ZExp.ZBlock.wrap(zinj);
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  | (
    ...
    let zinj = ZExp.InjZT(status, place_cursor(tag), arg_opt);
    syn_perform_operand(ctx, a, (zinj, ty, id_gen));
   ...
   */
  | (Construct(SCloseParens), InjZE(err, tag, zarg))
      when ZExp.is_after(zarg) =>
    let inj = UHExp.Inj(err, tag, Some(ZExp.erase(zarg)));
    let ze = ZExp.ZBlock.wrap(ZExp.place_after_operand(inj));
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  /*
   ...
   inj[ _ ]( _ |)  =(SCloseParens)=>  inj[ _ ]( _ )|
   */
  | (Construct(SCloseParens), CursorE(OnDelim(2, Before), Inj(_) as inj)) =>
    let ze = ZExp.ZBlock.wrap(ZExp.place_after_operand(inj));
    Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, ze)));
  ...
  | (Delete, CursorE(_, EmptyHole(_) as operand)) =>
    ...
    ze |> ZExp.is_before
      ? Succeeded(SynDone((ze, Hole, id_gen))) : CursorEscaped(After);

  /* ( _ <|)   ==>   ( _| ) */
  | (Backspace, CursorE(OnDelim(_, Before), _)) =>
    syn_perform_operand(ctx, MoveLeft, (zoperand, ty, id_gen))
  /* (|> _ )   ==>   ( |_ ) */
  ...
  | (
      ...
      CursorE(
        OnDelim(k, After),
        (Fun(_, _, e) | Match(_, e, _) | Parenthesized(e)) as operand,
      ),
    ) =>
    let place_cursor =
      switch (operand) {
  ...
  | (
      ...
      CursorE(OnDelim(1, Before), Parenthesized(_) as operand),
    ) =>
    Succeeded(
      SynDone((
  ...
  | (_, InjZE(err, tag, zarg)) =>
    switch (ana_perform(ctx, a, (zarg, id_gen), HTyp.Hole)) {
    | Failed => Failed
    | CursorEscaped(side) =>
      syn_perform_operand(
        ctx,
    ...
    | Succeeded((zarg', id_gen)) =>
      let zinj = ZExp.ZBlock.wrap(InjZE(err, tag, zarg'));
      Succeeded(SynDone(Statics_Exp.syn_fix_holes_z(ctx, id_gen, zinj)));
    }
    ...
    | Some(ty1) =>
      ...
      | Succeeded((zscrut, ty1, id_gen)) =>
        ...
        | Some(ty) =>
          ...
          let (match_err, id_gen) =
            if (Incon.is_exhaustive(con)) {
              (MatchErrStatus.StandardErrStatus(NotInHole), id_gen);
            } else {
              let (u, id_gen) = IDGen.next_hole(id_gen);
...
and ana_perform =
  ...
  | Succeeded(AnaExpands({kw: Match, prefix, subject, suffix, id_gen})) =>
    let (zmatch, id_gen) =
      zcase_of_scrut_and_suffix(id_gen, subject, suffix);
    let new_zblock =
      (prefix, ZExp.ExpLineZ(zmatch |> ZOpSeq.wrap), [])
  ...
  | Succeeded(AnaExpands({kw: Let, prefix, subject, suffix, id_gen})) =>
    ...
    Succeeded(Statics_Exp.ana_fix_holes_z(ctx, id_gen, new_zblock, ty));
  | Succeeded(AnaExpands({kw: Fun, prefix: _, subject, suffix: _, id_gen})) =>
    let (zhole, id_gen) = id_gen |> ZPat.new_EmptyHole;
    let new_ze =
      ZExp.ZBlock.wrap(FunZP(NotInHole, ZOpSeq.wrap(zhole), subject));
...
and ana_perform_block =
  ...
  /* Movement */
  | (MoveTo(_) | MoveToPrevHole | MoveToNextHole | MoveLeft | MoveRight, _) =>
    ana_move(ctx, a, (zblock, id_gen), ty)
...
and ana_perform_opseq =
  ...
  /* Movement handled at top level */
  | (MoveTo(_) | MoveToPrevHole | MoveToNextHole | MoveLeft | MoveRight, _) =>
    ana_move(ctx, a, (ZExp.ZBlock.wrap'(zopseq), id_gen), ty)
  ...
  | (Delete, ZOperator((OnOp(After as side), _), _))
  | (Backspace, ZOperator((OnOp(Before as side), _), _)) =>
    ana_perform_opseq(ctx, Action_common.escape(side), (zopseq, id_gen), ty)
  ...
  | (Delete, ZOperator((OnOp(Before), op), surround)) =>
    ...
    ana_perform_opseq(ctx, Backspace, (new_zopseq, id_gen), ty);
  ...
  | (Construct(SOp(SSpace)), ZOperator(zoperator, _))
      when ZExp.is_after_zoperator(zoperator) =>
    ana_perform_opseq(ctx, MoveRight, (zopseq, id_gen), ty)
  ...
  | (
      Construct(SParenthesized),
      ZOperand(CursorE(_, Inj(_, _, None)) as zoperand, surround),
    )
      when ZExp.is_after_zoperand(zoperand) =>
    switch (ana_perform_operand(ctx, a, (zoperand, id_gen), ty)) {
    | Failed
    | CursorEscaped(_)
    | Succeeded(AnaExpands(_)) => Failed
    | Succeeded(AnaDone((new_zexp, id_gen))) =>
      let (zexp, id_gen) = resurround_z(id_gen, new_zexp, surround);
      Succeeded(
        AnaDone(Statics_Exp.ana_fix_holes_z(ctx, id_gen, zexp, ty)),
      );
    }
...
and ana_perform_operand =
  ...
  /*
   ...
   1|  =(Inj)=>  inj[ |? ]( 1 )
   */
  | (Construct(SInj), CursorE(_, operand)) =>
    let (ztag, id_gen) = ZTag.new_TagHole(id_gen);
    let arg_opt =
      switch (operand) {
      | EmptyHole(_) => None
    ...
    let ze = ZExp.ZBlock.wrap(ZExp.InjZT(NotInHole, ztag, arg_opt));
    Succeeded(AnaDone(Statics_Exp.ana_fix_holes_z(ctx, id_gen, ze, ty)));
   ...
   */
  | (Backspace, CursorE(OnDelim(0, After), Inj(_, _, Some(arg)))) =>
    let zexp = ZExp.place_before(arg);
    Succeeded(AnaDone(Statics_Exp.ana_fix_holes_z(ctx, id_gen, zexp, ty)));
  ...
  | (
      ...
      when UHTag.is_tag_char(c.[0]) =>
    let place_cursor = side == After ? ZTag.place_before : ZTag.place_after;
    let ze = ZExp.InjZT(status, place_cursor(tag), arg_opt);
    ana_perform_operand(ctx, a, (ze, id_gen), ty);
  ...
  | (Delete, CursorE(_, EmptyHole(_) as operand)) =>
    ...
    ze |> ZExp.is_before
      ? Succeeded(AnaDone((ze, id_gen))) : CursorEscaped(After);

  /* ( _ <|)   ==>   ( _| ) */
  | (Backspace, CursorE(OnDelim(_, Before), _)) =>
    ana_perform_operand(ctx, MoveLeft, (zoperand, id_gen), ty)
  /* (|> _ )   ==>   ( |_ ) */
  | (Delete, CursorE(OnDelim(_, After), _)) =>
  ...
  | (Construct(SMatch), CursorE(_, operand)) =>
    Succeeded(
      mk_AnaExpandsToMatch(~id_gen, ~scrut=UHExp.Block.wrap(operand), ()),
    )
  | (Construct(SLet), CursorE(_, operand)) =>
    Succeeded(
  ...
  | (_, InjZT(_, ztag, arg_opt)) =>
    switch (Action_Tag.perform(id_gen, a, ztag)) {
    | Succeeded((ztag', id_gen)) =>
      let ze = ZExp.ZBlock.wrap(InjZT(NotInHole, ztag', arg_opt));
      Succeeded(AnaDone(Statics_Exp.ana_fix_holes_z(ctx, id_gen, ze, ty)));
    | CursorEscaped(side) =>
      let a' = Action_common.escape(side);
      ana_perform_operand(ctx, a', (zoperand, id_gen), ty);
  ...
  | (_, InjZE(_, tag, zarg)) =>
    switch (ana_perform(ctx, a, (zarg, id_gen), HTyp.Hole)) {
    | Succeeded((zarg', id_gen)) =>
      let ze = ZExp.ZBlock.wrap(InjZE(NotInHole, tag, zarg'));
      Succeeded(AnaDone(Statics_Exp.ana_fix_holes_z(ctx, id_gen, ze, ty)));
    | CursorEscaped(side) =>
      let a' = Action_common.escape(side);
      ana_perform_operand(ctx, a', (zoperand, id_gen), ty);
