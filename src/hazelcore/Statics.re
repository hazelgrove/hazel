    ...
    lazy(
      optimize_syn_fixer(
        ...
        (
          {
            let operand_nih =
              operand |> UHPat.set_err_status_operand(NotInHole);
            switch (operand) {
            | EmptyHole(_) =>
        ...
        (
          ...
          | BinOp(_, Cons, skel1, skel2) =>
            ...
            | Some(ty_elt) =>
              ...
              let skel =
                Skel.BinOp(NotInHole, Operators.Pat.Cons, skel1, skel2);
              ((skel, seq), ctx, u_gen);
            | None =>
              let ((skel1, seq), (ty_elt, ctx), u_gen) =
                Lazy.force(
...
module Exp = {
      ...
      optimize_syn_fixer(
        ...
        (
          ...
          | BinOp(_, Cons, skel1, skel2) =>
            ...
            let skel =
              Skel.BinOp(NotInHole, Operators.Exp.Cons, skel1, skel2);
            ((skel, seq), ty, u_gen);
          }: (
  ...
  let ana_fix_holes_block =
      (ctx, u_gen, ~renumber_empty_holes=false, block, ty) =>
    Lazy.force(
      ana_fix_holes_block,
      ctx,
      u_gen,
  ...
  let ana_fix_holes_opseq =
      (ctx, u_gen, ~renumber_empty_holes=false, opseq, ty) =>
    Lazy.force(
      ana_fix_holes_opseq,
      ctx,
      u_gen,
