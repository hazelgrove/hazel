...
and erase_zopseq: zopseq => UHPat.opseq =
  zos => ZOpSeq.erase(~erase_zoperand, ~erase_zbinop=erase_zoperator, zos)
and erase_zoperand: zoperand => UHPat.operand =
  zop =>
    switch (zop) {
    | CursorP(_, operand) => operand
    | ParenthesizedZ(zp) => UHPat.Parenthesized(erase(zp))
    | UnaryOpZ(err, unop, child) =>
      UHPat.UnaryOp(err, unop, erase_zoperand(child))
...
and is_after_zoperand =
  ...
  | InjZ(_, _, _)
  | ParenthesizedZ(_) => false
  | UnaryOpZ(_, _, child) => is_after_zoperand(child)
