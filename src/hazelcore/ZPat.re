...
let valid_cursors_operand: UHPat.operand => list(CursorPosition.t) =
  CursorPosition.(
    ...
    // | UserOp(_, _, x) => text_cursors(Var.length(x))
...
and is_after_zoperand =
  ...
  // | CursorP(cursor, UserOp(_, _, x)) => cursor == OnText(Var.length(x))
...
and place_after_operand = operand =>
  ...
  // | UserOp(_, _, x) => CursorP(OnText(Var.length(x)), operand)
