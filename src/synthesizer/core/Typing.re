...
let rec getType = (delta: hole_context, gamma: context, e: exp): type_ =>
  ...
  | Bool(_) => Bool_t
  | Cons(e1, e2) =>
    Cons_t(getType(delta, gamma, e1), getType(delta, gamma, e2))
  | Nil => Any_t
  | Var(x) =>
  ...
  | Unit => Unit_t
  | Pair(e1, e2) =>
    Pair_t(getType(delta, gamma, e1), getType(delta, gamma, e2))
  | Fst(e') =>
    switch (getType(delta, gamma, e')) {
    | Pair_t(t1, _) => t1
  ...
  | Ctor(id, d, e1) =>
    if (Tools.lookup(d, sigma)
        |> Tools.lookup(id) == getType(delta, gamma, e1)) {
      D(d);
    } else {
...
let rec getResType = (delta, r: res) =>
  ...
  | Rfloat(_) => Any_t
  | Rbool(_) => Bool_t
  | Rfunc(n, id, typ, e, env) =>
    getType(delta, generateContext(delta, env), Function(n, id, typ, e))
  | Rapp(r1, r2) =>
    switch (getResType(delta, r1)) {
  ...
  | Rctor(id, adt, r') =>
    if (Tools.lookup(adt, sigma)
        |> Tools.lookup(id) == getResType(delta, r')) {
      D(adt);
    } else {
...
and generateContext = (delta, env) =>
  ...
  | [] => []
  | [(x, r), ...env'] => [
      (x, (getResType(delta, r), AnnNone)),
      ...generateContext(delta, env'),
    ]
  };
...
let rec getExType = (delta, ex) => {
  ...
  | Efunc(v, ex1) =>
    Function_t(
      Typecasting.valToRes(v) |> getResType(delta),
      getExType(delta, ex1),
    )
  | Ector(id, adt, ex1) =>
    ...
    if (t == getExType(delta, ex1)) {
      ...
      failwith(
        "Type error: Example doesn't have type required by example constructor",
      );
    };
  };
};
...
let getConstraintType = (delta, exs: excons) => {
  ...
  | [x, ..._] =>
    ...
    | [] => x
    | _ =>
      failwith("Contexts are not consistent for set of example constraints")
    }
  };
};
...
let rec generateHoleContextU_h = (delta, us) => {
  ...
  | [] => delta
  | [(id, exs), ...xs] =>
    generateHoleContextU_h(
      [(id, getConstraintType(delta, exs)), ...delta],
      xs,
    )
  };
};
