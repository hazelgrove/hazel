...
let updateHoleContext = (delta, h, gs) => {
  List.filter(((h', _)) => h != h', updateHoleContext_h(delta, gs));
};

let rec updateUnfilledHoles = gs =>
  switch (gs) {
  | [] => []
  | [(_, h, _, exs), ...gs'] => [(h, exs), ...updateUnfilledHoles(gs')]
  };
...
let rec guessAndCheck_h = (delta, gamma, f, typ, exs, i) =>
  if (i > 8) {
    ...
    let checked =
      List.filter(
        e => Unevaluator.constrainExp(delta, f, e, exs) |> optionPred,
        es,
      );
    switch (checked) {
...
and guessAndOrBranch = (delta, holeFillings, gamma, h, typ, exs, depth) => {
  ...
  | None when depth <= 3 =>
    ...
    let bs =
      Brancher.branch(delta, gamma, holeFillings, typ, exs)
      |> List.map(((exp, goals, unevalCons)) => {
           let f = [(h, exp), ...holeFillings];
           let u = List.map(((gamma, h, typ, xs)) => (h, xs), goals);
           let delta' = List.filter(((h', _)) => h != h', delta);
           let delta' =
             List.map(((gamma, h, typ, _)) => (h, (gamma, typ)), goals)
             @ delta';
           ((u, f), delta');
         });
    Some((depth + 1, bs));
