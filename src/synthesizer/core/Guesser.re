...
let guessPairs = (delta, gamma, t1, t2, i, j) => {
  let es =
    List.filter(e => Typing.getType(delta, gamma, e) == t1, memo[i - 1])
    |> List.map(e1 =>
         List.filter(
           e2 => Typing.getType(delta, gamma, e2) == t2,
           memo[j - 1],
         )
         |> List.map(e2 => Pair(e1, e2))
       )
    |> List.concat;
  es;
};
    ...
    List.map(
      ((c, t)) =>
        ...
        | Pair_t(t1, t2) =>
          ...
          |> List.concat
          |> List.map(e => Ctor(c, adt, e));
        | _ =>
          List.filter(e => Typing.getType(delta, gamma, e) == t, memo[i - 2])
          |> List.map(e => Ctor(c, adt, e))
        },
      constructors,
    )
  ...
  let args =
    List.map(
      e => {
        ...
        List.map(x => Application(e, x), candidates);
      },
      funcs,
    );
  List.concat(args);
...
let guess =
    (delta: hole_context, gamma: context, typ: type_, i: int): list(exp) =>
  if (i == 1) {
    let terms = List.filter(((_, (t, _))) => t == typ, gamma);
    memo[0] = [Unit] @ List.map(((x, _)) => Var(x), gamma);
    ...
    let pairs = partition(i);
    let apps =
      List.map(((n, m)) => guessApp(delta, gamma, typ, n, m), pairs)
      |> List.concat;
    memo[i - 1] = firsts @ seconds @ ctors @ apps;
    List.filter(e => Typing.getType(delta, gamma, e) == typ, memo[i - 1]);
  };
