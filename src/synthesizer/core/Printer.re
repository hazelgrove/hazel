...
and string_of_exp = (e: exp): string =>
  ...
  | Function(n, identifier, typ, exp) =>
    "("
    ++ string_of_identifier(n)
    ++ "): \\"
    ...
    ++ "."
    ++ string_of_exp(exp)
  | Application(exp, exp2) =>
    string_of_exp(exp) ++ " " ++ string_of_exp(exp2)
  | Hole(hole_identifier) =>
    "??_" ++ string_of_hole_identifier(hole_identifier)
  | Unit => "()"
  | Pair(exp, exp2) =>
    "(" ++ string_of_exp(exp) ++ ", " ++ string_of_exp(exp2) ++ ")"
...
and string_of_res = (r: res): string =>
  ...
  | Rnil => "Nil"
  | Rfunc(n, identifier, typ, exp, environment) =>
    "["
    ++ string_of_env(environment)
    ++ "]("
    ++ string_of_identifier(n)
    ++ "): \\"
    ++ string_of_identifier(identifier)
  ...
  | Rhole(hole_identifier, environment) =>
    ...
    ++ string_of_env(environment)
    ++ "]??_"
    ++ string_of_hole_identifier(hole_identifier)
  | Runit => "()"
  | Rpair(res, res2) =>
    "(" ++ string_of_res(res) ++ ", " ++ string_of_res(res2) ++ ")"
  | Rfst(res) => "fst(" ++ string_of_res(res) ++ ")"
  | Rsnd(res) => "snd(" ++ string_of_res(res) ++ ")"
  | Rctor(name, _, res) =>
    "C" ++ string_of_int(name) ++ " " ++ string_of_res(res)
  | Rictor(name, _, res) =>
    "Ci" ++ string_of_int(name) ++ " " ++ string_of_res(res)
  | Rcase(res, branches, env) =>
    "[" ++ string_of_env(env) ++ "] case ";
    string_of_res(res) ++ " of {" ++ string_of_branches(branches) ++ "}";
  }
...
and string_of_type_ = (t: type_): string =>
  ...
  | Bool_t => "Bool"
  | Cons_t(a, b) =>
    "Cons(" ++ string_of_type_(a) ++ "," ++ string_of_type_(b) ++ ")"
  | Function_t(a, b) =>
    "(" ++ string_of_type_(a) ++ "->" ++ string_of_type_(b) ++ ")"
  | Unit_t => "Unit"
  | Pair_t(a, b) =>
    "(" ++ string_of_type_(a) ++ ", " ++ string_of_type_(b) ++ ")"
...
and string_of_example = (ex: example): string =>
  ...
  | Epair(ex1, ex2) =>
    "(" ++ string_of_example(ex1) ++ ", " ++ string_of_example(ex2) ++ ")"
  | Efunc(v, ex') =>
    "{" ++ string_of_value(v) ++ " -> " ++ string_of_example(ex') ++ "}"
  | Ector(id, _, ex') =>
    "C" ++ string_of_int(id) ++ " " ++ string_of_example(ex')
  }
...
and string_of_one_constraint_ = (c): string =>
  ...
  | [(environment, (hole_identifier, example)), ...cs] =>
    ...
    ++ string_of_hole_identifier(hole_identifier)
    ++ "->"
    ++ string_of_example(example)
    ++ "; "
    ++ string_of_one_constraint_(cs)
  }
...
and string_of_context = (e: context): string =>
  switch (e) {
  | [] => "-"
  | [(identifier, (type_, _)), ...ms] =>
    string_of_identifier(identifier)
    ++ "->"
    ++ string_of_type_(type_)
    ++ "; "
    ++ string_of_context(ms)
  }
and string_of_hole_context = (c): string =>
  ...
  | [(hole_identifier, (context, type_)), ...cs] =>
    ...
    ++ string_of_type_(type_)
    ++ "; "
    ++ string_of_hole_context(cs)
  }
and string_of_guess_output = (c): string =>
  switch (c) {
  | [] => "-"
  | [x, ...cs] => string_of_exp(x) ++ "; " ++ string_of_guess_output(cs)
  }
and string_of_solver_output = (c): string => {
  let (hf, hc) = c;
  "("
  ++ string_of_hole_fillings(hf)
  ++ ", "
  ++ string_of_hole_context(hc)
  ++ ")";
}
