...
let rec branch =
        (delta: hole_context, gamma: context, f, typ: type_, exs: excons) => {
  let datatypes =
    List.map(((id, (t, _))) => t, gamma)
    |> List.filter(t =>
         switch (t) {
         | D(_) => true
         | _ => false
    ...
    |> List.sort_uniq((t1, t2) => 0);

  List.map(d => branch_indiv(delta, gamma, f, typ, exs, d), datatypes)
  |> List.concat;
}
...
and branch_indiv = (delta, gamma, f, typ, exs, datatype) => {
  ...
  List.map(
    e => {
      ...
      let distributedExs =
        distribute(delta, f, exs, datatype, e, constructors);
      let unevalCons: option(unevalcons) =
        List.map(
      ...
      let goals =
        List.mapi(
          (i, (id, (pat, Hole(h)))) => {
            ...
            | _ =>
              failwith(
                "Sam took a shortcut and this isn't implemented yet. Blame him",
              )
            };
          },
          branches,
