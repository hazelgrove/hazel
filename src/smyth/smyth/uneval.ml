...
module FuelLimited = struct
  let rec check fuel delta sigma hf exp worlds =
    let check_one (env, ex) =
      ...
      | _ -> Nondet.none
    in
    worlds |> List.map check_one |> Nondet.one_of_each
  ...
  and uneval fuel delta sigma hf res ex =
    let rec blocking_hole (r : res) : hole_name option =
      match r with
      (* Determinate results *)
      | RFix (_, _, _, _) | RTuple _ | RCtor (_, _) -> None
    ...
    | RCtor (name1, arg1), ExCtor (name2, arg2) ->
        if String.equal name1 name2 then uneval fuel delta sigma hf arg1 arg2
        else Nondet.none
    ...
    | RFix (env, f, PatParam x, body), ExInputOutput (input, output) ->
        let fix_env_extension = Pat.bind_rec_name_res f res in
        let* x_env_extension =
    ...
    | RApp (r1, RARes r2), _ -> (
      match Res.to_value r2 with
      | Some v2 -> uneval fuel delta sigma hf r1 @@ ExInputOutput (v2, ex)
      | None -> Nondet.none )
    | RProj (n, i, arg), _ ->
    ...
    | RCase (env, scrutinee, branches), _ ->
        if !minimal_uneval then
          let* hf_guesses = guesses delta sigma scrutinee in
          let* hf' =
            Nondet.lift_option @@ Constraints.merge_solved [hf; hf_guesses]
          in
          let ks_guesses = (hf_guesses, Hole_map.empty) in
  ...
  and simplify_assertions fuel delta sigma rcs =
    let simplify_one (res, value) =
      if Res.final res then
        uneval fuel delta sigma Hole_map.empty res (Example.from_value value)
      else Nondet.none
    in
    rcs |> List.map simplify_one |> Nondet.one_of_each
    |> Nondet.map Constraints.merge
    |> Nondet.collapse_option
end
