...
let propagate (hf : hole_filling) : hole_filling =
  let rec propagate_exp exp =
    ...
    (* Main case *)
    | EHole hole_name -> (
      match Hole_map.find_opt hole_name hf with
      | Some hole_exp ->
          if Exp.syntactically_equal exp hole_exp then EHole hole_name
          else propagate_exp hole_exp
...
let restrict (delta : hole_ctx) (hf : hole_filling) :
    (hole_name * exp) list option =
  delta
  |> List.map (fun (hole_name, _) ->
