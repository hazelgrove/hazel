...
module Update = {
  ...
  let calculate = (~schedule_action: t => unit, model: Model.t): Model.t => {
    current: model.current |> Page.Update.calculate(~schedule_action),
    undo_stack: model.undo_stack,
    redo_stack: model.redo_stack,
  };
};
...
module Selection = {
  ...
  let get_cursor_info = (model: Model.t) =>
    Page.Focus.get_cursor_info(model.current);
};
