    ...
    List.fold_left(
      ((msg, mapping), elem) => {
        ...
        | Omd.Code_block(_, lang, code) =>
          ...
          /* `code-block` is what explainthis.css styles: preserved spacing and
             a scroll rather than a wrap, which is the only way a Fumola form
             stays on one line -- a symbol opens with a backtick, so an inline
             span would close on its own first character. */
          let plain = () =>
            Node.pre(
