...
/* `T(a, b, …)` as a type pattern at the head of a parameterized
   ...
   general form covers arbitrary arity and highlights the head and
   the parameter list as a whole. */
...
let param_tpat_general_coloring_ids =
    (~head_id: Id.t, ~params_list_id: Id.t): list((Id.t, Id.t)) => [
  (Piece.id(_head), head_id),
  (Piece.id(_general_ps), params_list_id),
];
...
let param_tpat_general_form: form = {
  let explanation = "`%s` is a parameterized type constructor bound in the body. Its [*parameters*](%s) are type variables available inside the definition.";
  {
    id: ParamTPat(General),
    syntactic_form: [_head, mk_ap_tpat([[_general_ps]])],
    expandable_id: None,
    explanation,
    examples: [],
  };
