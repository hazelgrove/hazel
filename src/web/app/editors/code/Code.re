...
let view =
  ...
  /* A tab projector on the last line defers linebreaks that no
     ...
     text flow reserves the hang-below rows. A text layer ending in a
     linebreak gets no final line box in pre flow, so an empty last line
     left the editor one row short (caret overhanging into the result
     area; worst in stacked cells): a zero-width space reserves the last
     row wherever the caller asks for one. */
  let nodes =
    switch (DeferredLinebreaks.consume()) {
    | 0 => nodes
    | n => nodes @ [Node.text(String.make(n, '\n'))]
    };
  reserve_trailing_row ? nodes @ [Node.text("\xe2\x80\x8b")] : nodes;
};
