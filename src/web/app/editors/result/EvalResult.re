...
module View = {
  ...
  let strudel_view = (~play_action, desc) =>
    Node.(
      div(
  ...
  let audio_view = (~play, exp: Exp.t): option(Node.t) =>
    if (is_sound(exp)) {
      let desc = sound_description(exp);
      let play_action = () =>
        switch (interpret_sound(exp)) {
        | Some(pattern) => Util.Strudel.playPattern(pattern)
        | None => ()
        };
      /* Auto-play if not locked and sound changed */
      if (!play && last_sound^ != desc) {
        let _ =
          try(play_action()) {
          | _ => ()
          };
      ...
      last_sound := desc;
      Some(strudel_view(~play_action, desc));
    } else {
      if (!play) {
        Util.Strudel.stopMusic();
      };
