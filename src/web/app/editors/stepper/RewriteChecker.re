...
let distribute_mul_over_add = (exp: Exp.t): option(Exp.t) => {
  ...
  | BinOp(times_op, left, right) when is_times_op(times_op) =>
    ...
    | (_, BinOp(plus_op, add_left, add_right)) when is_plus_op(plus_op) =>
      let left_product = normalized_product(times_op, left, add_left);
      let right_product = normalized_product(times_op, left, add_right);
      Some(normalized_sum(plus_op, left_product, right_product));
    | (BinOp(plus_op, add_left, add_right), _) when is_plus_op(plus_op) =>
      let left_product = normalized_product(times_op, add_left, right);
      let right_product = normalized_product(times_op, add_right, right);
      Some(normalized_sum(plus_op, left_product, right_product));
    | _ => None
    };
  | _ => None
