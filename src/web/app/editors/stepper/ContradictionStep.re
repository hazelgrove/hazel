...
/* `contradiction <exp> end` as a stepper row
 * (docs/prover-obligations.md, Phase 4e). A TERMINAL step, so — exactly
 * like `EvalStep` — the row carries no model, no actions and no nested
 * stepper: the cited fact lives in the proof syntax and the checker's
 * outgoing (`true`) is rendered by the surrounding row machinery. */
...
module F =
  ...
  let view_justification =
    ...
    WebUtil.Node.text("contradiction");
