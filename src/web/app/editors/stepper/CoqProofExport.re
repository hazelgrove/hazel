...
let rewrite_tactics_for_rule_id = (~domain=CoqExport.Integers, rule_id) =>
  ...
  | (_, "arith.collect_like_terms") => ["try hazel_rewrite_search 8%nat"]
  | (_, "arith.reorder_add_terms") => ["try hazel_rewrite_search 8%nat"]
  ...
  | (_, "alg.cancel_common_add") => ["try hazel_rewrite_search 10%nat"]
  | _ => []
  };
...
let domain_for_summary = (summary: RewriteChecker.trace_summary) =>
  summary.rule_ids |> List.exists(is_trig_rule_id)
...
let tactics_for_summary = (~domain, summary: RewriteChecker.trace_summary) => {
  ...
  recorded_tactics @ ["cbn", "try hazel_rewrite_search 8%nat", "reflexivity"];
};
...
let tactic_for_axiom_step = name =>
  [
    ...
    "cbn",
    "try hazel_rewrite_search 6%nat",
    "reflexivity",
  ]
  |> tactic_script;

let default_tactic =
  ["cbn", "try hazel_rewrite_search 6%nat", "reflexivity"] |> tactic_script;
