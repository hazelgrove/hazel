...
let rewrite_tactics_for_rule_id = (~domain=CoqExport.Integers, rule_id) =>
  ...
  | (_, "arith.collect_like_terms") => ["try hazel_rewrite_search 8"]
  | (_, "arith.reorder_add_terms") => ["try hazel_rewrite_search 8"]
  ...
  | (_, "alg.cancel_common_add") => ["try hazel_rewrite_search 10"]
  | _ => []
  };
...
let tactics_for_summary = (~domain, summary: RewriteChecker.trace_summary) => {
  ...
  recorded_tactics @ ["cbn", "try hazel_rewrite_search 8", "reflexivity"];
};
...
let tactic_for_axiom_step = name =>
  [
    ...
    "cbn",
    "try hazel_rewrite_search 6",
    "reflexivity",
  ]
  |> tactic_script;

let default_tactic =
  ["cbn", "try hazel_rewrite_search 6", "reflexivity"] |> tactic_script;
