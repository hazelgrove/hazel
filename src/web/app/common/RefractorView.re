...
let mk_data =
  ...
  List.filter_map(
    ((id, entry)) => {
      /* Construct full Base.projector on demand for rendering. Projectors
       * store their underlying syntax as a term, so parse the term's segment
       * range into a term for the projector to access. */
      let* syntax =
        ...
        |> (seg => Option.bind(seg, MakeTerm.for_projection));
      let p = Refractors.to_projector(syntax, id, entry);
