...
//     let tiles: Id.Map.t(Tile.t);
//   }
// ) => {
//   let code_svg = DecUtil.code_svg(~font_metrics=M.font_metrics);

//   let term = id => Id.Map.find(id, M.terms);
//   let tile = id => Id.Map.find(id, M.tiles);

//   let rows = M.measured.rows;

//   let shard_measurements = (id: Id.t): list((Id.t, int, Measured.measurement)) =>
//     term(id).ids
//     |> List.concat_map(id =>
//       Measured.find_shards(id, M.measured)
//       |> List.map((i, m) => (id, i, m))
//     );
...
//     ~has_caret: bool,
//     ~sort: Sort.t,
//     ~shapes: (Nib.Shape.t, Nib.Shape.t),
//     ~measurement: Measured.measurement
//   ) => {
//     let path = shard_path(shapes, m.last.col - m.origin.col);
//     let path_cls =
//       ["tile-path", "raised", Sort.to_string(s)]
//       @ (index == this_index ? ["indicated-caret"] : ["indicated"]);
//     let base_cls = ["tile-indicated"];
//     code_svg(~origin, ~base_cls, ~path_cls, path);
//   };

//   let shards = (id: Id.t, index) =>
//     shard_measurements(id)
//     |> List.map(((id', index', measurement)) => {
//       let t = tile(id');
//       shard(
//           ~has_caret=
//             id == id' && index == index',
//           ~sort=t.mold.out,
//           ~shapes=Mold.nib_shapes(~index=index', t.mold),
//           ~measurement,
//         )
//       });

//   let bi_lines = (id: Id.t) => {
//     let shards: Measured.Shards.t =
//       shard_measurements(id)
//       |> List.map(((_, index, m)) => (index, m));
...
//   let uni_lines = (id, (l, r)) => {
//     open SvgUtil.Path;
//     let shards: Measured.Shards.t =
//       shard_measurements(id)
//       |> List.map(((_, index, m)) => (index, m));
//     let s = tile(id).mold.out;
//     let l_line = {
