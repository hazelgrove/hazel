...
let rec typ_view = (ty: Typ.t): Node.t =>
...
let cls_str = (ci: Semantics.Statics.t): string =>
  switch (ci) {
...
let error_view = (err: Statics.error) =>
  switch (err) {
  | FreeVariable =>
    div(
...
let happy_view = (suc: Statics.happy) => {
  switch (suc) {
...
let status_view = (err: Statics.error_status) => {
  switch (err) {
...
let view_of_info = (ci: Statics.t): Node.t => {
  let is_err = Statics.is_error(ci);
  switch (ci) {
  | Invalid => div([clss([infoc, "unknown"])], [text("? No Info")])
  | InfoExp({mode, self, _}) =>
    let error_status = Statics.error_status(mode, self);
    div(
      [clss([infoc, "exp"])],
...
let view = (id: int, ci: Statics.t): Node.t =>
  div(
    [
      clss(["cursor-inspector"] @ [Statics.is_error(ci) ? errorc : happyc]),
