...
/* In-flight compaction stream handle. Not part of Model.t because
   [ApiHttp.streaming_handle] holds a closure and is not serializable; strictly
   transient and meaningless across reloads. Cleared in the stream's
   [on_done] and on Stop. */
let pending_compaction_stream_handle: ref(option(ApiHttp.streaming_handle)) =
  ref(None);
...
let send_compaction_request =
  ...
  let handle =
    OpenRouterHttp.start_streaming_chat(
      ~payload,
      ~key=api_key,
      ~on_chunk,
      ~on_done,
