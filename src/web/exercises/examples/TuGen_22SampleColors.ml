...
let exercise : Tutorial.spec = {
  id = Option.get (Haz3lcore.Id.of_string "b0000022-0022-0022-0022-000000000022");
  title = "22 - Sample Colors";
  version = 1;
  module_name = "TuGen_22SampleColors";
  prompt = {x|# Sample colors
...
3. Switch to *many* mode and click a sample inside the map. Samples from the same outer call are colored; samples from a different call are gray.|x};
  display_hint = {x||x};
  task_reference = {x|### Colors
- **green**: the focus's own run
- **pink**: started before the focus
- **blue**: finished after the focus
- **gray**: a different, unrelated call|x};
  your_impl =
    Haz3lcore.Zipper.caret_to_start (Option.get (Haz3lcore.TextRoundtrip.of_text ~root:Exp {x|let first = 1 + 2 in
let second = 3 * 4 in
let third = 5 + 6 in
