(* Auto-generated by: ./hazel gen-tutorial -- DO NOT EDIT *)
(* Source: hazel-programs/tutorial/08-clamp.hz *)

let exercise : Tutorial.spec = {
  id = Option.get (Haz3lcore.Id.of_string "7000000c-7507-4000-8000-000000000000");
  title = "08 Clamp";
  version = 1;
  module_name = "TuGen_08Clamp";
  prompt = {md|Work through the inline instructions in the editor below.|md};
  display_hint = "";
  task_reference = "";
  your_impl =
    Option.get (Haz3lcore.Parser.to_zipper ~root:Exp {hz|# PROBES TUTORIAL - PART 8: WRITING EXERCISE (CLAMP)               #

# Now it's your turn to write some Hazel code.                     #
# Implement `clamp`: constrain a number to be within [lo, hi].     #
#                                                                   #
#   clamp(5, 0, 10) == 5    (in range — return x)                  #
#   clamp(-3, 0, 10) == 0   (below min — return lo)                #
#   clamp(15, 0, 10) == 10  (above max — return hi)                #
#                                                                   #
# Replace the ? with your implementation using if/else:             #
#   if cond then expr else expr                                     #
#                                                                   #
# Turn on auto-probe and click inside your function as you write   #
# to see intermediate values update live.                           #
# =============================================================== #

let clamp = fun (x, lo, hi) ->
?


in

test
clamp(5, 0, 10)
== 5
end;

test
clamp(-3, 0, 10)
== 0
end;

test
clamp(15, 0, 10)
== 10
end;

test
clamp(0, 0, 10)
== 0
end;

test
clamp(10, 0, 10)
== 10
end

# END OF PART 8 - Select the next slide from the top menu       #|hz});
  hidden_tests =
    {
      tests =
        Option.get (Haz3lcore.Parser.to_zipper ~root:Exp {hz|test true end|hz});
      hints = [];
    };
  wrapper = false;
  show_report = false;
}
