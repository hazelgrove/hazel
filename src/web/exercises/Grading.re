...
module MutationTestingReport = {
  ...
  let mk =
      (
        ~test_validation,
        ~hidden_bugs_state: list(wrong_impl(Haz3lcore.Editor.Model.t)),
        ~hidden_bugs,
      )
      : t => {
    ...
    let hints =
      List.map(
        (wrong_impl: wrong_impl(Haz3lcore.Editor.Model.t)) =>
          wrong_impl.hint,
        hidden_bugs_state,
      );
...
module SyntaxReport = {
  ...
  let mk = (~your_impl: Haz3lcore.Editor.Model.t, ~tests: syntax_tests): t => {
    let user_impl_term =
      switch (Haz3lcore.Editor.Model.get_cached_term(your_impl)) {
      | Exp(t) => t
