...
let edit_buggy_impl =
    (state: state, idx: int, impl: Editor.Model.t, new_hint: hint) => {
  let buggy_impl = {
    impl,
    hint: new_hint,
  };
...
let term_of = (editor: Editor.Model.t): Language.Exp.t =>
...
let persist = (state: state, ~instructor_mode: bool) => {
  let zippers =
    ...
    |> List.map(((pos, editor: Editor.Model.t)) => {
         (pos, PersistentZipper.persist(editor |> Editor.get_z))
       });
  let persistent_hidden_bugs =
    state.eds.hidden_bugs
    |> List.map(({impl, hint}: wrong_impl(Editor.Model.t)) => {
         {
           impl: PersistentZipper.persist(impl |> Editor.get_z),
           hint,
         }
       });
  {
...
let unpersist =
  ...
  let lookup = (pos, default) =>
    if (visible_in(pos, ~instructor_mode)) {
      ...
      | Some(persisted_zipper) =>
        ...
        Editor.of_zipper(zipper);
      | None => Editor.of_zipper(default)
      };
    } else {
      Editor.of_zipper(default);
    };
  ...
  let hidden_bugs =
    hidden_bugs
    |> List.map(({impl, hint}) => {
         let impl = Editor.of_zipper(PersistentZipper.unpersist(impl));
         {
           impl,
           hint,
         };
