...
(* Source: hazel-programs/study/debugging/companion-plotter/companion-plotter.hz *)

let out : string * Haz3lcore.PersistentZipper.t =
  ( "Study / debugging / companion-plotter / companion-plotter",
    Haz3lcore.PersistentZipper.of_slide_text
      {x|# Companion Planting Grid #
...
(crop1 == "🌿" && crop2 == "🌱") then Beneficial
else if (crop1 == "🍄" && crop2 == "☘️") ||
(crop1 == "☘️" && crop2 == "🍄") then Beneficial
else if (crop1 == "🌱" && crop2 == "🍄") ||
(crop1 == "🍄" && crop2 == "🌱") then Harmful
else Neutral
in
...
hint "🌱 and 🌿 are companions"
test
companionEffect("🌱", "🌿") == Beneficial
end;

hint "🍄 and ☘️ are companions"
test
companionEffect("🍄", "☘️") == Beneficial
end;

hint "🌱 next to 🌿 gains health"
test
let m = garden(init, [
PlantCrop(0, 0),              # 🌱 at (0,0) #
SelectSeed(1),
PlantCrop(0, 1),              # 🌿 at (0,1) #
CalculateHealth
]) in
healthAt(m, 0, 0) > 50
end;
...
hint "🌱 and 🍄 are rivals"
test
companionEffect("🌱", "🍄") == Harmful
end;

hint "🌱 next to 🍄 loses health"
test
let m = garden(init, [
PlantCrop(1, 1),              # 🌱 at center #
SelectSeed(2),
PlantCrop(1, 0),              # 🍄 to the left #
CalculateHealth
]) in
healthAt(m, 1, 1) < 50
end;
...
test
companionEffect("🌱", "🌱") == Neutral
end;

hint "unrelated crops are neutral"
test
companionEffect("🌸", "☘️") == Neutral
end;
...
PlantCrop(1, 1),              # 🌱 in center #
SelectSeed(1),
PlantCrop(0, 1),              # 🌿 above: +10 #
SelectSeed(2),
PlantCrop(1, 0),              # 🍄 left: -10 #
CalculateHealth
]) in
...
SelectSeed(2),
PlantCrop(0, 1),              # 🍄 above: -10 #
PlantCrop(2, 1),              # 🍄 below: -10 #
PlantCrop(1, 0),              # 🍄 left: -10 #
PlantCrop(1, 2),              # 🍄 right: -10 #
CalculateHealth
]) in
healthAt(m, 1, 1) >= 0
end;
