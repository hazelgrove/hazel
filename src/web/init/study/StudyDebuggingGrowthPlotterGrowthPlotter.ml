...
(* Source: hazel-programs/study/debugging/growth-plotter/growth-plotter.hz *)

let out : string * Haz3lcore.PersistentZipper.t =
  ( "Study / debugging / growth-plotter / growth-plotter",
...
# Advance a growth stage by one step #
# Seed -> Sprout -> Growing -> Mature -> ReadyToHarvest #
...
# Check if a cell has a planted crop #
...
# Get the stage of a cell (defaults to Seed for empty) #
...
# Water the entire field - advance all planted crops #
...
# Get crop kind from a cell (for testing) #
...
# Growth stage progression tests #
...
let m = run(initModel, [
PlantCrop(0, 0),
WaterField,  # Only at Sprout stage #
HarvestCrop(0, 0)
]) in
isPlanted(getCell(m.field, 0, 0))
end;
