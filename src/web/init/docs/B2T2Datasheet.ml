let out : string * Haz3lcore.PersistentZipper.t =
  ( "[B2T2] Datasheet",
    {
      zipper =
        "((selection((focus Left)(content())(mode \
         Normal)))(backpack())(relatives((siblings(((Secondary((id \
         f27e32ec-3568-460a-a36b-6cfce99f99bf)(content(Comment\"# Reference \
         #\"))))(Secondary((id \
         fac62c32-b1d6-4109-acf4-cc3992f3ded8)(content(Whitespace\"\\n\"))))(Secondary((id \
         bf579f4b-bf06-4110-ad55-bdb5c42a06ba)(content(Whitespace\"\\n\"))))(Secondary((id \
         c0f67443-a299-45b4-b864-53662d953092)(content(Comment\"# Q. Where can \
         we learn about the programming medium covered by this datasheet? \
         #\"))))(Secondary((id \
         54592828-5a87-47dd-9ea8-98c02077fb90)(content(Whitespace\"\\n\"))))(Secondary((id \
         9a65ff7e-48f2-4801-9db8-b8440c2703ca)(content(Comment\"# (Feel free \
         to link to multiple kinds of artifacts: repositories, papers, videos, \
         etc. #\"))))(Secondary((id \
         b4dd9d82-bd8f-4ec5-ac4b-59bc0c3b9a75)(content(Whitespace\"\\n\"))))(Secondary((id \
         999d45aa-c156-4f51-8b99-e707074a01dd)(content(Comment\"# Please also \
         include version information where applicable.) #\"))))(Secondary((id \
         c9236855-8a5d-4851-ba93-e2fd64ea70ed)(content(Whitespace\"\\n\"))))(Secondary((id \
         518f9d49-0597-4833-ada9-bd62616465c3)(content(Whitespace\"\\n\"))))(Secondary((id \
         5757c069-2a31-4541-83a9-eed53e86e2a3)(content(Whitespace\"\\n\"))))(Secondary((id \
         15959d89-5be0-4e4a-bce6-00010f32c495)(content(Comment\"# Q. What is \
         the URL of the version of the benchmark being used? \
         #\"))))(Secondary((id \
         28c5a78d-f767-41fc-9810-90134ccb898e)(content(Whitespace\"\\n\"))))(Secondary((id \
         a796a88c-ce39-47c2-9c0e-77880c35aa4b)(content(Whitespace\"\\n\"))))(Secondary((id \
         9fbcaef1-e62f-4777-b5c8-baa5d067afe3)(content(Comment\"# Q. On what \
         date was this version of the datasheet last updated? \
         #\"))))(Secondary((id \
         071fbaa2-be5d-4591-85a2-08b86a5067b0)(content(Whitespace\"\\n\"))))(Secondary((id \
         5e309892-de31-4766-8957-b7fbfabb0868)(content(Whitespace\"\\n\"))))(Secondary((id \
         fab6fefc-4888-478e-a3dc-32e8acde3cdb)(content(Comment\"# Q. If you \
         are not using the latest benchmark available on that date, please \
         explain why not. #\"))))(Secondary((id \
         0196282a-8e74-463d-b963-152971dd5fa6)(content(Whitespace\"\\n\"))))(Secondary((id \
         c93628e3-0a3a-4436-a430-fe1848e78929)(content(Whitespace\"\\n\"))))(Secondary((id \
         348f7e3e-6ca1-4b94-8c21-55fe88f32dc7)(content(Comment\"# Example \
         Tables #\"))))(Secondary((id \
         2d0f9b6f-90e7-4585-8aee-1e08cddbd13f)(content(Whitespace\"\\n\"))))(Secondary((id \
         efb5ff6c-caf9-4fc1-8ac3-8160b9b1f41e)(content(Whitespace\"\\n\"))))(Secondary((id \
         06b53f0b-4c2a-4928-896c-7dcde82f5a5d)(content(Comment\"# Q. Do tables \
         express heterogeneous data, or must data be homogenized? \
         #\"))))(Secondary((id \
         31b46bb0-a060-4985-9425-895e27200f98)(content(Whitespace\"\\n\"))))(Secondary((id \
         b350be91-eab8-46b1-8584-0c6eabdb1c76)(content(Whitespace\"\\n\"))))(Secondary((id \
         66f4cecb-127c-41e5-8b9c-f936bb29882d)(content(Comment\"# Q. Do tables \
         capture missing data and, if so, how? Do missing values affect the \
         output constraints of any operations, for example `groupBy`? \
         #\"))))(Secondary((id \
         b8df7f79-6dff-4b5a-8057-a020f7892f5d)(content(Whitespace\"\\n\"))))(Secondary((id \
         8f59adec-5273-4be5-8702-20fda5f9161b)(content(Whitespace\"\\n\"))))(Secondary((id \
         a7acccc7-5c7b-44de-bb70-b4de6b353208)(content(Comment\"# Q. Are \
         mutable tables supported? Are there any limitations? \
         #\"))))(Secondary((id \
         6bce6449-5b61-468b-bef9-838f1d9750bd)(content(Whitespace\"\\n\"))))(Secondary((id \
         d81d6bed-6691-4120-899b-d25cfc829a29)(content(Whitespace\"\\n\"))))(Secondary((id \
         e860faca-a504-43c6-9238-4104aab66c78)(content(Comment\"# You may \
         reference, instead of duplicating, the responses to the above \
         questions in answering those below: #\"))))(Secondary((id \
         cd3fb2c8-0551-412f-83f5-f031ab327507)(content(Whitespace\"\\n\"))))(Secondary((id \
         592f9869-21dd-4d63-90ff-189e4d53b8ef)(content(Whitespace\"\\n\"))))(Secondary((id \
         781f2c3f-1e93-4348-a44d-9e33bdca8bb3)(content(Comment\"# Q. Which \
         tables are inexpressible? Why? #\"))))(Secondary((id \
         4ee37c98-3450-45c9-a9f7-23eb11632f28)(content(Whitespace\"\\n\"))))(Secondary((id \
         579316d6-4e5c-4314-bbc4-9cf2d5a6fb8a)(content(Whitespace\"\\n\"))))(Secondary((id \
         cfda6a20-853f-472f-a9d9-a6a904e93547)(content(Comment\"# Q. Which \
         tables are only partially expressible? Why, and what\\226\\128\\153s \
         missing? #\"))))(Secondary((id \
         6cc998a7-d463-4f66-b139-46580b0bd028)(content(Whitespace\"\\n\"))))(Secondary((id \
         4a441abc-a1a0-4673-966b-1c9ed4e4590f)(content(Whitespace\"\\n\"))))(Secondary((id \
         7a7dd45e-3d6a-4c9f-bd0f-8f4901626838)(content(Comment\"# Q. Which \
         tables\\226\\128\\153 expressibility is unknown? Why? \
         #\"))))(Secondary((id \
         6fd6137d-5922-4c94-b5f3-692c17607ab1)(content(Whitespace\"\\n\"))))(Secondary((id \
         a89e73a7-d5ea-4ef5-8030-6a8b144fe078)(content(Whitespace\"\\n\"))))(Secondary((id \
         0f19f3ce-10e5-4c55-85db-efbc1d6848c3)(content(Comment\"# Q. Which \
         tables can be expressed more precisely than in the benchmark? How? \
         #\"))))(Secondary((id \
         b8be6ed7-b380-48b7-8016-6aaa9b6bf6bc)(content(Whitespace\"\\n\"))))(Secondary((id \
         e6d268ec-4777-4cb3-900e-2c87b3546bb7)(content(Whitespace\"\\n\"))))(Secondary((id \
         9254f65f-9230-4563-ac10-8e5a0f3aad8b)(content(Comment\"# Q. How \
         direct is the mapping from the tables in the benchmark to \
         representations in your system? How complex is the encoding? \
         #\"))))(Secondary((id \
         56ca255b-5da2-4fe9-835f-521edde7df50)(content(Whitespace\"\\n\"))))(Secondary((id \
         4de86a16-ad75-4595-8d43-19000b2a5090)(content(Whitespace\"\\n\"))))(Secondary((id \
         e06217e6-3cca-4ed4-a0ea-d3d1a48dc902)(content(Comment\"# TableAPI \
         #\"))))(Secondary((id \
         fb93a854-ceea-43f2-a4d9-f0192bf6a916)(content(Whitespace\"\\n\"))))(Secondary((id \
         4f965f19-347d-4f59-bc1f-18860f28792b)(content(Whitespace\"\\n\"))))(Secondary((id \
         494cb373-e2a5-4f42-ae34-37e0bebd386c)(content(Comment\"# Q. Are there \
         consistent changes made to the way the operations are represented? \
         #\"))))(Secondary((id \
         2efa0b10-24a4-4ce8-9bdd-45131344b656)(content(Whitespace\"\\n\"))))(Secondary((id \
         d5866307-1378-42ac-91d3-c7ac2f2b4101)(content(Whitespace\"\\n\"))))(Secondary((id \
         3c59f639-8162-486b-85e1-f36aa2632633)(content(Comment\"# Q. Which \
         operations are entirely inexpressible? Why? #\"))))(Secondary((id \
         2ac2f1b4-339f-4cd5-a6fa-809ceadadc96)(content(Whitespace\"\\n\"))))(Secondary((id \
         5c19aa2a-48fb-4666-83ec-112af9646ca2)(content(Whitespace\"\\n\"))))(Secondary((id \
         0f3e54d4-a31e-48bd-a1cc-a7ccd0dd08ef)(content(Comment\"# Q. Which \
         operations are only partially expressible? Why, and \
         what\\226\\128\\153s missing? #\"))))(Secondary((id \
         4c5ffd4f-83fe-4379-93ac-84ad33ba5297)(content(Whitespace\"\\n\"))))(Secondary((id \
         ec31c9a2-01dc-4325-93a2-6c5bded71a45)(content(Whitespace\"\\n\"))))(Secondary((id \
         df258a0a-8bb8-49ce-ae73-f5eea68fd135)(content(Comment\"# Q. Which \
         operations\\226\\128\\153 expressibility is unknown? Why? \
         #\"))))(Secondary((id \
         1cb4638f-61c5-43b0-b651-2b9fff444257)(content(Whitespace\"\\n\"))))(Secondary((id \
         e48faee5-3f86-4f69-8b6d-cd348728d92f)(content(Whitespace\"\\n\"))))(Secondary((id \
         91be40f2-2343-46ec-bf1c-3be47eddda51)(content(Comment\"# Q. Which \
         operations can be expressed more precisely than in the benchmark? \
         How? #\"))))(Secondary((id \
         fe4c0cc7-5236-47b8-af77-7c05edeacc79)(content(Whitespace\"\\n\"))))(Secondary((id \
         7132096e-8e86-4746-9d91-87eb5d4d3366)(content(Whitespace\"\\n\"))))(Secondary((id \
         f01ce7e4-671e-4b28-ac28-d650ba7e6829)(content(Comment\"# Example \
         Programs #\"))))(Secondary((id \
         c6c4a9c1-5f61-41e8-b7d3-0cd840b301f6)(content(Whitespace\"\\n\"))))(Secondary((id \
         e3bf348f-4fdb-4a1c-b8d7-93bd0a29c96f)(content(Whitespace\"\\n\"))))(Secondary((id \
         2c471aef-b509-4ce0-bd27-89cca11bfd08)(content(Comment\"# Q. Which \
         examples are inexpressible? Why? #\")))))((Secondary((id \
         46adee40-a579-4289-8197-bd760ab1c2e6)(content(Whitespace\"\\n\"))))(Secondary((id \
         a0e935c2-4d50-457d-911a-5f4abe1f09d6)(content(Whitespace\"\\n\"))))(Secondary((id \
         2f50c953-a5d5-4c0d-8bc3-7c6ae9373687)(content(Comment\"# Q. Which \
         examples\\226\\128\\153 expressibility is unknown? Why? \
         #\"))))(Secondary((id \
         55385d50-1232-4ad3-8ce1-8e0ba8a98d8f)(content(Whitespace\"\\n\"))))(Secondary((id \
         c861952e-c29b-47c3-8cc9-1a7ec95b92af)(content(Whitespace\"\\n\"))))(Secondary((id \
         57e71972-49f8-49bd-ba9b-6ac633b15e55)(content(Comment\"# Q. Which \
         examples, or aspects thereof, can be expressed especially precisely? \
         How? #\"))))(Secondary((id \
         04195c01-c343-4571-9be2-076e14171b7f)(content(Whitespace\"\\n\"))))(Secondary((id \
         4b056bb6-2ae9-4ab3-99b3-3d3d5191d8b3)(content(Whitespace\"\\n\"))))(Secondary((id \
         105496b0-2ffd-43b7-9ece-ac98560d02f3)(content(Comment\"# Q. How \
         direct is the mapping from the pseudocode in the benchmark to \
         representations in your system? How complex is the encoding? \
         #\"))))(Secondary((id \
         6008036c-b890-4498-93f7-129d2fec79fd)(content(Whitespace\"\\n\"))))(Secondary((id \
         17c563fa-c403-48ab-8a94-e0d923118f4d)(content(Whitespace\"\\n\"))))(Secondary((id \
         92a66d3e-54b9-4dea-b660-072d16dc0e2f)(content(Comment\"# Errors \
         #\"))))(Secondary((id \
         83a42096-b515-4269-adc4-325f935115d6)(content(Whitespace\"\\n\"))))(Secondary((id \
         f1cbc83e-16cf-4314-a590-70440b61d365)(content(Whitespace\"\\n\"))))(Secondary((id \
         5bf33e33-13f9-47a8-b464-01692a9672f0)(content(Comment\"# There are \
         (at least) two parts to errors: representing the source program that \
         causes the error, and generating output that explains it. The term \
         \\226\\128\\156error situation\\226\\128\\157 refers to a \
         representation of the cause of the error in the program source. \
         #\"))))(Secondary((id \
         eec1e50b-d2b8-4ab5-82d7-86e5e95c59a3)(content(Whitespace\"\\n\"))))(Secondary((id \
         d750aad5-56e8-4762-a1ea-17ba6e92dfda)(content(Whitespace\"\\n\"))))(Secondary((id \
         386670d0-3a2a-42b5-9880-8a326b384d96)(content(Comment\"# For each \
         error situation it may be that the language: #\"))))(Secondary((id \
         b0b52096-053f-4208-aa38-bd7010ee730c)(content(Whitespace\"\\n\"))))(Secondary((id \
         4bd37098-a582-442b-84e9-3f82258fa139)(content(Whitespace\"\\n\"))))(Secondary((id \
         4cacc62a-9940-47fa-8333-0a175c55ea85)(content(Comment\"# - \
         isn\\226\\128\\153t expressive enough to capture it \
         #\"))))(Secondary((id \
         30afd67c-8b8e-4c40-ac07-bfe6ec0cdf50)(content(Whitespace\"\\n\"))))(Secondary((id \
         2e07e41c-f909-4de2-a468-3ad9a231b6c8)(content(Comment\"# - can at \
         least partially express the situation #\"))))(Secondary((id \
         f758c765-369a-4910-9e0b-b7f25b9a33f1)(content(Whitespace\"\\n\"))))(Secondary((id \
         cffa90ab-2416-4c42-9ce8-14df09ce4dbc)(content(Comment\"# - prevents \
         the program from being constructed #\"))))(Secondary((id \
         ec97f6d8-cb5d-47c1-9224-9aec6ec4948c)(content(Whitespace\"\\n\"))))(Secondary((id \
         cb4e3cc0-78e7-4415-aadd-99c7fb9aff86)(content(Whitespace\"\\n\"))))(Secondary((id \
         eb3e5a12-5c6b-43ce-8d75-5146b45aae39)(content(Comment\"# \
         Expressiveness, in turn, can be for multiple artifacts: \
         #\"))))(Secondary((id \
         d7de6552-78f8-4169-adc1-3b2c8a36d829)(content(Whitespace\"\\n\"))))(Secondary((id \
         adc578fb-869e-473b-937b-3193d2ebeed1)(content(Whitespace\"\\n\"))))(Secondary((id \
         628cb572-6834-4c50-941c-47702a5f12ef)(content(Comment\"# - the buggy \
         versions of the programs #\"))))(Secondary((id \
         dd0828fe-88dd-4bfd-94c8-05ef31913afc)(content(Whitespace\"\\n\"))))(Secondary((id \
         0d8bdf0d-a50b-4509-8a65-ebf5b0c38515)(content(Comment\"# - the \
         correct variants of the programs #\"))))(Secondary((id \
         37388d64-b4af-4b88-abf3-37cf76c7e8e8)(content(Whitespace\"\\n\"))))(Secondary((id \
         39baf50c-9d15-4b0c-997d-6e0f12fbf980)(content(Comment\"# - the type \
         system\\226\\128\\153s representation of the constraints \
         #\"))))(Secondary((id \
         cb4f37bc-59a7-4146-a3ef-cb5c3b2c19ec)(content(Whitespace\"\\n\"))))(Secondary((id \
         8ebdbf8f-d024-4df9-ab6f-59dddea60743)(content(Comment\"# - the type \
         system\\226\\128\\153s reporting of the violation \
         #\"))))(Secondary((id \
         18461bbf-de07-4dc6-a20c-0adcb1bd3f0e)(content(Whitespace\"\\n\"))))(Secondary((id \
         e4f60462-82ec-4605-8415-dc228c5f048f)(content(Whitespace\"\\n\"))))(Secondary((id \
         b44f1762-9a28-47da-baa2-e2b6bdcfd96c)(content(Comment\"# Q. Which \
         error situations are known to be inexpressible? Why? \
         #\"))))(Secondary((id \
         748e86ea-f7be-44f0-8acd-cf2723882887)(content(Whitespace\"\\n\"))))(Secondary((id \
         5398268c-b809-4637-b808-aa58fa390edb)(content(Whitespace\"\\n\"))))(Secondary((id \
         f8d8f0f8-8960-47e6-a31b-fa9258c6cb6b)(content(Comment\"# Q. Which \
         error situations are only partially expressible? Why, and \
         what\\226\\128\\153s missing? #\"))))(Secondary((id \
         b11f8d73-f2e9-418a-9f18-8c855b29e078)(content(Whitespace\"\\n\"))))(Secondary((id \
         116298d0-2fd4-49a5-beab-1c4f6bd82d84)(content(Whitespace\"\\n\"))))(Secondary((id \
         a76b81d8-3631-4548-ae62-1f0ac532a1e6)(content(Comment\"# Q. Which \
         error situations\\226\\128\\153 expressibility is unknown? Why? \
         #\"))))(Secondary((id \
         7aaf8cd6-f671-4c68-9102-4e195f04ff9d)(content(Whitespace\"\\n\"))))(Secondary((id \
         00470f94-14ac-4f8a-92d6-7f2277788680)(content(Whitespace\"\\n\"))))(Secondary((id \
         4e9313dc-3c89-46a4-b6f8-343414aea211)(content(Comment\"# Q. Which \
         error situations can be expressed more precisely than in the \
         benchmark? How? #\"))))(Secondary((id \
         f1f1705c-a193-4263-9994-20a6a2ac13c4)(content(Whitespace\"\\n\"))))(Secondary((id \
         7bef89c8-c8d8-4233-b9ee-263a6911c657)(content(Whitespace\"\\n\"))))(Secondary((id \
         e955d15d-36eb-4b9c-bd83-c938f91e98ff)(content(Comment\"# Q. Which \
         error situations are prevented from being constructed? How? \
         #\"))))(Secondary((id \
         ec6abb58-c2a3-4deb-9e76-6f0662cbae6b)(content(Whitespace\"\\n\"))))(Secondary((id \
         aef88fac-b166-459a-9a7e-cb35424c030f)(content(Whitespace\"\\n\"))))(Secondary((id \
         fddc1faa-fb50-4969-820c-063e351e94dd)(content(Comment\"# Q. For each \
         error situation that is at least partially expressible, what is the \
         quality of feedback to the programmer? #\"))))(Secondary((id \
         1ba04c54-3f58-4e0b-994a-2c9b4acbb8d1)(content(Whitespace\"\\n\"))))(Secondary((id \
         bc060f1f-4173-4cd4-ad37-437ec32be8b8)(content(Whitespace\"\\n\"))))(Secondary((id \
         d3b50aec-185d-4040-b563-77f1472a38e9)(content(Comment\"# Q. For each \
         error situation that is prevented from being constructed, what is the \
         quality of feedback to the programmer? #\"))))(Grout((id \
         05a7daa3-0774-44d0-aead-fe4b4b0d9f62)(shape \
         Convex))))))(ancestors())))(caret Outer))";
      backup_text =
        "# Reference #\n\n\
         # Q. Where can we learn about the programming medium covered by this \
         datasheet? #\n\
         # (Feel free to link to multiple kinds of artifacts: repositories, \
         papers, videos, etc. #\n\
         # Please also include version information where applicable.) #\n\n\n\
         # Q. What is the URL of the version of the benchmark being used? #\n\n\
         # Q. On what date was this version of the datasheet last updated? #\n\n\
         # Q. If you are not using the latest benchmark available on that \
         date, please explain why not. #\n\n\
         # Example Tables #\n\n\
         # Q. Do tables express heterogeneous data, or must data be \
         homogenized? #\n\n\
         # Q. Do tables capture missing data and, if so, how? Do missing \
         values affect the output constraints of any operations, for example \
         `groupBy`? #\n\n\
         # Q. Are mutable tables supported? Are there any limitations? #\n\n\
         # You may reference, instead of duplicating, the responses to the \
         above questions in answering those below: #\n\n\
         # Q. Which tables are inexpressible? Why? #\n\n\
         # Q. Which tables are only partially expressible? Why, and \
         what\226\128\153s missing? #\n\n\
         # Q. Which tables\226\128\153 expressibility is unknown? Why? #\n\n\
         # Q. Which tables can be expressed more precisely than in the \
         benchmark? How? #\n\n\
         # Q. How direct is the mapping from the tables in the benchmark to \
         representations in your system? How complex is the encoding? #\n\n\
         # TableAPI #\n\n\
         # Q. Are there consistent changes made to the way the operations are \
         represented? #\n\n\
         # Q. Which operations are entirely inexpressible? Why? #\n\n\
         # Q. Which operations are only partially expressible? Why, and \
         what\226\128\153s missing? #\n\n\
         # Q. Which operations\226\128\153 expressibility is unknown? Why? #\n\n\
         # Q. Which operations can be expressed more precisely than in the \
         benchmark? How? #\n\n\
         # Example Programs #\n\n\
         # Q. Which examples are inexpressible? Why? #\n\n\
         # Q. Which examples\226\128\153 expressibility is unknown? Why? #\n\n\
         # Q. Which examples, or aspects thereof, can be expressed especially \
         precisely? How? #\n\n\
         # Q. How direct is the mapping from the pseudocode in the benchmark \
         to representations in your system? How complex is the encoding? #\n\n\
         # Errors #\n\n\
         # There are (at least) two parts to errors: representing the source \
         program that causes the error, and generating output that explains \
         it. The term \226\128\156error situation\226\128\157 refers to a \
         representation of the cause of the error in the program source. #\n\n\
         # For each error situation it may be that the language: #\n\n\
         # - isn\226\128\153t expressive enough to capture it #\n\
         # - can at least partially express the situation #\n\
         # - prevents the program from being constructed #\n\n\
         # Expressiveness, in turn, can be for multiple artifacts: #\n\n\
         # - the buggy versions of the programs #\n\
         # - the correct variants of the programs #\n\
         # - the type system\226\128\153s representation of the constraints #\n\
         # - the type system\226\128\153s reporting of the violation #\n\n\
         # Q. Which error situations are known to be inexpressible? Why? #\n\n\
         # Q. Which error situations are only partially expressible? Why, and \
         what\226\128\153s missing? #\n\n\
         # Q. Which error situations\226\128\153 expressibility is unknown? \
         Why? #\n\n\
         # Q. Which error situations can be expressed more precisely than in \
         the benchmark? How? #\n\n\
         # Q. Which error situations are prevented from being constructed? \
         How? #\n\n\
         # Q. For each error situation that is at least partially expressible, \
         what is the quality of feedback to the programmer? #\n\n\
         # Q. For each error situation that is prevented from being \
         constructed, what is the quality of feedback to the programmer? # ";
    } )
