...
/* TableRendererView - the web (Vdom) half of TableRenderer: column menus,
   table rendering, and the badge icon. The logic half (parsing, menu state,
   serialization) lives in haz3lcore's TableRenderer. Paired with it in
   RichProbeViewRegistry. */

type model = TableRenderer.model;
type action = TableRenderer.action;
type value = TableRenderer.value;
