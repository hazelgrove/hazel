...
let view_of_layout =
    (diff_steps: list(CursorPath.steps), l: KindLayout.t): list(Node.t) => {
  let col: ref(int) = ref(0);
  let rec go = (indent: int, dpaths, l: KindLayout.t) =>
    switch (l) {
    | Linebreak =>
      col := indent;
      [
...
let view =
    (~width=30, ~pos=0, ~diff_steps: list(CursorPath.steps)=[], k: Kind.t)
    : Node.t => {
  let l =
    k
    |> KindDoc.mk(~enforce_inline=false)
    |> LayoutOfDoc.layout_of_doc(~width, ~pos);
  switch (l) {
  | None => failwith("unimplemented: view_of_kind on layout failure")
  | Some(l) =>
    Node.div(
      [Attr.classes(["code", "KindCode"])],
