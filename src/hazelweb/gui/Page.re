...
let type_label =
  div(~attr=Attr.class_("label"), [text("Result of type: ")]);

let type_view = (ty: HTyp.t): Node.t => {
  div(~attr=Attr.class_("cell-type"), [type_label, HTypCode.view(ty)]);
};
...
let card_panel = (~inject, ~model, ~result: Result.t): Node.t =>
  div(
    ~attr=Attr.id(ViewUtil.card_dom_id),
    [
      card_caption(model),
