let get_model_action =
  ...
  | CtrlOrCmd_S => Some(SerializeToConsole)
  | CtrlOrCmd_Z => Some(Undo)
  | CtrlOrCmd_Shift_Z => Some(Redo)
  | CtrlOrCmd_Alt_I => Some(EditAction(SwapUp))
  | CtrlOrCmd_Alt_K => Some(EditAction(SwapDown))
  | CtrlOrCmd_Alt_J => Some(EditAction(SwapLeft))
  | CtrlOrCmd_Alt_L => Some(EditAction(SwapRight))
  };
};
