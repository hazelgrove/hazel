...
let pat_hole_view =
  ...
  let val_open = cursor_inspector.strategy_guide_val;
  let structure_open = cursor_inspector.strategy_guide_structure;
  let any_open = cursor_inspector.strategy_guide_any;
  let ignore_open = cursor_inspector.strategy_guide_ignore;
  ...
  let ignore =
    subsection_header(Toggle_strategy_guide_ignore, "Ignore?", ignore_open);
  let ignore_body =
    Node.div(
      [Attr.classes(["panel-title-bar", "body-bar"])],
      [ignore_view],
