...
let get_all_actions = (cursor_info: CursorInfo.t) => {
  let keycombo_actions =
    Hashtbl.fold(
      (combo, Cell.{action_fn, description}, acc) => {
        let action = action_fn(cursor_info);
        [(combo, description, action), ...acc];
      },
      Cell.kc_actions,
      [],
    );
...
let view = (~inject: Update.Action.t => Vdom.Event.t, model: Model.t) => {
  ...
  let all_actions = get_all_actions(cursor_info);
  let action_nodes =
    List.map(
      ((combo, description, action)) => {
        let description = Vdom.Node.text(description);
  ...
  let panel_body =
    Node.div(
      [Attr.classes(["action-panel-body", "panel-body"])],
      action_nodes,
    );
...
/*
 let view = (~inject: Update.Action.t => Vdom.Event.t, model: Model.t) => {
   ...
   /*
    let constructNum =
      action_button(
        Action.Construct(Action.SChar("N")),
   ...
   let constructNewLine =
     ...
     /*
    let constructVar = {
      let action = Action.Construct(SChar("N"));
      info_button(
        ~can_perform=is_action_allowed(action),
        Vdom.Node.text("enter variables directly"),
      );
    };

    let constructBoolLit = {
      let action = Action.Construct(SChar("B"));
      info_button(
        ~can_perform=is_action_allowed(action),
