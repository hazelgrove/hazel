...
## Usage
The Hazel CLI can be invoked from the command line using the `hzcli` script located in the root of the repository. The script accepts various commands and options to perform different tasks.

### Running a Hazel Program
To run a Hazel program, use the `run` command followed by the path to the Hazel file. For example:

```sh
$ ./hzcli run path/to/hazel_file.hz
```
...
```sh
$ ./hzcli format path/to/hazel_file.hz
```
...
```sh
$ ./hzcli analyze path/to/hazel_file.hz
```
...
```sh
$ echo "let x = 5 in x + 3" | ./hzcli analyze -
